...
// ==========================================
// CUSTOS ADQUIRIDOS - FUNCIONALIDADES
// ==========================================

// Configuração inicial do formulário
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Custos Adquiridos - Inicializando...');
    ...
    console.log('✅ Custos Adquiridos - Inicializado com sucesso');
});

function configurarPeriodoAtual() {
    const campoPeriodo = document.getElementById('periodoEmissao');
    if (campoPeriodo) {
        const hoje = new Date();
...
function configurarEventListeners() {
    ...
    // Checkbox "Mostrar apenas despesas"
    const checkboxDespesas = document.getElementById('mostrarDespesas');
    if (checkboxDespesas) {
        checkboxDespesas.addEventListener('change', function() {
            const isChecked = this.checked;
            console.log('Mostrar apenas despesas:', isChecked ? 'Sim' : 'Não');
            
            // Aqui você pode implementar lógica adicional
            // Por exemplo, filtrar campos ou alterar comportamento
        });
    }
    
    // Validação em tempo real dos campos
    const campos = ['tipoDocumento', 'profissionalFornecedor', 'centroResultado', 'categoriaFinanceira', 'tipoImpressao'];
...
function coletarDadosFormulario() {
    const dados = {
        periodoEmissao: document.getElementById('periodoEmissao')?.value || '',
        ...
        categoriaFinanceira: document.getElementById('categoriaFinanceira')?.value || 'todos',
        tipoImpressao: document.getElementById('tipoImpressao')?.value || 'resumido',
        mostrarDespesas: document.getElementById('mostrarDespesas')?.checked || false
    };
...
function validarFormulario() {
    ...
    if (!dados.periodoEmissao.trim()) {
        erros.push('Período de emissão é obrigatório');
        marcarCampoComErro('periodoEmissao', 'Período de emissão é obrigatório');
    }
    
    // Outros campos são opcionais, apenas verificar se são válidos
    const camposSelect = ['tipoDocumento', 'profissionalFornecedor', 'centroResultado', 'categoriaFinanceira', 'tipoImpressao'];
    camposSelect.forEach(campo => {
        const elemento = document.getElementById(campo);
        if (elemento && !elemento.value) {
            elemento.value = 'todos'; // Definir valor padrão se vazio
        }
    });
...
function marcarCampoComErro(fieldId, mensagem) {
    const campo = document.getElementById(fieldId);
    if (!campo) return;
...
function limparFormulario() {
    ...
    const camposSelect = ['tipoDocumento', 'profissionalFornecedor', 'centroResultado', 'categoriaFinanceira'];
    ...
    const checkboxDespesas = document.getElementById('mostrarDespesas');
    if (checkboxDespesas) {
        checkboxDespesas.checked = false;
    }
...
function visualizarRelatorio() {
    console.log('📊 Gerando relatório de custos adquiridos...');
    ...
    setTimeout(() => {
        ...
        // Mostrar resultado
        mostrarResultadoRelatorio(dados);
...
function mostrarResultadoRelatorio(dados) {
    // Aqui você implementaria a exibição do relatório
    // Por exemplo, abrir modal, navegar para página de resultados, etc.
    
    console.log('📊 Exibindo relatório de Custos Adquiridos:');
    console.log('- Período:', dados.periodoEmissao);
    ...
    console.log('- Categoria Financeira:', dados.categoriaFinanceira);
    console.log('- Tipo de Impressão:', dados.tipoImpressao);
    console.log('- Mostrar apenas despesas:', dados.mostrarDespesas);
    
    // Exemplo de implementação futura:
    // window.open(`/relatorios/custos-adquiridos?${new URLSearchParams(dados).toString()}`, '_blank');
}
...
console.log('✅ Custos Adquiridos - Script carregado com sucesso');
