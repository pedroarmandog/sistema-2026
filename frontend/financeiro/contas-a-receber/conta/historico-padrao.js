...
let historicosData = [
  {
    id: 1,
    codigo: 999,
    descricao: "Transferência Frente de Caixa",
    ativo: true,
  },
  ...
  { id: 6, codigo: 994, descricao: "Devolução", ativo: true },
  {
    id: 7,
    codigo: 993,
    descricao: "Recebimento Operadora de Cartão",
    ativo: true,
  },
...
function renderizarTabelaHistoricos() {
  ...
  tbody.innerHTML = historicosFiltrados
    .map(
      (historico) => `
        <tr>
...
function pesquisarHistoricos() {
  ...
  if (!termoPesquisa) {
    ...
    historicosFiltrados = historicosData.filter(
      (historico) =>
...
function editarHistorico(id) {
  ...
  if (historico) {
    ...
    alert(
      `Editar histórico: ${historico.descricao}\n\nFuncionalidade será implementada em breve!`,
    );
  }
}
...
async function excluirHistorico(id) {
  ...
  if (historico) {
    const confirmou = await confirmar(
      `Deseja realmente excluir o histórico "${historico.descricao}"?`,
    );
    if (confirmou) {
...
function adicionarNovoHistorico() {
  alert(
    "Funcionalidade de adicionar novo histórico será implementada em breve!",
  );
}
