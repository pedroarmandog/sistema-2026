...
function carregarContas() {
    ...
    // Quando integrar com API, carregar as contas aqui
    contasData.forEach(conta => {
        const option = document.createElement('option');
        option.value = conta.id;
        option.textContent = conta.identificacao;
...
function atualizarSaldoConta(contaId) {
    ...
    const conta = contasData.find(c => c.id === parseInt(contaId));
    if (conta) {
        // Aqui você integraria com a API para buscar o saldo atual
        saldoElement.textContent = `R$ ${formatarValor(conta.saldo || 0)}`;
    } else {
        saldoElement.textContent = 'R$ 0,00';
    }
}
...
function carregarMovimentacoes(contaId, periodo = null) {
    ...
    // Aqui você integraria com a API para buscar as movimentações
    // Por enquanto, mostrar estado vazio
    mostrarEstadoVazio('Nenhuma movimentação encontrada para esta conta');
}

// Função para mostrar estado vazio
function mostrarEstadoVazio(mensagem = 'Nenhuma movimentação encontrada') {
    const container = document.querySelector('.movimentacoes-container');
    if (!container) return;

    container.innerHTML = `
        <div class="empty-state-movimento">
            <i class="fas fa-chart-line"></i>
            <p>${mensagem}</p>
            <p class="empty-subtitle">Selecione uma conta para visualizar as movimentações</p>
        </div>
    `;
}

// Função para adicionar nova movimentação
function adicionarNovaMovimentacao() {
    alert('Funcionalidade de adicionar movimentação será implementada em breve!\n\nAqui você poderá integrar com a API.');
}

// Função para configurar event listeners
function configurarEventListenersMovimento() {
    // Botão Adicionar Movimentação
    const btnAdicionar = document.getElementById('btnAdicionarMovimento');
    if (btnAdicionar) {
        btnAdicionar.addEventListener('click', adicionarNovaMovimentacao);
    }
    ...
    if (filtroPeriodo) {
        filtroPeriodo.addEventListener('change', function() {
            if (contaSelecionada) {
                carregarMovimentacoes(contaSelecionada, this.value);
            }
        });
    }
}
...
// Inicializar página
function inicializarMovimentoConta() {
    configurarEventListenersMovimento();
    carregarContas();
    mostrarEstadoVazio();
}
