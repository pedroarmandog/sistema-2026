...
// Dados dos bancos (pré-cadastrados como padrão do sistema)
let bancosData = [
    { id: 1, descricao: 'BANCO DO BRASIL S/A', codigo: 1, ativo: true },
    { id: 2, descricao: 'BANCO REAL', codigo: 356, ativo: true },
    { id: 3, descricao: 'BANCO SANTANDER (BRASIL) S.A.', codigo: 33, ativo: true },
    { id: 4, descricao: 'BESC S/A.', codigo: 27, ativo: false },
    { id: 5, descricao: 'BRADESCO', codigo: 237, ativo: true },
    { id: 6, descricao: 'CAIXA ECONÔMICA FEDERAL', codigo: 104, ativo: true },
    { id: 7, descricao: 'H.S.B.C. BAMERINDOS', codigo: 399, ativo: true },
    { id: 8, descricao: 'ITAÚ', codigo: 341, ativo: true },
    { id: 9, descricao: 'PAGAMENTO EM CARTEIRA', codigo: 0, ativo: true },
    { id: 10, descricao: 'SICRED', codigo: 412, ativo: true },
    { id: 11, descricao: 'UNIBANCO', codigo: 409, ativo: true }
];

let bancosFiltrados = [...bancosData];

// Função para renderizar tabela de bancos
function renderizarTabelaBancos() {
    const tbody = document.getElementById('tabelaBancoBody');
    if (!tbody) return;

    if (!bancosFiltrados || bancosFiltrados.length === 0) {
        tbody.innerHTML = `
            <tr>
    ...
    tbody.innerHTML = bancosFiltrados.map(banco => `
        <tr>
            <td>${banco.descricao}</td>
            <td>${banco.codigo}</td>
            <td>
                ${banco.ativo ? 
                    '<i class="fas fa-check icon-ativo-banco"></i>' : 
                    '<i class="fas fa-times icon-inativo-banco"></i>'
                }
            </td>
            <td>
                <button class="btn-editar-banco" onclick="editarBanco(${banco.id})" title="Editar">
                    <i class="fas fa-edit"></i>
                </button>
            </td>
            <td>
                <button class="btn-excluir-banco" onclick="excluirBanco(${banco.id})" title="Excluir">
...
// Função para pesquisar bancos
function pesquisarBancos() {
    const termoPesquisa = document.getElementById('pesquisaBanco').value.toLowerCase().trim();
    
    if (!termoPesquisa) {
        bancosFiltrados = [...bancosData];
    } else {
        bancosFiltrados = bancosData.filter(banco => 
            banco.descricao.toLowerCase().includes(termoPesquisa)
        );
    }
...
function editarBanco(id) {
    ...
    if (banco) {
        console.log('Editar banco:', banco);
        alert(`Editar banco: ${banco.descricao}\n\nFuncionalidade será implementada em breve!`);
    }
}
...
function excluirBanco(id) {
    const banco = bancosData.find(b => b.id === id);
    if (banco) {
        const confirmar = confirm(`Deseja realmente excluir o banco "${banco.descricao}"?`);
        if (confirmar) {
            bancosData = bancosData.filter(b => b.id !== id);
            bancosFiltrados = bancosFiltrados.filter(b => b.id !== id);
            renderizarTabelaBancos();
...
function adicionarNovoBanco() {
    alert('Funcionalidade de adicionar novo banco será implementada em breve!');
}

// Função para configurar event listeners
function configurarEventListenersBanco() {
    // Botão Adicionar Banco
    const btnAdicionar = document.getElementById('btnAdicionarBanco');
    if (btnAdicionar) {
        btnAdicionar.addEventListener('click', adicionarNovoBanco);
    }

    // Botão Pesquisar
    const btnPesquisar = document.getElementById('btnPesquisarBanco');
    if (btnPesquisar) {
        btnPesquisar.addEventListener('click', pesquisarBancos);
    }

    // Input de pesquisa (Enter)
    const inputPesquisa = document.getElementById('pesquisaBanco');
    if (inputPesquisa) {
