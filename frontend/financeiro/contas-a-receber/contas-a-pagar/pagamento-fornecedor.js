...
// Dados de exemplo para pagamentos (vazio por padrão)
let pagamentosData = [];
...
function renderizarTabelaPagamentos() {
            ...
            <tr class="empty-state">
                <td colspan="12">
                    <div class="empty-message">
                        ...
                        <p>Nenhum documento a pagar encontrado</p>
                    </div>
                </td>
            </tr>
        `;
    ...
    tbody.innerHTML = pagamentosData.map(doc => `
        <tr>
            ...
            <td>${doc.nota || '-'}</td>
            <td>R$ ${formatarValor(doc.saldo)}</td>
            <td>R$ ${formatarValor(doc.multa)}</td>
            <td>R$ ${formatarValor(doc.juros)}</td>
            <td>R$ ${formatarValor(doc.desconto)}</td>
            <td><strong>R$ ${formatarValor(doc.pagar)}</strong></td>
            <td>
...
// Função para pesquisar pagamentos
function realizarPesquisa() {
    ...
    console.log('Pesquisando pagamentos...', { fornecedor, documento, nota, valor, dataVencimento, previsao });
...
function configurarEventListeners() {
    ...
    const camposFiltro = ['filtroFornecedor', 'filtroDocumento', 'filtroNota', 'filtroValor'];
    ...
    // Botão Doc. Pagar
    const btnDocPagar = document.getElementById('btnDocPagar');
    if (btnDocPagar) {
        btnDocPagar.addEventListener('click', function() {
            alert('Funcionalidade de criar documento a pagar será implementada em breve!');
        });
    }
    ...
    if (btnPagar) {
        btnPagar.addEventListener('click', function() {
            if (documentosSelecionados.length === 0) {
                alert('Selecione ao menos um documento para pagar.');
                return;
            }
            alert(`Processar pagamento de ${documentosSelecionados.length} documento(s)`);
        });
    }
