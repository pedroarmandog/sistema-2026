...
async function carregarDetalhesCliente(clienteId) {
    try {
        ...
        // Fazer a requisição
        console.log('� Fazendo requisição para:', `http://localhost:3000/api/clientes/${clienteId}`);
...
function preencherDetalhesCliente(cliente) {
    ...
    try {
        preencherElemento('clienteDataNascimento', formatarData(cliente.data_nascimento));
    } catch (error) {
        console.error('❌ Erro ao formatar data:', error);
        preencherElemento('clienteDataNascimento', cliente.data_nascimento);
    }
    
    preencherElemento('clienteIdade', cliente.idade ? `${cliente.idade} anos` : null);
    ...
    try {
        preencherElemento('clienteLimiteCredito', formatarMoeda(cliente.limite_credito));
    } catch (error) {
    ...
    try {
        if (cliente.pets && Array.isArray(cliente.pets) && cliente.pets.length > 0) {
                ...
                try {
                    ...
                    if (resp.ok) {
                        ...
                        if (json.success && Array.isArray(json.pets)) {
                            console.log('🔁 Pets carregados via /api/pets:', json.pets.length);
...
function showHaverInline() {
    ...
    try {
        if (clienteAtual) {
            saldo = formatarMoeda(clienteAtual.saldo_haver || clienteAtual.saldo || clienteAtual.haver || 0);
        }
    } catch (e) { console.warn('Erro ao obter saldo haver', e); }
    ...
    if (tbody) {
        ...
        let movimentos = [];
        if (clienteAtual) {
            movimentos = clienteAtual.haver_movimentos || clienteAtual.movimentos_haver || clienteAtual.movimentos || [];
        }

        if (!Array.isArray(movimentos) || movimentos.length === 0) {
            const tr = document.createElement('tr');
            tr.innerHTML = '<td colspan="6" style="padding:12px; color:#6b7280;">Nenhum movimento encontrado</td>';
            tbody.appendChild(tr);
        } else {
            movimentos.forEach(mov => {
                const tr = document.createElement('tr');
                const dataHora = formatarDataHora(mov.data_hora || mov.created_at || mov.data || '');
...
function showHistoricoInline() {
    ...
    if (periodBtn) {
        periodBtn.addEventListener('click', (e) => {
            openRangeCalendarPopup(e.currentTarget, periodStart, periodEnd, (s, f) => {
                periodStart = s; periodEnd = f;
...
function carregarHistoricoPeriodo(startDate, endDate) {
    ...
    let movimentos = clienteAtual && clienteAtual.historico ? clienteAtual.historico : [];
    ...
    if (!Array.isArray(movimentos) || movimentos.length === 0) {
        const tr = document.createElement('tr');
        tr.innerHTML = '<td colspan="7" style="padding:12px; color:#6b7280;">Nenhum registro encontrado para o período selecionado</td>';
        tbody.appendChild(tr);
        return;
    }
...
function openRangeCalendarPopup(anchorEl, startDate, endDate, onChange) {
    ...
    function buildMonthView(monthDate) {
        ...
        for (let d=1; d<=daysInMonth; d++) {
            ...
            const curDate = new Date(monthDate.getFullYear(), monthDate.getMonth(), d);
            ...
            function updateDayStyles() {
                ...
                try {
                    ...
                    } else if (selStart && curDate.toDateString() === selStart.toDateString()) {
                        day.style.background = 'var(--color-primary)';
                        day.style.color = 'white';
                    }
                } catch (e) { /* ignore */ }
...
async function toggleStatusCliente() {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/clientes/${clienteAtual.id}`, {
            method: 'PUT',
            headers: {
        ...
        if (data.success) {
            ...
            document.getElementById('clienteStatusText').textContent = novoStatus ? 'Sim' : 'Não';
    ...
    function renderPets(pets) {
        ...
        // Criar cards de pet (estilo semelhante à screenshot)
        pets.forEach(pet => {
            ...
            const idadeText = pet.data_nascimento ? calcularIdadePet(pet.data_nascimento) : '';
            ...
            if (pet.tags) {
                if (typeof pet.tags === 'string') {
                    ...
                    if (t.startsWith('[')) {
                        try { tagsArray = JSON.parse(t); } catch (e) { tagsArray = t.split(',').map(s => s.trim()).filter(Boolean); }
                    } else {
                        tagsArray = t.split(',').map(s => s.trim()).filter(Boolean);
                    }
            ...
            const tagsHtml = tagsArray.length > 0 ? `<div style="margin-top:8px">Tags: ${tagsArray.map(tag => `<span style=\"display:inline-block;background:#eef6ff;color:#0366d6;padding:4px 8px;border-radius:12px;font-size:12px;margin-right:6px;\">${tag}</span>`).join('')}</div>` : '';
            ...
            card.addEventListener('click', async function() {
                try {
                    const clienteIdForUrl = (typeof clienteAtual !== 'undefined' && clienteAtual && clienteAtual.id) ? clienteAtual.id : '';
                    ...
                    candidates.push(window.location.origin + window.location.pathname.replace(/[^/]*$/, '') + `pets/pet-details.html${query}`);
                    // 2) Caminho absoluto quando frontend é servido pelo backend ou Live Server
                    candidates.push(window.location.origin + `/pets/pet-details.html${query}`);
                    // 3) Alguns setups servem arquivos diretamente de /frontend
                    candidates.push(window.location.origin + `/frontend/pets/pet-details.html${query}`);
                    // 4) Forçar backend (localhost:3000)
                    candidates.push(`${location.protocol}//localhost:3000/pets/pet-details.html${query}`);

                    let found = null;
                    for (const url of candidates) {
                        try {
                            const resp = await fetch(url, { method: 'HEAD' });
                            if (resp && resp.ok) { found = url; break; }
                        } catch (e) {
...
function mostrarCarregamento() {
    ...
    const elementos = [
        'clienteNome', 'clienteNomeHeader', 'clienteTelefone', 
        'clienteEndereco', 'clienteCpf', 'clienteEmail'
    ];
