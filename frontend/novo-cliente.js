class NovoClienteManager {
    ...
    preencherFormulario(cliente) {
        ...
        if (cliente.telefones_adicionais && Array.isArray(cliente.telefones_adicionais)) {
        ...
        if (cliente.grupo_cliente && typeof grupoSelector !== 'undefined') {
            setTimeout(() => {
                grupoSelector.definirGrupo(cliente.grupo_cliente, cliente.grupo_cliente_id);
    ...
    setupRealTimeValidation() {
        ...
        const telefoneField = document.getElementById('telefone');
        if (telefoneField) {
            telefoneField.addEventListener('blur', () => this.validateTelefone(telefoneField));
            telefoneField.addEventListener('input', () => this.validateTelefone(telefoneField));
        }
    }
    ...
    validateCPF(field) {
        ...
        if (cpfValue.length !== 11) {
            this.showFieldError(field, errorElement, 'CPF deve ter exatamente 11 dígitos');
            return false;
        }
    ...
    validateTelefone(field) {
        ...
        if (telefoneValue.length < 10 || telefoneValue.length > 11) {
            this.showFieldError(field, errorElement, 'Telefone deve ter 10 ou 11 dígitos');
            return false;
        }
        
        // Se tem 11 dígitos, o terceiro dígito deve ser 9 (celular)
        if (telefoneValue.length === 11 && telefoneValue[2] !== '9') {
            this.showFieldError(field, errorElement, 'Para celular, o terceiro dígito deve ser 9');
            return false;
        }
    ...
    async searchCep(cep) {
        ...
        try {
            ...
            if (!data.erro) {
                ...
                const ufField = document.getElementById('uf') || document.getElementById('estado');
                ...
                this.showMessage('✅ Endereço encontrado e preenchido automaticamente!', 'success');
            } else {
                this.showMessage('❌ CEP não encontrado. Verifique o número digitado.', 'error');
            }
        } catch (error) {
            console.error('Erro ao buscar CEP:', error);
            this.showMessage('❌ Erro ao buscar CEP. Verifique sua conexão e tente novamente.', 'error');
        } finally {
            // Restaurar botão
            const btnSearchCep = document.querySelector('.btn-search-cep');
        ...
        setTimeout(() => {
            ...
            if (btnAddEmail) {
                ...
                btnAddEmail.addEventListener('click', (e) => {
                    ...
                    if (!emailsContainer) {
                        console.error('❌ Container de email não encontrado no momento do clique');
                        return;
                    }
    ...
    async submitForm() {
        ...
        try {
            ...
            const camposTelefone = document.querySelectorAll('input[name^="telefone_adicional_"]');
            ...
            if (telefonesAdicionais.length > 0) {
                formData.append('telefones_adicionais', JSON.stringify(telefonesAdicionais));
            }

            // Coletar emails adicionais
            const emailsAdicionais = [];
            const camposEmail = document.querySelectorAll('input[name^="email_adicional_"]');
...
// Adicionar state inicial para detectar navegação
history.pushState(null, null, window.location.href);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try { window.novoClienteManager = manager; } catch(e) { console.warn('Não foi possível expor manager globalmente', e); }
    ...
    if (formulario) {
        ...
        // Interceptar submissão do formulário
            ...
            formulario.addEventListener('submit', function(e){
                ...
                e.preventDefault();
                console.log('🛡️ Fallback: prevenindo submit padrão e chamando novoClienteManager.submitForm()');
                try {
                    if (window.novoClienteManager && typeof window.novoClienteManager.submitForm === 'function') {
                        window.novoClienteManager.submitForm();
                    } else if (typeof manager !== 'undefined' && manager && typeof manager.submitForm === 'function') {
                        manager.submitForm();
                    } else {
...
class GrupoClienteSelector {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (gruposLocais && gruposLocais.length > 0) {
                    ...
                    console.log('📦 Grupos carregados do localStorage:', this.grupos.length);
                } else {
    ...
    handleKeyNavigation(e) {
        ...
        const currentActive = dropdown.querySelector('.grupo-dropdown-item.selected');
    ...
    definirGrupo(nomeGrupo, grupoId = null) {
        ...
        if (nomeGrupo) {
            const grupo = this.grupos.find(g => 
                g.nome.toLowerCase() === nomeGrupo.toLowerCase() || 
                g.id == grupoId
            );
            if (grupo) {
                this.grupoSelecionado = grupo;
