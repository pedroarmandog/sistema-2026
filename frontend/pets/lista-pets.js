...
function criarCardPet(pet) {
    ...
    const idade = pet.data_nascimento ? calcularIdade(new Date(pet.data_nascimento)) : null;
    
    // Processar tags
    const tags = pet.tags ? pet.tags.split(',').map(tag => tag.trim()).filter(tag => tag) : [];
...
function confirmarExclusao(petId, nomePet) {
    const confirmacao = confirm(`Tem certeza que deseja excluir o pet "${nomePet}"?\n\nEsta ação não pode ser desfeita.`);
