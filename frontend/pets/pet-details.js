...
console.log('🐶 pet-details.js carregado');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btnInativar) btnInativar.addEventListener('click', (ev) => {
            ...
            try {
                ...
                if (m && m[1]) {
                    ...
                    (async function fetchClientNameIfMissing(id) {
                        try {
                            let resp = null;
                            try { resp = await fetch(`/api/clientes/${encodeURIComponent(id)}`); } catch (e) { resp = null; }
                            if (!resp || !resp.ok) {
                                try { resp = await fetch(`${location.protocol}//localhost:3000/api/clientes/${encodeURIComponent(id)}`); } catch (e) { resp = null; }
                            }
                            if (resp && resp.ok) {
                                const json = await resp.json();
                                const c = json && (json.cliente || json || json.data);
                                const nome = c && (c.nome || c.name || c.fullName || c.nome_completo) || null;
                                if (nome) {
    ...
    function activateTab(name) {
        ...
        try {
            ...
            if (tabsActions) {
                ...
                tabsActions.style.display = (name === 'vacinas' || name === 'vermifugos' || name === 'antiparasitarios') ? 'flex' : 'none';
                // exibir apenas o bloco correspondente (se existir)
                Array.from(tabsActions.children).forEach(child => {
                    try {
                        const cls = child.className || '';
                        if (name === 'vermifugos' && cls.indexOf('vermifugos-actions') !== -1) child.style.display = 'flex';
                        else if (name === 'antiparasitarios' && cls.indexOf('antiparasitarios-actions') !== -1) child.style.display = 'flex';
                        else if (name === 'vacinas' && cls.indexOf('vacinas-actions') !== -1) child.style.display = 'flex';
                        else child.style.display = 'none';
                    } catch (e) { child.style.display = 'none'; }
                });
            }
        } catch (e) { /* ignore */ }
    }
    ...
    function initHistoricoControls() {
        ...
        // definir default (Clínico)
        pills.forEach(p => p.classList.remove('active'));
        const defaultPill = document.querySelector('.pill-button[data-historico-type="clinico"]');
        if (defaultPill) defaultPill.classList.add('active');

        pills.forEach(p => {
            p.addEventListener('click', () => {
                // alternar active (somente um ativo por vez)
                pills.forEach(x => x.classList.remove('active'));
                p.classList.add('active');
                const tipo = p.getAttribute('data-historico-type');
                // TODO: filtrar os itens do histórico por tipo (clinico / estetico)
                // por enquanto vamos apenas aplicar a classe e deixar espaço para integração futura
                console.log('Filtro histórico selecionado:', tipo);
            });
        });
    }
    ...
    function initHistoricoFilterModal() {
        ...
        if (btnFilter) btnFilter.addEventListener('click', (e) => { e.preventDefault(); showHistoricoFilterModal(); });
        if (cancel) cancel.addEventListener('click', (e) => { e.preventDefault(); closeHistoricoFilterModal(); });
        if (selectAll) selectAll.addEventListener('click', (e) => { e.preventDefault(); document.querySelectorAll('.hf-item').forEach(cb => cb.checked = true); });
        if (clear) clear.addEventListener('click', (e) => { e.preventDefault(); document.querySelectorAll('.hf-item').forEach(cb => cb.checked = false); });
        if (ok) ok.addEventListener('click', (e) => { e.preventDefault(); applyHistoricoModalSelection(); });
    ...
    const historicoSelectedFilters = new Set(['banhos','tosas','proc-esteticos','consultas','procedimentos','exames','receitas','vacinas','vermifugos','antiparasitas']);
    ...
    function showHistoricoFilterModal() {
        ...
        document.querySelectorAll('.hf-item').forEach(cb => {
            try { cb.checked = historicoSelectedFilters.has(cb.value); } catch (e) {}
        });
        modal.style.display = 'flex';
    }
    ...
    function applyHistoricoModalSelection() {
        const checked = Array.from(document.querySelectorAll('.hf-item')).filter(cb => cb.checked).map(cb => cb.value);
...
function initTabsActions() {
    ...
    if (btnMaisVacinas) btnMaisVacinas.addEventListener('click', () => {
        const urlParams = new URLSearchParams(window.location.search);
        const petId = urlParams.get('pet_id') || urlParams.get('id');
        const clienteId = urlParams.get('cliente_id');
        const q = clienteId ? `?pet_id=${encodeURIComponent(petId)}&cliente_id=${encodeURIComponent(clienteId)}` : `?pet_id=${encodeURIComponent(petId)}`;
    ...
    if (btnPeriodicos) btnPeriodicos.addEventListener('click', (e) => { e.preventDefault(); mostrarNotificacao('Ação "Periódicos" (Vacinas) ainda não implementada.', 'info'); });

    // vermifugos
    const btnMaisVermifugos = document.getElementById('btnAdd_vermifugos');
    if (btnMaisVermifugos) btnMaisVermifugos.addEventListener('click', () => {
        const urlParams = new URLSearchParams(window.location.search);
        const petId = urlParams.get('pet_id') || urlParams.get('id');
        const clienteId = urlParams.get('cliente_id');
        const q = clienteId ? `?pet_id=${encodeURIComponent(petId)}&cliente_id=${encodeURIComponent(clienteId)}` : `?pet_id=${encodeURIComponent(petId)}`;
    ...
    if (btnPeriodicosV) btnPeriodicosV.addEventListener('click', (e) => { e.preventDefault(); mostrarNotificacao('Ação "Periódicos" (Vermífugos) ainda não implementada.', 'info'); });

    // antiparasitarios
    const btnMaisAntip = document.getElementById('btnAdd_antiparasitarios');
    if (btnMaisAntip) btnMaisAntip.addEventListener('click', () => {
        const urlParams = new URLSearchParams(window.location.search);
        const petId = urlParams.get('pet_id') || urlParams.get('id');
        const clienteId = urlParams.get('cliente_id');
        const q = clienteId ? `?pet_id=${encodeURIComponent(petId)}&cliente_id=${encodeURIComponent(clienteId)}` : `?pet_id=${encodeURIComponent(petId)}`;
    ...
    const btnPeriodicosA = document.getElementById('btnPeriodicos_antiparasitarios');
    if (btnPeriodicosA) btnPeriodicosA.addEventListener('click', (e) => { e.preventDefault(); mostrarNotificacao('Ação "Periódicos" (Antiparasitários) ainda não implementada.', 'info'); });
}
...
async function loadHistoricoIntoTab(petId) {
    ...
    // encontrar card container dentro da aba historico
    let card = historicoTab.querySelector('.card');
    if (!card) {
        card = document.createElement('div');
        card.className = 'card';
        historicoTab.appendChild(card);
    }

    // mostrar loading
    card.innerHTML = `<h3>Histórico de Atendimentos</h3><div class="table-empty">Carregando...</div>`;

    // tentar buscar do backend (relativo -> localhost fallback)
    let resp = null;
    try { resp = await fetch(`/api/pets/${encodeURIComponent(petId)}/historico`); } catch (e) { resp = null; }
    if (!resp || !resp.ok) {
        try { resp = await fetch(`${location.protocol}//localhost:3000/api/pets/${encodeURIComponent(petId)}/historico`); } catch (e) { resp = null; }
    }
    ...
    if (resp && resp.ok) {
        try {
            ...
            items = json && (json.historico || json.data || json.items || json) || [];
    ...
    // Se não obteve itens do fetch, tentar usar elementos já presentes no DOM (fallback simples)
    if ((!items || items.length === 0)) {
        const existing = Array.from(historicoTab.querySelectorAll('.historico-item'));
        if (existing && existing.length > 0) {
            // reconstruir a lista a partir dos elementos existentes
            items = existing.map(el => {
                return {
                    html: el.innerHTML,
    ...
    if (!items || items.length === 0) {
        card.innerHTML = `
            ...
            <div class="card-empty">Não há histórico encontrado!</div>
        `;
        return;
    }

    // montar conteúdo com os itens
    const itensHtml = items.map(it => {
        if (it && it.html) {
            // fallback quando pegamos do DOM
            return `<div class="historico-item" data-tipo="${(it.tipo||'').replace(/\"/g,'')}">${it.html}</div>`;
        }
        // tentar mapear campos conhecidos
        const atendimento = it.atendimento || it.id || it.numero || '';
        const data = it.data || it.data_atendimento || it.created_at || '';
        const servico = it.servico || it.servico_nome || it.servico_descricao || it.descricao || '';
        const profissional = it.profissional || it.responsavel || it.colaborador || '';
        const tipo = it.tipo || it.categoria || '';
        const linhaData = data ? ` - ${formatarData(data)}` : '';
        const atendimentoLabel = atendimento ? `Atendimento: ${atendimento}` : '';
        return `
            <div class="historico-item" data-tipo="${tipo}">
                <p><strong>Atendimento:</strong> ${atendimentoLabel}${linhaData}</p>
                <p><strong>Serviço:</strong> ${servico || '-'}</p>
                <p><strong>Profissional:</strong> ${profissional || '-'}</p>
            </div>`;
    }).join('\n');

    card.innerHTML = `<h3>Histórico de Atendimentos</h3>${itensHtml}`;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btn) {
        btn.addEventListener('click', function(e) {
            e.preventDefault();
            if (!petId) return;
            loadHistoricoIntoTab(petId).catch(err => { console.error('Erro ao atualizar histórico:', err); mostrarNotificacao('Falha ao atualizar histórico', 'error'); });
        });
    }
...
async function carregarPet(petId, clienteId) {
    try {
        ...
        if (!resp || !resp.ok) {
            try {
                resp = await fetch(`${location.protocol}//localhost:3000/api/pets/${encodeURIComponent(petId)}`);
            } catch (err) {
                resp = null;
            }
        }
...
function renderPet(pet, clienteId) {
    ...
    if (clientEl) {
        console.debug('renderPet: clientEl initial text=', clientEl.textContent && clientEl.textContent.trim());
        // Tentar extrair o nome do cliente a partir de vários campos comuns
        const nomeCliente = (pet && (
            (pet.Cliente && pet.Cliente.nome) || pet.cliente_nome || pet.cliente || pet.owner_name || pet.nome_cliente || pet.clienteName
        )) || null;
        const idCliente = (pet && ((pet.Cliente && pet.Cliente.id) || pet.cliente_id)) || clienteId || '';
        ...
        function applyClientText(name, id) {
            ...
            try {
                ...
                try { clientEl.textContent = name || '—'; } catch (ee) {}
            }
        }
    ...
    if (ativoToggle) { try { ativoToggle.checked = !!pet.ativo; } catch (e) {} }
    setText('petAtivo', pet.ativo ? 'Sim' : 'Não');
    setText('petObservacao', pet.observacao || pet.observacoes || '—');

    // Alergias - suportar array ou string
    const rawAlerg = pet.alergias || pet.alergia || pet.reacoes_alergicas || pet.alergias_list;
    let alergiasText = '—';
    if (rawAlerg) {
        if (Array.isArray(rawAlerg)) alergiasText = rawAlerg.join(', ');
            ...
            if (t.startsWith('[')) {
                try { const parsed = JSON.parse(t); if (Array.isArray(parsed)) alergiasText = parsed.join(', '); else alergiasText = parsed; } catch (e) { alergiasText = t; }
            } else {
                alergiasText = t;
            }
        } else {
...
function calcularIdadePet(dataNascimento) {
    ...
    let nascimento;
    try {
        if (typeof dataNascimento === 'string' && dataNascimento.indexOf('/') !== -1) {
    ...
    } catch (e) { nascimento = new Date(dataNascimento); }
    ...
    if (hoje.getDate() < nascimento.getDate()) { meses--; if (meses < 0) { anos--; meses += 12; } }
...
function clearGlobalModalOverlays() {
    try {
        const selectors = ['.modal-overlay', '.modal-backdrop', '.overlay', '.swal-overlay', '#exitConfirmModal'];
...
async function inativarPet(petId) {
    ...
    if (!resp.ok) {
        ...
        throw new Error((text && text.length ? text : ('Erro do servidor: ' + (resp.status || ''))));
    }
    return true;
}
...
async function inativarOuExcluirPet(petId) {
    ...
    try {
        let resp = null;
        try {
            resp = await fetch(`/api/pets/${encodeURIComponent(petId)}`, { method: 'DELETE' });
        } catch (e) { resp = null; }

        if (!resp || !resp.ok) {
            try { resp = await fetch(`${location.protocol}//localhost:3000/api/pets/${encodeURIComponent(petId)}`, { method: 'DELETE' }); } catch (e) { resp = null; }
        }
        ...
        if (resp && (resp.status === 405 || resp.status === 404 || resp.status === 400)) {
        ...
        try {
            ...
            const msg = (resp ? (`Erro ao excluir (status ${resp.status})`) : 'Sem resposta ao tentar excluir');
...
async function confirmarInativacao() {
    ...
    try {
        ...
        const petId = petIdEl ? (petIdEl.textContent || petIdEl.innerText || '').trim() : null;
        if (!petId) {
        ...
        try { mostrarNotificacao('Pet inativado com sucesso', 'success'); } catch(e){ showToast('Pet inativado com sucesso', 'success'); }
        closeConfirmDeleteModal();
        // redirecionar para a página do cliente (se disponível) para não mostrar mais a ficha do pet
        const params = new URLSearchParams(window.location.search);
        const clienteIdLocal = params.get('cliente_id') || params.get('clienteId') || params.get('cliente') || params.get('cid');
        if (clienteIdLocal) {
            setTimeout(() => { try { window.location.href = `../client-details.html?id=${encodeURIComponent(clienteIdLocal)}`; } catch(e) { window.history.back(); } }, 700);
        } else {
            // fallback: voltar no histórico
            setTimeout(() => { try { window.history.back(); } catch(e) { location.href = '../clientes.html'; } }, 700);
        }
    } catch (e) {
        console.error('Erro ao inativar pet:', e);
        showToast('Erro ao inativar pet: ' + (e && e.message ? e.message : ''), 'error', 5000);
        if (btn) btn.disabled = false;
    }
}

// Expor as funções globalmente (por segurança, caso HTML use onclick inline)
try { window.closeConfirmDeleteModal = closeConfirmDeleteModal; window.confirmarInativacao = confirmarInativacao; } catch (e) {}
...
function mostrarNotificacao(mensagem, tipo = 'info') {
    ...
    switch (tipo) {
        ...
        case 'error': icone = 'fas fa-exclamation-circle'; break;
        default: icone = 'fas fa-info-circle';
    }
...
async function loadResourceIntoTab(resourceId, petId, clienteId, columns) {
    ...
    const headersHtml = columns.map(c => `<th>${c.label}</th>`).join('') + '<th>Ações</th>';

    // Personalizar cabeçalho para vacinas (dois controles) ou usar botão padrão
    const headerTitle = (columns[0].label === 'Data aplicação' && resourceId === 'vacinas') ? 'Vacinas aplicadas' : columns[0].label;
    ...
    try {
        let resp = null;
        try { resp = await fetch(`/api/pets/${encodeURIComponent(petId)}/${resourceId}`); } catch (e) { resp = null; }
        if (!resp || !resp.ok) {
            try { resp = await fetch(`${location.protocol}//localhost:3000/api/pets/${encodeURIComponent(petId)}/${resourceId}`); } catch (e) { resp = null; }
        }
        ...
        if (resp && resp.ok) {
            ...
            try {
                let pr = null;
                try { pr = await fetch(`/api/pets/${encodeURIComponent(petId)}`); } catch (e) { pr = null; }
                if (!pr || !pr.ok) {
                    try { pr = await fetch(`${location.protocol}//localhost:3000/api/pets/${encodeURIComponent(petId)}`); } catch (e) { pr = null; }
                }
                if (pr && pr.ok) {
        ...
        items.forEach(it => {
            ...
            const rowHtml = columns.map(c => `<td>${(it[c.key] != null && it[c.key] !== '') ? it[c.key] : '-'}</td>`).join('');
            tr.innerHTML = rowHtml + `<td><button class="btn">Editar</button></td>`;
            tbody.appendChild(tr);
        });

        const btnAdd = document.getElementById(`btnAdd_${resourceId}`);
        if (btnAdd) btnAdd.addEventListener('click', () => {
            const q = clienteId ? `?pet_id=${encodeURIComponent(petId)}&cliente_id=${encodeURIComponent(clienteId)}` : `?pet_id=${encodeURIComponent(petId)}`;
            window.location.href = `${resourceId}-cadastro.html${q}`; // página de cadastro não criada automaticamente
        });
...
function initPesoControls() {
    ...
    if (btnShowList) btnShowList.addEventListener('click', (e) => { e.preventDefault(); const urlParams = new URLSearchParams(window.location.search); const petId = urlParams.get('pet_id') || urlParams.get('id'); showListView(petId); });
    if (btnShowChart) btnShowChart.addEventListener('click', (e) => { e.preventDefault(); const urlParams = new URLSearchParams(window.location.search); const petId = urlParams.get('pet_id') || urlParams.get('id'); showChartView(petId); });

    if (btnCancel) btnCancel.addEventListener('click', (e) => { e.preventDefault(); closePesoModal(); });
    if (btnSave) btnSave.addEventListener('click', (e) => { e.preventDefault(); savePesoFromModal(); });
...
function openPesoModal() {
    ...
    try { delete modal.dataset.editIndex; } catch(e) {}
    if (dateInput) dateInput.value = new Date().toISOString().slice(0,10);
    if (kgInput) { kgInput.value = ''; try { kgInput.focus(); } catch(e) {} }
    modal.style.display = 'flex';
}
...
function savePesoFromModal() {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const petId = urlParams.get('pet_id') || urlParams.get('id');
    if (!petId) { mostrarNotificacao('ID do pet ausente', 'error'); return; }
    if (!dateInput || !kgInput) return;
    const date = dateInput.value;
    const kg = parseFloat(kgInput.value);
    if (!date) { mostrarNotificacao('Informe a data', 'error'); return; }
    if (!kg || isNaN(kg) || kg <= 0) { mostrarNotificacao('Informe um peso válido', 'error'); return; }

    const key = `pet_${petId}_pesos`;
    let arr = [];
    try { arr = JSON.parse(localStorage.getItem(key) || '[]'); } catch (e) { arr = []; }
    const modal = document.getElementById('pesoModal');
    const editIndex = modal && modal.dataset && modal.dataset.editIndex != null ? Number(modal.dataset.editIndex) : null;
    ...
    try { localStorage.setItem(key, JSON.stringify(arr)); } catch (e) { console.error('Falha ao salvar peso', e); }
...
function renderPesoChart(petId) {
    ...
    const labels = arr.map(r => {
        try { return parseDateLocal(r.date).toLocaleDateString('pt-BR'); } catch (e) { return r.date; }
    });
    ...
    if (window.Chart && canvas) {
                    ...
                    x: {
                        ...
                        ticks: {
                            ...
                            callback: function(value, index) {
                                try { return this.chart.data.labels[index]; } catch (e) { return value; }
                            }
                        }
                    },
                    y: {
            ...
            plugins: [
                {
                    ...
                    afterDatasetsDraw: function(chart, args, options) {
                        ...
                        const avgVal = dataset.data.reduce((s,v)=>s+v,0)/dataset.data.length;
        ...
        } catch (e) { console.warn('Erro ao destruir pesoChart existente', e); }

        canvas.style.height = '240px';
        pesoChart = new Chart(ctx, cfg);
        try { window.pesoChart = pesoChart; } catch (e) {}
        return;
    }
...
function renderPesoList(petId) {
    ...
    if (!arr || arr.length === 0) {
        listWrap.innerHTML = '<div class="peso-list-empty" style="color:#9aa0a6; padding:8px">Nenhum registro</div>';
        return;
    }
    ...
    const sorted = mapped.slice().sort((a,b)=> parseDateLocal(b.date) - parseDateLocal(a.date));
    const itemsHtml = sorted.map(it => {
        const d = (function(dt){ try { return parseDateLocal(dt).toLocaleDateString('pt-BR'); } catch(e){ return dt; } })(it.date);
    ...
    listWrap.querySelectorAll('.peso-delete').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            try { deletePeso(petId, idx); } catch (err) { console.error(err); }
        });
    });
    listWrap.querySelectorAll('.peso-edit').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.preventDefault();
            const idx = Number(btn.getAttribute('data-idx'));
            try { editPeso(petId, idx); } catch (err) { console.error(err); }
        });
    });
}
...
function compressAndResizeImage(file, maxWidth = 1000, maxHeight = 1000, quality = 0.65) {
        ...
        reader.onload = function(evt) {
            ...
            img.onload = function() {
                try {
                    ...
                    const ratio = Math.min(1, Math.min(maxWidth / origW, maxHeight / origH));
            ...
            img.onerror = function(err) { resolve(evt.target.result); };
...
function initPetPhotoControls() {
    ...
    if (wrapper) {
        ...
        wrapper.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); if (input) input.click(); } });
    }

    if (editBtn) {
        editBtn.addEventListener('click', (e) => { e.stopPropagation(); if (input) input.click(); });
    }
    ...
    if (input) {
        input.addEventListener('change', async (e) => {
            ...
            if (!file) return;
            if (!file.type || !file.type.startsWith('image/')) { mostrarNotificacao('Selecione uma imagem válida', 'error'); return; }
            const urlParams = new URLSearchParams(window.location.search);
            const petId = urlParams.get('pet_id') || urlParams.get('id');
            if (!petId) { mostrarNotificacao('ID do pet ausente', 'error'); return; }
            try {
                // reduzir/resample e comprimir antes de salvar
                const compressedDataUrl = await compressAndResizeImage(file, 1000, 1000, 0.65);
                try { localStorage.setItem(`pet_${petId}_photo`, compressedDataUrl); } catch (err) { console.error('Falha ao salvar foto', err); }
                // atualizar UI
                if (img) { img.src = compressedDataUrl; img.style.display = 'block'; }
                if (fallback) fallback.style.display = 'none';
                if (removeBtn) { removeBtn.style.display = 'inline-block'; }
                // marcar wrapper como tendo foto
                try { if (wrapper && wrapper.classList) { wrapper.classList.add('has-photo'); wrapper.classList.remove('no-photo'); } } catch(e){}
                mostrarNotificacao('Foto atualizada', 'success');
            } catch (err) {
                console.error('Erro ao processar imagem:', err);
                mostrarNotificacao('Erro ao processar a imagem. Tente outro arquivo.', 'error');
            }
        });
    }
    ...
    (function applySavedPhoto() {
        try {
            ...
            const dataUrl = localStorage.getItem(key);
            if (dataUrl && img) {
                img.src = dataUrl; img.style.display = 'block'; if (fallback) fallback.style.display = 'none';
                if (removeBtn) removeBtn.style.display = 'inline-block';
                try { if (wrapper && wrapper.classList) { wrapper.classList.add('has-photo'); wrapper.classList.remove('no-photo'); } } catch(e){}
            } else {
                if (img) img.style.display = 'none'; if (fallback) fallback.style.display = 'flex';
                if (removeBtn) removeBtn.style.display = 'none';
                try { if (wrapper && wrapper.classList) { wrapper.classList.add('no-photo'); wrapper.classList.remove('has-photo'); } } catch(e){}
            }
        } catch (e) { /* ignore */ }
    })();
    ...
    if (removeBtn) {
        removeBtn.addEventListener('click', (e) => {
            ...
            if (!petId) { mostrarNotificacao('ID do pet ausente', 'error'); return; }
            const key = `pet_${petId}_photo`;
            try { localStorage.removeItem(key); } catch (err) { console.error('Falha ao remover foto', err); }
            // atualizar UI
...
document.addEventListener('DOMContentLoaded', function() { initPetPhotoControls(); });
