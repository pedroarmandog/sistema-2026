...
if (typeof window.mostrarNotificacao !== 'function') {
    window.mostrarNotificacao = function(mensagem, tipo = 'info') {
        try {
            ...
            const anterior = document.querySelector('.notification.notification-success') || document.querySelector('.notification');
            ...
            iconeEl.className = (tipo === 'success') ? 'fas fa-check' : (tipo === 'error' ? 'fas fa-exclamation' : 'fas fa-info');
            ...
            // auto remover
            setTimeout(() => { try { if (notificacao.parentElement) notificacao.remove(); } catch (e) {} }, 4500);
        } catch (e) {
            try { alert(mensagem); } catch (ignore) {}
        }
    };
}
...
function showConfirmDeleteModalMeusPets(message, onConfirm){
    try {
        ...
        newOk.addEventListener('click', function(){
            modal.style.display = 'none'; try{ document.body.style.overflow=''; }catch(e){}
            if (typeof onConfirm === 'function') onConfirm();
        });
        newCancel.addEventListener('click', function(){ modal.style.display = 'none'; try{ document.body.style.overflow=''; }catch(e){} });

        // fechar com ESC
        const escHandler = function(ev){ if (ev.key === 'Escape') { modal.style.display = 'none'; try{ document.body.style.overflow=''; }catch(e){}; document.removeEventListener('keydown', escHandler); } };
...
function nomeDoPet(p){ return p.nome || p.name || p.nome_pet || p.pet_nome || ''; }
function nomeDoCliente(c){ return c.nome || c.name || c.nome_cliente || ''; }
...
function grupoKeyFromName(name){ if(!name) return ''; return name.toString().toLowerCase().trim().replace(/\s+/g,'-').replace(/[()]/g,''); }
...
function renderPetsTable(pets){
    const tbody = document.getElementById('petsTableBody');
        ...
        if (!pets || pets.length === 0) { 
            tbody.innerHTML = '<tr><td colspan="5">Nenhum pet encontrado</td></tr>'; 
            return; 
        }
    ...
    const rows = pets.map(p => {
        ...
        const clienteId = p.cliente_id || p.cliente || p.clienteId || p.tutor_id || null;
        const cliente = clienteId ? _meusClientsMap[clienteId] : null;
        const clienteDisplay = cliente ? (cliente.id ? `${cliente.id} - ${nomeDoCliente(cliente)}` : nomeDoCliente(cliente)) : '—';
        const ativo = (p.ativo === true || p.ativo === '1' || p.ativo === 1 || p.ativo === 'true');

        return `
        <tr data-pet-id="${id}">
          <td>${id}</td>
          <td>${escapeHtml(petName)}</td>
          <td>${escapeHtml(clienteDisplay)}</td>
    ...
    if (!document.getElementById('__rowActionsDropdown')) {
        ...
        d.innerHTML = `
            <div class="action-item delete" data-action="delete"><i class="fas fa-trash-alt"></i><span>Excluir</span></div>
        ...
        rowDd.querySelectorAll('.action-item').forEach(item => {
            item.onclick = function(e){
                ...
                if (action === 'edit') {
                    ...
                    let clienteId = null;
                    if (tr) {
                        const clienteText = tr.children[2] ? tr.children[2].textContent || '' : '';
                        const m = clienteText.match(/^(\d+)\s*-\s*/);
                        if (m) clienteId = m[1];
                    }
                    const q = clienteId ? `?pet_id=${encodeURIComponent(petId)}&cliente_id=${encodeURIComponent(clienteId)}` : `?pet_id=${encodeURIComponent(petId)}`;
                    window.location.href = `/frontend/pets/cadastro-pet.html${q}`;
                } else if (action === 'delete') {
                    // confirmar exclusão com modal estilizado
                    showConfirmDeleteModalMeusPets('Tem certeza que deseja inativar?', function(){
                        // executar exclusão/inativação
                        (async function doDelete(id){
                            try {
                                const tryUrls = [ `/api/pets/${encodeURIComponent(id)}`, `http://localhost:3000/api/pets/${encodeURIComponent(id)}`, `http://127.0.0.1:3000/api/pets/${encodeURIComponent(id)}` ];
                                let resp = null;
                                for (const u of tryUrls) {
                                    try { resp = await fetch(u, { method: 'DELETE', credentials: 'include' }); } catch(e){ resp = null; }
                                    if (resp && resp.ok) break;
                                }
                                if (resp && resp.ok) {
                                    // remover do array local e re-render
                                    _meusPetsData = _meusPetsData.filter(p => String(p.id || p.codigo || p.pet_id || p._id || '') !== String(id));
                                    try { window._meusPetsData = _meusPetsData; } catch(e) {}
                                    renderPetsTable(_meusPetsData);
                                    // Mostrar notificação no visual desejado
                                    try {
                                        if (typeof window.mostrarNotificacao === 'function') {
                                            // usar mesmo texto e estilo do novo-cliente
                                            window.mostrarNotificacao('Pet excluido com sucesso!', 'success');
                                        } else {
                                            // fallback: criar notificação com estilo inline igual ao novo-cliente
                                            const prev = document.querySelector('.notification.notification-success'); if (prev) prev.remove();
                                            const notif = document.createElement('div');
                                            notif.className = 'notification notification-success';
                                            notif.style.position = 'fixed'; notif.style.top = '20px'; notif.style.right = '20px'; notif.style.zIndex = '13000';
                                            notif.style.minWidth = '300px'; notif.style.maxWidth = '520px'; notif.style.padding = '14px 18px 14px 72px';
                                            notif.style.borderRadius = '10px'; notif.style.background = '#e8f7ec'; notif.style.color = '#124925';
                                            notif.style.boxShadow = '0 10px 30px rgba(0,0,0,0.12)'; notif.style.fontWeight = '700'; notif.style.display = 'flex'; notif.style.alignItems = 'center'; notif.style.gap = '12px';
                                            // barra lateral removida (anti-visual indesejado)
                                            // Observação: a barra lateral foi removida para coincidir com o visual esperado.
                                            // ícone circular
                                            const icon = document.createElement('i'); icon.className = 'fas fa-check'; icon.style.position = 'absolute'; icon.style.left = '12px'; icon.style.top = '50%'; icon.style.transform = 'translateY(-50%)'; icon.style.width = '40px'; icon.style.height = '40px'; icon.style.display = 'inline-flex'; icon.style.alignItems = 'center'; icon.style.justifyContent = 'center'; icon.style.borderRadius = '50%'; icon.style.background = '#27ae60'; icon.style.color = '#ffffff'; icon.style.fontSize = '14px'; icon.style.boxShadow = '0 4px 12px rgba(39,174,96,0.22)'; notif.appendChild(icon);
                                            // mensagem
                                            const span = document.createElement('span'); span.className = 'message-text'; span.textContent = 'Pet excluido com sucesso!'; span.style.display = 'block'; span.style.lineHeight = '1.2'; notif.appendChild(span);
                                            // botão fechar
                                            const btn = document.createElement('button'); btn.className = 'notification-close'; btn.setAttribute('aria-label','Fechar'); btn.style.marginLeft = 'auto'; btn.style.background = 'transparent'; btn.style.border = 'none'; btn.style.cursor = 'pointer'; btn.style.color = 'rgba(18,73,37,0.9)'; btn.innerHTML = '<i class="fas fa-times"></i>'; btn.addEventListener('click', ()=>notif.remove()); notif.appendChild(btn);
                                            document.body.appendChild(notif);
                                            setTimeout(()=>{ try{ if (notif.parentElement) notif.remove(); }catch(e){} }, 4500);
                                        }
                                    } catch(e) { try{ alert('Pet excluido com sucesso'); }catch(_){} }
                                    return;
                                }
                            } catch(e){ console.warn('erro ao chamar DELETE', e); }
                            // fallback local
                            _meusPetsData = _meusPetsData.filter(p => String(p.id || p.codigo || p.pet_id || p._id || '') !== String(id));
                            try { window._meusPetsData = _meusPetsData; } catch(e) {}
                            renderPetsTable(_meusPetsData);
                            notify('Pet removido localmente', 'info');
                        })(petId);
                    });
                }
            };
...
async function fetchPetsAndClients(){
    ...
    try { window._meusPetsData = _meusPetsData; } catch(e) { /* ignore */ }
    console.debug('[meus-pets] pets loaded, count=', (_meusPetsData||[]).length);
    try { renderPetsTable(_meusPetsData); }
    catch(err){ console.error('[meus-pets] renderPetsTable failed', err); if (tbody) tbody.innerHTML = '<tr><td colspan="5">Erro ao renderizar pets</td></tr>'; }
    // sinalizar que os dados de pets foram carregados (para popular sugestões caso estejam abertas)
    try { document.dispatchEvent(new CustomEvent('meusPetsLoaded')); } catch(e) { console.warn('evento meusPetsLoaded falhou', e); }
    }catch(err){
        console.error(err);
        if (tbody) tbody.innerHTML = '<tr><td colspan="5">Erro ao carregar. Verifique se o backend está rodando e se o endpoint /api/pets e /api/clientes estão acessíveis.</td></tr>';
    }
}

function applyPetSearchFilter(){
    const q = (document.getElementById('searchPetInput')?.value || '').toLowerCase().trim();
    if (!q) { renderPetsTable(_meusPetsData); return; }
    const filtered = _meusPetsData.filter(p => {
        ...
        const clienteId = p.cliente_id || p.cliente || p.clienteId || p.tutor_id || null;
        const cliente = clienteId ? _meusClientsMap[clienteId] : null;
        const clienteName = cliente ? (nomeDoCliente(cliente)||'').toLowerCase() : '';

        return petName.includes(q) || id.includes(q) || micro.includes(q) || clienteName.includes(q);
    });
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if (pathname.endsWith('/meus-pets.html') || pathname.endsWith('/pets/meus-pets.html')){
        // botões
        const btnAdd = document.getElementById('btnAddPet');
    const btnExport = document.getElementById('btnExport');
    const input = document.getElementById('searchPetInput');
    if (btnAdd) btnAdd.addEventListener('click', ()=>{ window.location.href = '/pets/cadastro-pet.html'; });
    if (btnExport) btnExport.addEventListener('click', async ()=>{
        ...
        try {
            ...
            console.debug('[meus-pets] export requested. filtros:', {
                selectedGrupos: _selectedGrupos,
                filter_grupo_input: document.getElementById('filter_grupo')?.value,
                selectedEspecies: _selectedEspecies,
                filter_especie_input: document.getElementById('filter_especie')?.value,
                selectedAlimentacoes: _selectedAlimentacaoArr,
                filter_alimentacao_input: document.getElementById('filter_alimentacao')?.value,
            ...
            if (!pets || pets.length === 0) {
                showNotification('Nenhum pet corresponde aos filtros selecionados.', 'info');
                return;
            }

            // garantir XLSX carregado (instala via CDN se necessário)
            try { await ensureXLSX(); } catch (e) { console.warn('Falha ao carregar XLSX dinamicamente', e); showNotification('Não foi possível carregar a biblioteca de exportação (XLSX). Tente novamente ou verifique a conexão.', 'error'); return; }

            exportPetsToXLS(pets);
        } catch (err) {
            console.error('Erro no handler de export:', err);
            showNotification('Erro ao processar exportação. Veja console para detalhes.', 'error');
        }
    });
        if (input) { input.addEventListener('input', debounce(applyPetSearchFilter, 200)); }
                ...
                if (isClosed) {
                    ...
                    const onTransitionEnd = function(ev){
                        if (ev.propertyName === 'max-height') {
                            ...
                            try { if (window.__meusPetsSuggestContainer) window.__meusPetsSuggestContainer.style.display = 'none'; } catch(e){}
                            try { if (window.__meusPetsTagSuggestContainer) window.__meusPetsTagSuggestContainer.style.display = 'none'; } catch(e){}
    ...
    if (btnCancelar && panel){ btnCancelar.addEventListener('click', function(e){ e.preventDefault(); panel.style.display = 'none'; btnToggle?.classList.remove('active'); try { if (window.__meusPetsSuggestContainer) window.__meusPetsSuggestContainer.style.display = 'none'; } catch(e){}; try { if (window.__meusPetsTagSuggestContainer) window.__meusPetsTagSuggestContainer.style.display = 'none'; } catch(e){} }); }
    ...
    // suportar múltiplas seleções
        ...
        async function carregarGruposForMeusPets(){
            ...
            try {
                ...
                if (resp.ok) {
                    ...
                    console.log('✅ Grupos carregados para meus-pets da API:', grupos.length);
                } else {
                    console.warn('⚠️ API de grupos respondeu', resp.status, '— usando fallback local');
                }
            } catch (e) {
            ...
            if(!grupos || grupos.length === 0){
                grupos = [ { nome: 'Banho (QUENTE)', cor:'#FF6B6B' }, { nome: 'Banho (FRIO)', cor:'#4ECDC4' }, { nome: 'Assinantes', cor:'#FFD700' } ];
            }
        ...
        document.addEventListener('meusPetsLoaded', function(){
            try { carregarGruposForMeusPets(); } catch(e){}
            try { carregarEspeciesForMeusPets(); } catch(e){}
            try { carregarAlimentacoesForMeusPets(); } catch(e){}
            try { carregarTagsForMeusPets(); } catch(e){}
        });
        ...
        if(inputGrupo && suggestContainer){
            ...
            } catch(e){ console.warn('não foi possível mover suggestContainer para body', e); }
            ...
            document.addEventListener('click', function(e){ if(!suggestContainer.contains(e.target) && e.target !== inputGrupo) suggestContainer.style.display = 'none'; });
            window.addEventListener('scroll', function(){ if(suggestContainer.style.display === 'block') positionSuggest(); });
            window.addEventListener('resize', function(){ if(suggestContainer.style.display === 'block') positionSuggest(); });
        }
    ...
    // (variáveis de seleção agora declaradas em escopo superior)
        ...
        function carregarTagsForMeusPets(){
            ...
            _meusPetsData.forEach(p => {
                try {
                    if (Array.isArray(p.tags)) p.tags.forEach(t=>{ if(t && String(t).trim()!=='') set.add(String(t).trim()); });
                    else if (typeof p.tags === 'string') {
                        const raw = p.tags.trim();
                        if (raw.startsWith('[')) {
                            try { const parsed = JSON.parse(raw); if (Array.isArray(parsed)) parsed.forEach(t=>{ if(t && String(t).trim()!=='') set.add(String(t).trim()); }); }
                            catch(e){ raw.replace(/\[|\]/g,'').split(',').map(s=>s.trim()).filter(Boolean).forEach(t=>set.add(t)); }
                        } else {
            ...
            tags.forEach(name => {
                ...
                div.textContent = val;
                div.addEventListener('click', function(e){ e.stopPropagation(); addSelectedTag(val); });
                container.appendChild(div);
            });
            return tags;
        }
        ...
        function addSelectedTag(val){
            ...
            if(!_selectedTags.includes(val)){
                ...
                renderTags('filter_tag_selected', _selectedTags.map(v=>({ value:v, label:v, type:'tag' })));
                const it = document.getElementById('filter_tag'); if(it) it.value = '';
            }
        }
        ...
        function addSelectedGrupo(key, name){
            ...
            if(!_selectedGrupos.some(g=>g.key===key)){
                ...
                const ig = document.getElementById('filter_grupo'); if(ig) ig.value = '';
            }
        }
        ...
        function addSelectedEspecie(val){
            ...
            if(!_selectedEspecies.includes(val)){
                ...
                renderTags('filter_especie_selected', _selectedEspecies.map(v=>({ value:v, label:v, type:'especie' })));
                const ie = document.getElementById('filter_especie'); if(ie) ie.value = '';
            }
        }
        ...
        function addSelectedAlimentacao(val){
            ...
            if(!_selectedAlimentacaoArr.includes(val)){
                ...
                renderTags('filter_alimentacao_selected', _selectedAlimentacaoArr.map(v=>({ value:v, label:v, type:'alim' })));
                const ia = document.getElementById('filter_alimentacao'); if(ia) ia.value = '';
            }
        }

        function removeSelectedAlimentacao(val){
            _selectedAlimentacaoArr = _selectedAlimentacaoArr.filter(v=>v !== val);
            renderTags('filter_alimentacao_selected', _selectedAlimentacaoArr.map(v=>({ value:v, label:v, type:'alim' })));
        }
        ...
        function addSelectedMes(val, label){
            ...
            if(!_selectedMeses.includes(n)){
                ...
                renderTags('filter_mes_selected', _selectedMeses.map(v=>({ value:v, label: label || getMonthName(v), type:'mes' })));
                const mh = document.getElementById('filter_mes'); if(mh) mh.value = _selectedMeses.join(',');
                const im = document.getElementById('filter_mes_display'); if(im) im.value = '';
            }
        }

        function removeSelectedMes(val){
            _selectedMeses = _selectedMeses.filter(v=>v !== val);
            renderTags('filter_mes_selected', _selectedMeses.map(v=>({ value:v, label: getMonthName(v), type:'mes' })));
            const mh = document.getElementById('filter_mes'); if(mh) mh.value = _selectedMeses.join(',');
        }
        ...
        function carregarEspeciesForMeusPets(){
            ...
            const set = new Set();
            _meusPetsData.forEach(p => {
                const especie = p.especie || p.raca || p.especie_raca || p.species || p.breed || '';
                if(especie && especie.toString().trim() !== '') set.add(especie.toString().trim());
            });
    ...
    // carregar tipos de alimentação únicos a partir dos pets carregados
        function carregarAlimentacoesForMeusPets(){
            ...
            const set = new Set();
            _meusPetsData.forEach(p => {
                const alim = p.alimentacao || p.tipo_alimentacao || p.alimentacao_tipo || p.food || '';
                if(alim && alim.toString().trim() !== '') set.add(alim.toString().trim());
            });
        ...
        if (inputEspecie && especieContainer) {
            ...
            } catch (e) { console.warn('não foi possível mover especieContainer para body', e); }
            ...
            document.addEventListener('click', function(e){ if(!especieContainer.contains(e.target) && e.target !== inputEspecie) especieContainer.style.display = 'none'; });
            window.addEventListener('scroll', function(){ if(especieContainer.style.display === 'block') positionEspecieSuggest(); });
            window.addEventListener('resize', function(){ if(especieContainer.style.display === 'block') positionEspecieSuggest(); });
        }
        ...
        if (inputAlimentacao && alimentacaoContainer) {
            ...
            } catch (e) { console.warn('não foi possível mover alimentacaoContainer para body', e); }
            ...
            function positionAlimentacaoSuggest(){
                ...
                alimentacaoContainer.style.top = (rect.bottom + window.scrollY + 6) + 'px';
            ...
            document.addEventListener('click', function(e){ if(!alimentacaoContainer.contains(e.target) && e.target !== inputAlimentacao) alimentacaoContainer.style.display = 'none'; });
            window.addEventListener('scroll', function(){ if(alimentacaoContainer.style.display === 'block') positionAlimentacaoSuggest(); });
            window.addEventListener('resize', function(){ if(alimentacaoContainer.style.display === 'block') positionAlimentacaoSuggest(); });
        }
        ...
        if (inputTag && tagContainer) {
            ...
            } catch (e) { console.warn('não foi possível mover tagContainer para body', e); }
            ...
            document.addEventListener('click', function(e){ if(!tagContainer.contains(e.target) && e.target !== inputTag) tagContainer.style.display = 'none'; });
            window.addEventListener('scroll', function(){ if(tagContainer.style.display === 'block') positionTagSuggest(); });
            window.addEventListener('resize', function(){ if(tagContainer.style.display === 'block') positionTagSuggest(); });
        }
    ...
    // configurar campo Possui plano para usar o mesmo estilo de suggestions do Tipo de alimentação
        ...
        if (inputPlanoDisplay && planoHidden && planoContainer) {
            ...
            } catch (e) { console.warn('não foi possível mover planoContainer para body', e); }
            ...
            planoContainer.querySelectorAll('.suggestion-item').forEach(item => item.addEventListener('click', function(e){
                e.stopPropagation();
            ...
            document.addEventListener('click', function(e){ if(!planoContainer.contains(e.target) && e.target !== inputPlanoDisplay) planoContainer.style.display = 'none'; });
            window.addEventListener('scroll', function(){ if(planoContainer.style.display === 'block') positionPlanoSuggest(); });
            window.addEventListener('resize', function(){ if(planoContainer.style.display === 'block') positionPlanoSuggest(); });
        }
        ...
        if (inputAtivoDisplay && ativoHidden && ativoContainer) {
            ...
            } catch (e) { console.warn('não foi possível mover ativoContainer para body', e); }
            ...
            inputAtivoDisplay.addEventListener('focus', function(){ positionAtivoSuggest(); ativoContainer.style.display = 'block'; });
            inputAtivoDisplay.addEventListener('input', function(){
                const q = (this.value||'').toLowerCase().trim();
                const items = ativoContainer.querySelectorAll('.suggestion-item');
                items.forEach(it => { const txt = (it.textContent||'').toLowerCase(); it.style.display = txt.indexOf(q) === -1 ? 'none' : 'block'; });
                // limpar valor oculto enquanto digita
                ativoHidden.value = '';
                positionAtivoSuggest();
            });

            ativoContainer.querySelectorAll('.suggestion-item').forEach(item => item.addEventListener('click', function(e){
                e.stopPropagation();
                const val = item.getAttribute('data-value') || '';
            ...
            document.addEventListener('click', function(e){ if(!ativoContainer.contains(e.target) && e.target !== inputAtivoDisplay) ativoContainer.style.display = 'none'; });
            window.addEventListener('scroll', function(){ if(ativoContainer.style.display === 'block') positionAtivoSuggest(); });
            window.addEventListener('resize', function(){ if(ativoContainer.style.display === 'block') positionAtivoSuggest(); });
        }
        ...
        if (inputMesDisplay && mesHidden && mesContainer) {
            ...
            } catch (e) { console.warn('não foi possível mover mesContainer para body', e); }
            ...
            mesContainer.querySelectorAll('.suggestion-item').forEach(item => item.addEventListener('click', function(e){
                e.stopPropagation();
                const val = item.getAttribute('data-value') || '';
            ...
            document.addEventListener('click', function(e){ if(!mesContainer.contains(e.target) && e.target !== inputMesDisplay) mesContainer.style.display = 'none'; });
            window.addEventListener('scroll', function(){ if(mesContainer.style.display === 'block') positionMesSuggest(); });
            window.addEventListener('resize', function(){ if(mesContainer.style.display === 'block') positionMesSuggest(); });
        }

        if (btnPesquisar){ btnPesquisar.addEventListener('click', function(e){ e.preventDefault();
            ...
            if(selectedGrupoKeys.length === 0 && inputVal) selectedGrupoKeys.push(grupoKeyFromName(inputVal));

            // espécies selecionadas (array) - fallback para input livre
            const especieInputVal = document.getElementById('filter_especie')?.value || '';
            const selectedEspecies = (_selectedEspecies && _selectedEspecies.length>0) ? _selectedEspecies.slice() : (especieInputVal ? [especieInputVal] : []);
            ...
            const selectedTagsLocal = (_selectedTags && _selectedTags.length>0) ? _selectedTags.slice() : (tagInputVal ? [tagInputVal] : []);

            // alimentações selecionadas
            const alimentacaoInputVal = document.getElementById('filter_alimentacao')?.value || '';
            const selectedAlimentacoes = (_selectedAlimentacaoArr && _selectedAlimentacaoArr.length>0) ? _selectedAlimentacaoArr.slice() : (alimentacaoInputVal ? [alimentacaoInputVal] : []);
            ...
            let selectedMeses = [];
            if(_selectedMeses && _selectedMeses.length>0) selectedMeses = _selectedMeses.slice();
            else if(mesHiddenVal) selectedMeses = mesHiddenVal.split(',').map(s=>Number(s)).filter(n=>!isNaN(n));
            ...
            const filtered = _meusPetsData.filter(p => {
                ...
                if (selectedGrupoKeys.length > 0) {
                    ...
                    if(!cliente) okGrupo = false;
                        ...
                        const grupoNome = cliente.grupo_cliente || cliente.grupo || cliente.group || '';
                ...
                if (normSelectedEspecies.length > 0) {
                    const especiePet = (p.especie || p.raca || p.species || p.breed || '').toString().trim();
                ...
                if (normSelectedAlims.length > 0) {
                    const alimPet = (p.alimentacao || p.tipo_alimentacao || p.alimentacao_tipo || p.food || '').toString().trim();
                ...
                let okPlano = true;
                if (planoVal) {
                    const planoPet = (p.plano || p.possui_plano || p.has_plan || '').toString().trim();
                    if (planoVal === 'sim') {
                        okPlano = normalizeStr(planoPet) === 'sim' || normalizeStr(planoPet) === 'true' || normalizeStr(planoPet) === 's' || planoPet === '1';
                ...
                if (selectedMeses.length > 0) {
                    let nascimentoRaw = p.data_nascimento || p.nascimento || p.dataNascimento || p.birthdate || p.dob || '';
                    let nascimentoDate = null;
                    if (nascimentoRaw) {
                        try {
                            if (typeof nascimentoRaw === 'string' && nascimentoRaw.indexOf('/') !== -1) {
                ...
                if (filterDataVal) {
                    ...
                    const possible = p.data_cadastro || p.dataCadastro || p.cadastro || p.created_at || p.createdAt || p.created || p.dataCriacao || p.data_criacao || p.criado_em || p.criadoEm || '';
                    ...
                    try {
                        ...
                        } else if (typeof possible === 'number' || (!isNaN(Number(possible)) && String(possible).length>0)) {
                ...
                if (selectedTagsLocal.length > 0) {
                    ...
                    let petTags = [];
                    try {
                        if (Array.isArray(p.tags)) petTags = p.tags.map(t=>String(t).trim());
                        else if (typeof p.tags === 'string') {
                            const raw = p.tags.trim();
                            if (raw.startsWith('[')) {
                                try { petTags = JSON.parse(raw); } catch(e) { petTags = raw.replace(/\[|\]/g,'').split(',').map(s=>s.trim()).filter(Boolean); }
                            } else {
                                petTags = raw.split(',').map(s=>s.trim()).filter(Boolean);
                            }
                        }
                    } catch(e){ petTags = []; }

                    const normPetTags = petTags.map(t=>normalizeStr(t));
                    okTags = selectedTagsLocal.some(st => normPetTags.includes(normalizeStr(st)));
                }

                return okGrupo && okEspecie && okAlimentacao && okPlano && okMes && okDataCadastro && okTags;
            });

            // se nenhum filtro aplicado, mostra todos (inclui Tags na checagem)
            if(selectedGrupoKeys.length === 0 && normSelectedEspecies.length === 0 && normSelectedAlims.length === 0 && (!planoVal) && selectedMeses.length === 0 && !filterDataVal && selectedTagsLocal.length === 0){
                renderPetsTable(_meusPetsData);
            } else {
                renderPetsTable(filtered);
            }
            ...
            btnToggle?.classList.remove('active');
            try { if (window.__meusPetsSuggestContainer) window.__meusPetsSuggestContainer.style.display = 'none'; } catch(e){}
            try { if (window.__meusPetsEspecieSuggestContainer) window.__meusPetsEspecieSuggestContainer.style.display = 'none'; } catch(e){}
            try { if (window.__meusPetsTagSuggestContainer) window.__meusPetsTagSuggestContainer.style.display = 'none'; } catch(e){}
        }); }
...
window.toggleCalendarioFilter = function(){
    ...
    if(!cal) return;
    if (cal.style.display === 'none' || !cal.style.display) mostrarCalendarioFilter(); else esconderCalendarioFilter();
}
...
function mostrarCalendarioFilter(){
    ...
    calendarioFilterAtual = new Date(brasilia.getFullYear(), brasilia.getMonth(), 1);
    renderizarCalendarioFilter();

    setTimeout(()=>{ cal.style.transition = 'all 0.25s cubic-bezier(0.4,0,0.2,1)'; cal.style.opacity = '1'; cal.style.transform = 'translateY(0)'; }, 10);

    setTimeout(()=>{ document.addEventListener('click', fecharCalendarioForaFilter); }, 120);
...
function esconderCalendarioFilter(){
    ...
    cal.style.opacity = '0'; cal.style.transform = 'translateY(-6px)';
    setTimeout(()=>{ cal.style.display = 'none'; }, 180);
...
function renderizarCalendarioFilter(){
    ...
    const primeiroDia = new Date(calendarioFilterAtual.getFullYear(), calendarioFilterAtual.getMonth(), 1);
    const ultimoDia = new Date(calendarioFilterAtual.getFullYear(), calendarioFilterAtual.getMonth() + 1, 0);
    ...
    for(let i=0;i<inicioDaSemana;i++){ const dv = document.createElement('div'); dv.style.cssText = `height:${cellH};`; diasGrid.appendChild(dv); }
    ...
    diaElemento.style.cssText = `height:${cellH}; display:flex; align-items:center; justify-content:center; cursor:pointer; border-radius:6px; font-size:13px; transition:all 0.18s ease; user-select:none;`;
        ...
        const dataAtual = new Date(calendarioFilterAtual.getFullYear(), calendarioFilterAtual.getMonth(), dia);
        const ehHoje = dataAtual.toDateString() === hojeBrasilia.toDateString();

        if(ehHoje){ diaElemento.style.background = '#2c5aa0'; diaElemento.style.color='white'; diaElemento.style.fontWeight='600'; }
        else diaElemento.style.color = '#333';

        if(dataSelecionadaFilter && dataAtual.toDateString() === dataSelecionadaFilter.toDateString()){ diaElemento.style.background = '#28a745'; diaElemento.style.color='white'; diaElemento.style.fontWeight='600'; }

        diaElemento.addEventListener('mouseenter', function(){ if(!ehHoje && (!dataSelecionadaFilter || dataAtual.toDateString() !== dataSelecionadaFilter.toDateString())){ this.style.background = '#f0f8ff'; this.style.color = '#2c5aa0'; } });
        diaElemento.addEventListener('mouseleave', function(){ if(!ehHoje && (!dataSelecionadaFilter || dataAtual.toDateString() !== dataSelecionadaFilter.toDateString())){ this.style.background = 'transparent'; this.style.color = '#333'; } });

        diaElemento.addEventListener('click', function(){ selecionarDataFilter(dataAtual); });

        diasGrid.appendChild(diaElemento);
    }

    document.getElementById('btnFiltroMesAnterior').onclick = function(){ calendarioFilterAtual.setMonth(calendarioFilterAtual.getMonth() - 1); renderizarCalendarioFilter(); };
    document.getElementById('btnFiltroMesProximo').onclick = function(){ calendarioFilterAtual.setMonth(calendarioFilterAtual.getMonth() + 1); renderizarCalendarioFilter(); };
}
        ...
        if (selectedTagsLocal.length > 0) {
            ...
            try {
                if (Array.isArray(p.tags)) petTags = p.tags.map(t=>String(t).trim());
                    ...
                    if (raw.startsWith('[')) {
                        try { petTags = JSON.parse(raw); } catch(e) { petTags = raw.replace(/\[|\]/g,'').split(',').map(s=>s.trim()).filter(Boolean); }
                    } else { petTags = raw.split(',').map(s=>s.trim()).filter(Boolean); }
                }
            } catch(e){ petTags = []; }
...
function generatePetsReportHTML(pets){
    const logoEl = document.querySelector('.logo-pet-cria') || document.querySelector('.logo img');
    const logoSrc = (logoEl && logoEl.getAttribute && logoEl.getAttribute('src')) ? logoEl.getAttribute('src') : '/logos/logo_pet_cria-removebg-preview.png';
    ...
    const rowsHtml = pets.map(p => {
        const id = p.id || p.codigo || p.pet_id || p._id || '';
        const petName = escapeHtml(nomeDoPet(p) || '');
        const cliente = (p.cliente_id && _meusClientsMap[p.cliente_id]) ? nomeDoCliente(_meusClientsMap[p.cliente_id]) : (p.cliente_nome || p.cliente || '');
        const especie = escapeHtml((p.especie || p.raca || p.species || '').toString());
        const raca = escapeHtml((p.raca || p.raca_pet || '').toString());
        const nascimento = escapeHtml(p.data_nascimento || p.nascimento || p.dob || '');
...
function exportPetsToXLS(pets){
    try{
        if (!window.XLSX) { showNotification('Biblioteca XLSX não encontrada', 'error'); return; }
        if(!pets || pets.length === 0){ showNotification('Nenhum pet para exportar', 'info'); return; }
        const dados = pets.map(p => ({
            ID: p.id || p.codigo || p.pet_id || p._id || '',
            Nome: nomeDoPet(p) || '',
            Cliente: (p.cliente_id && _meusClientsMap[p.cliente_id]) ? nomeDoCliente(_meusClientsMap[p.cliente_id]) : (p.cliente_nome || p.cliente || ''),
        ...
        showNotification(`✅ Exportado ${pets.length} pets em ${filename}`, 'success');
    } catch(e){ console.error('Erro exportPetsToXLS', e); showNotification('Erro ao exportar XLS', 'error'); }
}
...
function ensureXLSX(){
    return new Promise((resolve, reject) => {
        ...
        const existing = Array.from(document.getElementsByTagName('script')).find(s => s.src && s.src.indexOf('xlsx') !== -1);
        if (existing) {
            existing.addEventListener('load', () => resolve(window.XLSX));
            existing.addEventListener('error', () => reject(new Error('Failed to load existing XLSX script')));
            return;
        }
        const s = document.createElement('script');
        s.src = src;
        s.async = true;
        s.onload = function(){ if (window.XLSX) resolve(window.XLSX); else reject(new Error('XLSX loaded but window.XLSX not set')); };
        s.onerror = function(e){ reject(new Error('Erro ao carregar script XLSX: ' + e)); };
...
function openExportDialogForPets(){
    ...
    modal.style.background='rgba(0,0,0,0.45)'; modal.style.zIndex='20000';
    ...
    document.getElementById('__exportPetsClose').addEventListener('click', ()=>{ modal.style.display='none'; });

    document.getElementById('__exportPetsPreview').addEventListener('click', ()=>{
        const fmt = document.getElementById('__exportPetsFormat').value;
        const pets = getFilteredPetsUsingCurrentFilters();
        if(fmt === 'pdf'){
            const html = generatePetsReportHTML(pets);
            const w = window.open('about:blank','_blank');
            if(!w) { showNotification('Bloqueador de popups impediu a abertura', 'error'); return; }
...
function selecionarHojeFilter(){ const hoje = new Date(); const hojeBrasilia = new Date(hoje.toLocaleString("en-US", {timeZone: "America/Sao_Paulo"})); selecionarDataFilter(hojeBrasilia); }

function fecharCalendarioFilter(){ esconderCalendarioFilter(); }
