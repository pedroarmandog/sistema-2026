...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setInterval(() => {
        ...
        try{
            document.querySelectorAll('.nav-item-with-submenu > .nav-item').forEach(function(a){
                try{
        ...
        try{
            document.querySelectorAll('.nav-item[href$="marketing.html"]').forEach(function(a){
                try{
                    if(a.hasAttribute('href')){
                        a.dataset._origHref = a.getAttribute('href');
                        a.removeAttribute('href');
                ...
                try{
                    ...
                    if(href.indexOf('novo-produto.html') !== -1) {
                        ...
                        a.addEventListener('click', function(ev){
                            ...
                            }catch(e){ console.debug('erro interceptar novo-produto link', e); window.location.href = a.href; }
                        });
                    }
                }catch(e){}
            });
        }catch(e){ console.debug('falha configurar interceptador novo-produto', e); }
});
...
function handleTabChange(activeTab) {
    ...
    // Aqui você pode implementar a lógica para mostrar diferentes conteúdos
    // baseado na tab ativa
}

function loadDashboardData() {
    // Simular carregamento dos produtos com estoque baixo
...
function updateStatistics() {
    ...
    Promise.all([
        ...
        fetchTicketMedio(),
        fetchClientesNoMes()
    ]).then(([agendamentos, atendimentos, ticket, clientes]) => {
        updateStatCard(0, agendamentos);
        updateStatCard(1, atendimentos);
        updateStatCard(2, formatCurrency(ticket));
        updateStatCard(3, clientes);
    }).catch(error => {
...
async function fetchAtendimentosHoje() {
    try {
        // Buscar contagem de vendas/atendimentos realizados hoje via API
        const response = await fetch('http://localhost:3000/api/dashboard/vendas-hoje');
        if (!response.ok) {
            console.error('Erro na API vendas-hoje:', response.status);
            return 0;
        }
...
async function fetchTicketMedio() {
    try {
        const response = await fetch('http://localhost:3000/api/dashboard/ticket-medio');
        if (!response.ok) {
            console.error('Erro na API ticket-medio:', response.status);
            return 0;
        }
        const data = await response.json();
        if (data && typeof data.ticketMedio === 'number') {
            return data.ticketMedio;
        }
        return 0;
    } catch (error) {
...
async function fetchClientesNoMes() {
    try {
        console.log('🔍 Buscando clientes cadastrados no mês...');
        ...
        const clientesDoMes = clientes.filter(c => {
            ...
            const isDoMes = dataCriacao >= inicioMes;
            console.log(`Cliente ${c.nome}: ${dataCriacao.toISOString()} - ${isDoMes ? 'SIM' : 'NÃO'}`);
            return isDoMes;
        });
        
        console.log(`✅ ${clientesDoMes.length} clientes cadastrados no mês`);
        return clientesDoMes.length;
    } catch (error) {
...
function refreshWidget(widgetElement) {
    const loadingHTML = `
...
document.addEventListener('click', function(e) {
    if (e.target.closest('.btn-icon i.fa-sync')) {
...
(function() {
    ...
    async function carregarUsuarioLogado() {
        ...
        if (!usuarioId) return { nome: 'Usuário', nomeEmpresa: 'Nenhuma empresa cadastrada' };

        try {
            const API_BASE = (window.__API_BASE__ && window.__API_BASE__.toString()) || 'http://localhost:3000';
            
            // Buscar dados do usuário
            const responseUsuario = await fetch(API_BASE + '/api/usuarios/' + usuarioId);
            ...
            if (usuario.empresas && usuario.empresas.length > 0) {
                ...
                const empresaId = empresaObj && (empresaObj.id || empresaObj.ID || empresaObj.companyId);
                if (!empresaId) {
                    console.warn('carregarUsuarioLogado: empresaId indefinido em usuario.empresas[0]', empresaObj);
                } else {
                    const responseEmpresa = await fetch(API_BASE + '/api/empresas/' + empresaId);
                    if (responseEmpresa.ok) {
                        const empresa = await responseEmpresa.json();
                        nomeEmpresa = empresa.nomeFantasia || empresa.razaoSocial || 'Nenhuma empresa cadastrada';
                    } else {
                        console.warn('carregarUsuarioLogado: empresa fetch falhou', responseEmpresa.status);
                    }
                }
            }
    ...
    function initUserProfileDropdown() {
        ...
        userMenus.forEach(userMenu => {
            ...
            carregarUsuarioLogado().then(dados => {
                ...
                if (userNameSpan && (userNameSpan.textContent === 'Pedro' || userNameSpan.textContent === '...')) {
