...
async function abrirDropdownFornecedor(inputEl) {
    try {
        ...
        function closeWrapper() {
            try { window.removeEventListener('scroll', updatePosition, true); } catch(e){}
            try { window.removeEventListener('resize', updatePosition); } catch(e){}
        ...
        let fornecedores = [];
        try { fornecedores = await ApiClient.getFornecedores(); } catch(e){ console.warn('[manutecao-produtos] falha ao buscar fornecedores:', e); fornecedores = []; }

        function renderList(arr){
            list.innerHTML = '';
            if (!Array.isArray(arr) || arr.length === 0) {
                const empty = document.createElement('div'); empty.textContent = 'Nenhum fornecedor encontrado'; empty.style.padding = '6px 8px'; empty.style.color = '#666'; list.appendChild(empty); return;
            }
            arr.forEach(f => {
                const el = document.createElement('div');
                el.style.padding = '8px 10px'; el.style.cursor = 'pointer'; el.style.borderRadius = '4px';
                el.textContent = f.nome || f.name || (f.razaoSocial || f.codigo || '');
                el.title = el.textContent;
                el.addEventListener('mouseenter', () => el.style.background = '#f1f5fb');
        ...
        searchBox.addEventListener('input', function(){
            ...
            const filtered = fornecedores.filter(f => (f.nome||'').toString().toLowerCase().indexOf(v) !== -1 || (f.razaoSocial||'').toString().toLowerCase().indexOf(v) !== -1);
            renderList(filtered);
        });

        btnFechar.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); closeWrapper(); });

    } catch(err) { console.error('[manutecao-produtos] abrirDropdownFornecedor falhou:', err); }
}
...
function salvarEstadoSubmenu(submenuId, isOpen) {
    ...
    try { /* intentionally do nothing */ } catch(e){}
}
...
function limparEstadoSubmenus() { try { /* no-op: não remover nada do localStorage */ } catch(e){} }
...
function configurarEventListenersManutencao() {
    ...
    if (linkColunas) {
        linkColunas.addEventListener('click', (e) => {
            e.preventDefault();
            alert('Seleção de colunas será implementada em breve!');
        });
    }
        ...
        if (input) {
            ...
            try {
                if (input.id === 'filtroFornecedor') {
                    input.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); abrirDropdownFornecedor(input); });
                    input.addEventListener('focus', function(e){ abrirDropdownFornecedor(input); });
                }
                // abrir dropdown de agrupamento ao clicar no campo de filtro
                if (input.id === 'filtroAgrupamento') {
                    input.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); abrirDropdownAgrupamentoFiltro(input); });
                    input.addEventListener('focus', function(e){ abrirDropdownAgrupamentoFiltro(input); });
                }
            } catch(e) { console.debug('erro ao ligar dropdown fornecedor', e); }
        }
    });
...
function realizarPesquisaProdutos() {
    ...
    filtros.produto = filtroProduto ? filtroProduto.value.trim().toLowerCase() : '';
    filtros.fornecedor = filtroFornecedor ? filtroFornecedor.value.trim().toLowerCase() : '';
    // também capturar id do fornecedor selecionado (se setado pelo dropdown)
    filtros.fornecedorId = filtroFornecedor && filtroFornecedor.dataset && filtroFornecedor.dataset.fornecedorId ? String(filtroFornecedor.dataset.fornecedorId) : '';
    filtros.agrupamento = filtroAgrupamento ? filtroAgrupamento.value.trim().toLowerCase() : '';

    // Logs de debug para inspecionar filtros antes da chamada API
    try {
        console.log('[manutecao-produtos] filtros iniciais:', JSON.stringify(filtros));
        console.log('[manutecao-produtos] filtros.fornecedorId:', filtros.fornecedorId);
    } catch (e) { console.debug('[manutecao-produtos] erro log filtros', e); }
    ...
    (async function(){
        try {
            ...
            try {
                if (params.fornecedorId && Array.isArray(resp) && resp.length === 0 && filtros.fornecedor) {
                    console.log('[manutecao-produtos] fallback: nenhum item retornado por fornecedorId, tentando por nome do fornecedor');
                    ...
                    console.log('[manutecao-produtos] fallback resposta (por fornecedor):', Array.isArray(resp) ? resp.length : resp);
                }
            } catch(e) { console.debug('[manutecao-produtos] erro no fallback por fornecedor', e); }
            // Log da resposta da API para depuração
            try {
                console.log('[manutecao-produtos] API /itens resposta (array):', Array.isArray(resp) ? resp.length : 'não é array');
                console.log('[manutecao-produtos] amostra da resposta:', (Array.isArray(resp) && resp.length>0) ? resp.slice(0,3) : resp);
            } catch(e) { console.debug('[manutecao-produtos] erro ao logar resposta API', e); }
            if (!Array.isArray(resp)) {
                ...
                produtosData = resp.map(item => {
                    const fornecedor = (item.fornecedores && Array.isArray(item.fornecedores) && item.fornecedores.length > 0)
                        ? (typeof item.fornecedores[0] === 'string' ? item.fornecedores[0] : (item.fornecedores[0].nome || ''))
                        : (item.fornecedor || '');
                    // tentar obter id do fornecedor (quando disponível como objeto no backend)
                    let fornecedorId = null;
                    try {
                        if (item.fornecedores && Array.isArray(item.fornecedores) && item.fornecedores.length > 0 && typeof item.fornecedores[0] === 'object' && item.fornecedores[0].id) {
                    ...
                    } catch(e) { fornecedorId = null; }
                    return {
                        backendId: item.id || item.codigo || '',
                        codigo: item.codigo || item.id || '',
                        descricao: item.nome || item.descricao || '',
                        custo: (item.custoBase !== undefined && item.custoBase !== null) ? Number(item.custoBase) : (item.custo || null),
                        venda: (item.preco !== undefined && item.preco !== null) ? Number(item.preco) : (item.venda || null),
                ...
                try { console.log('[manutecao-produtos] produtosData após mapeamento:', produtosData.length); } catch(e) {}
            }
            ...
            renderizarTabelaProdutos();
            atualizarPaginacaoManutencao();
            console.log(`🔍 Pesquisa realizada (API) - ${filteredData.length} produtos encontrados`);
        } catch (e) {
            ...
            filteredData = produtosData.filter(produto => {
                const matchProduto = !filtros.produto || 
                    (produto.descricao && produto.descricao.toLowerCase().includes(filtros.produto)) ||
                    (produto.codigo && produto.codigo.toString().includes(filtros.produto));
                const matchFornecedor = !filtros.fornecedor || 
                    (produto.fornecedor && produto.fornecedor.toLowerCase().includes(filtros.fornecedor));
                const matchAgrupamento = !filtros.agrupamento || 
                    (produto.agrupamento && produto.agrupamento.toLowerCase().includes(filtros.agrupamento));
...
function renderizarTabelaProdutos() {
    ...
    dadosPaginados.forEach(produto => {
        ...
        tdCusto.textContent = (produto.custo !== null && produto.custo !== undefined) ? formatCurrency(produto.custo) : '';
        ...
        tdVenda.textContent = (produto.venda !== null && produto.venda !== undefined) ? formatCurrency(produto.venda) : '';
...
async function abrirDropdownAgrupamento(tdElement, produtoRow) {
    try {
        ...
        btnNovo.addEventListener('mouseenter', () => { btnNovo.style.background = '#1a73e8'; btnNovo.style.transform = 'translateY(-1px)'; });
        btnNovo.addEventListener('mouseleave', () => { btnNovo.style.background = '#2b8cff'; btnNovo.style.transform = 'none'; });
        ...
        btnFechar.addEventListener('mouseenter', () => { btnFechar.style.background = '#c82333'; btnFechar.style.transform = 'translateY(-1px)'; });
        btnFechar.addEventListener('mouseleave', () => { btnFechar.style.background = '#dc3545'; btnFechar.style.transform = 'none'; });
        footer.appendChild(btnFechar);
        ...
        console.log('[manutecao-produtos] dropdown adicionado ao DOM em fixed position left=' + left + 'px top=' + top + 'px');
        ...
        function renderList(data) {
            ...
            data.forEach(item => {
                ...
                itemEl.addEventListener('mouseenter', () => { itemEl.style.background = '#f1f5fb'; });
                ...
                itemEl.addEventListener('click', function(ev){
                    ...
                    if (Array.isArray(item.subgrupos) && item.subgrupos.length > 0) {
                        ...
                        parentEl.addEventListener('mouseenter', () => { parentEl.style.background = '#f1f5fb'; });
                        ...
                        const sep = document.createElement('div'); sep.style.height = '6px'; list.appendChild(sep);
                        ...
                        item.subgrupos.forEach(sub => {
                            ...
                            subEl.addEventListener('mouseenter', () => { subEl.style.background = '#f1f5fb'; });
    ...
    btnFechar.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); closeWrapper(); });
        ...
        btnNovo.addEventListener('click', async function(e){
            ...
            try {
                ...
                try {
                    const existing = Array.from(document.scripts).find(s => (s.src||'').indexOf('agrupamento.js') !== -1);
                    if (!existing) {
                        const script = document.createElement('script');
                        script.src = './agrupamento.js';
                        script.async = true;
                        script.onload = function(){
                            try { if (typeof openAdicionarAgrupamentoModal === 'function') openModalAndRefresh(); }
                            catch(e){ console.debug('onload open modal failed', e); }
                        };
                        script.onerror = function(err){ console.error('[manutecao-produtos] falha ao carregar agrupamento.js', err); };
                        ...
                        const waitFor = function(resolve, reject, attempts){
                            if (typeof openAdicionarAgrupamentoModal === 'function') return resolve(true);
                            if (attempts <= 0) return reject(new Error('timeout waiting for modal function'));
                            setTimeout(() => waitFor(resolve, reject, attempts-1), 150);
                        };
                        await new Promise((res, rej) => waitFor(res, rej, 10)).then(()=> openModalAndRefresh()).catch(err=>{ console.debug('[manutecao-produtos] modal not available after load', err); });
                    }
                } catch(err) {
...
async function abrirDropdownAgrupamentoFiltro(inputEl) {
    try {
        ...
        let agrupamentos = [];
        try { agrupamentos = await ApiClient.getAgrupamentos(); } catch(e){ console.warn('[manutecao-produtos] falha ao buscar agrupamentos (filtro):', e); agrupamentos = []; }

        function render(data){
            list.innerHTML = '';
            if (!Array.isArray(data) || data.length === 0) {
                const empty = document.createElement('div'); empty.textContent = 'Nenhum agrupamento encontrado'; empty.style.padding = '6px 8px'; empty.style.color = '#666'; list.appendChild(empty); return;
            }
            data.forEach(item => {
                const el = document.createElement('div');
                el.style.padding = '8px 10px'; el.style.cursor = 'pointer'; el.style.borderRadius = '4px';
                ...
                el.addEventListener('click', function(ev){
                    ...
                    if (Array.isArray(item.subgrupos) && item.subgrupos.length > 0) {
                        list.innerHTML = '';
                        const back = document.createElement('div'); back.textContent = '← Voltar'; back.style.padding = '6px 8px'; back.style.cursor = 'pointer'; back.style.color = '#2b8cff'; back.addEventListener('click', () => render(agrupamentos)); list.appendChild(back);

                        const parentEl = document.createElement('div'); parentEl.style.padding = '6px 8px'; parentEl.style.cursor = 'pointer'; parentEl.style.fontWeight = '600'; parentEl.textContent = item.nome || item.name || '';
                        parentEl.addEventListener('click', function(){ inputEl.value = item.nome || item.name || ''; if (item.id) inputEl.dataset.agrupamentoId = item.id; closeWrapper(); });
                        list.appendChild(parentEl);
                        const sep = document.createElement('div'); sep.style.height = '6px'; list.appendChild(sep);
                        item.subgrupos.forEach(sub => {
                            const s = document.createElement('div'); s.style.padding = '6px 12px'; s.style.cursor = 'pointer'; s.textContent = (item.nome||'') + ' / ' + (sub.nome || sub.name || sub);
                            s.addEventListener('click', function(){ inputEl.value = sub.nome || sub.name || sub; if (sub.id) inputEl.dataset.agrupamentoId = sub.id; closeWrapper(); });
                            list.appendChild(s);
                        });
                    } else {
        ...
        searchBox.addEventListener('input', function(){
            ...
            const filtered = agrupamentos.filter(a => (a.nome||'').toString().toLowerCase().indexOf(v) !== -1 || (a.subgrupos||[]).some(s => (s.nome||'').toString().toLowerCase().indexOf(v) !== -1));
            render(filtered);
        });

        function closeWrapper(){ try{ wrapper.parentElement && wrapper.parentElement.removeChild(wrapper); }catch(e){} }
        btnFechar.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); closeWrapper(); });

    } catch(err) { console.error('[manutecao-produtos] abrirDropdownAgrupamentoFiltro falhou:', err); }
}
...
async function salvarCelulaAlterada(backendId, tdElement, produtoRow) {
    try {
        ...
        const payload = {};
        if (colIndex === 1) { // descricao -> nome
            const novo = tdElement.textContent.trim();
            if (novo === (produtoRow.descricao || '').trim()) return;
            payload.nome = novo;
        } else if (colIndex === 2) { // custo
            const novoTexto = tdElement.textContent.trim();
            const novoNum = parseCurrencyText(novoTexto);
            if (novoNum === null) {
                // invalid number, revert
                tdElement.textContent = produtoRow.custo !== null && produtoRow.custo !== undefined ? formatCurrency(produtoRow.custo) : '';
                return;
            }
            if (novoNum === produtoRow.custo) return;
            payload.custoBase = novoNum;
        } else if (colIndex === 3) { // venda
            const novoTexto = tdElement.textContent.trim();
            const novoNum = parseCurrencyText(novoTexto);
            if (novoNum === null) {
                tdElement.textContent = produtoRow.venda !== null && produtoRow.venda !== undefined ? formatCurrency(produtoRow.venda) : '';
                return;
            }
            if (novoNum === produtoRow.venda) return;
            payload.preco = novoNum;
        } else if (colIndex === 4) { // agrupamento
            const novo = tdElement.textContent.trim();
        ...
        if (payload.custoBase !== undefined) produtoRow.custo = Number(payload.custoBase);
        if (payload.preco !== undefined) produtoRow.venda = Number(payload.preco);
        if (payload.agrupamento !== undefined) produtoRow.agrupamento = payload.agrupamento;

        // normalizar exibição (formatar moeda)
        if (colIndex === 2) tdElement.textContent = formatCurrency(produtoRow.custo);
        if (colIndex === 3) tdElement.textContent = formatCurrency(produtoRow.venda);
        ...
        try { window.showToast && window.showToast('Falha ao salvar alteração', 'error'); } catch(e){}
    }
}
...
async function salvarLinhaCompleta(rowElement, produtoRow) {
    try {
        ...
        const payload = {};
        if (descricao !== (produtoRow.descricao || '').trim()) payload.nome = descricao;
        if (custo !== null && custo !== produtoRow.custo) payload.custoBase = custo;
        if (venda !== null && venda !== produtoRow.venda) payload.preco = venda;
        if (agrupamento !== (produtoRow.agrupamento || '').trim()) payload.agrupamento = agrupamento;

        if (Object.keys(payload).length === 0) {
            try { window.showToast && window.showToast('Nenhuma alteração para salvar', 'info'); } catch(e){}
            return;
        }

        const btn = rowElement.querySelector('.btn-salvar');
        if (btn) {
            btn.disabled = true; btn.textContent = 'Salvando...'; btn.style.opacity = '0.7';
        }

        console.log('[manutecao-produtos] salvando produto:', backendId, payload);
        await ApiClient.atualizarProduto(backendId, payload);
        console.log('[manutecao-produtos] resposta sucesso para produto:', backendId);
        ...
        try { window.showToast && window.showToast('Alterações salvas', 'success'); } catch(e){}
    } catch (e) {
        console.error('[manutecao-produtos] erro ao salvar linha (id=' + (rowElement.dataset.backendId||'') + '):', e);
        const btn = rowElement.querySelector('button.btn-salvar');
        if (btn) { btn.disabled = false; btn.textContent = 'Salvar'; btn.style.opacity = '1'; }
        try { window.showToast && window.showToast('Erro ao salvar alterações', 'error'); } catch(e){}
    }
}
