...
function showSystemToast(message, opts){
    try{
        ...
        if(!document.getElementById('systemToastStyles')){
            var s = document.createElement('style'); s.id = 'systemToastStyles';
            s.innerHTML = '\n' +
        ...
        var toast = document.createElement('div'); toast.className = 'system-toast';
        var icon = document.createElement('div'); icon.className = 'icon'; icon.innerHTML = '⚠';
        var txt = document.createElement('div'); txt.className = 'text'; txt.textContent = message || '';
        toast.appendChild(icon); toast.appendChild(txt);

        container.appendChild(toast);

        // animação simples
        toast.style.opacity = '0'; toast.style.transform = 'translateY(-6px)';
        requestAnimationFrame(function(){ toast.style.transition = 'opacity .18s, transform .18s'; toast.style.opacity = '1'; toast.style.transform = 'none'; });

        setTimeout(function(){ try{ toast.style.opacity = '0'; toast.style.transform = 'translateY(-6px)'; setTimeout(function(){ try{ toast.remove(); }catch(e){} }, 220); }catch(e){} }, duration);
    }catch(e){ console.warn('showSystemToast error', e); }
}
...
function configurarBotaoBuscarPrecos(){
    ...
    btn.addEventListener('click', async function(e){
        ...
        resultsContainer.innerHTML = '<div style="padding:14px;color:#666">Buscando produtos...</div>';

        try{
            const res = await fetch(url);
            let data = [];
            if(res.ok){ data = await res.json().catch(()=>[]); }
            else { console.warn('Buscar preços API returned', res.status); }
            ...
            resultsContainer.innerHTML = '<div style="padding:14px;color:#c00">Erro ao buscar produtos.</div>';
        }
    });
}
...
function renderProdutosPrecos(items){
    ...
    if(!items || items.length===0){
        container.innerHTML = '<div style="padding:14px;color:#666">Nenhum produto com preço alterado encontrado neste período.</div>';
        return;
    }
    ...
    items.forEach(p => {
        ...
        const precoBtnVal = (typeof p.precoNovo !== 'undefined' && p.precoNovo !== null) ? p.precoNovo : ((typeof p.preco !== 'undefined' && p.preco !== null) ? p.preco : (p.preco_venda || p.precoVenda || ''));
    ...
    container.querySelectorAll('.btn-add-print').forEach(btn => {
        btn.addEventListener('click', function(e){
            ...
            let nome = nomeEl ? String(nomeEl.textContent || '').trim() : ('Produto ' + id);
            let estoqueText = estoqueEl ? String(estoqueEl.textContent || '').trim() : '';

            // sanitizar nome caso contenha textos residuais (por precaução)
            nome = nome.replace(/Adicionar\s+à?\s+lista\s+de\s+Impressão/ig, '').trim();
            ...
            if (precoRaw !== undefined && precoRaw !== null && String(precoRaw).trim() !== ''){
                try{
                    let cleaned = String(precoRaw).replace(/\s/g,'').replace(/R\$|\$/g,'').trim();
                    cleaned = cleaned.replace(/[^0-9\-,.]/g,'');
                    const commaCount = (cleaned.match(/,/g) || []).length;
                    const dotCount = (cleaned.match(/\./g) || []).length;
                    if (commaCount === 1 && dotCount === 0) cleaned = cleaned.replace(',', '.');
                    cleaned = cleaned.replace(/(\.(?=\d{3}(?:\.|,|$)))/g, '');
                    const n = Number(cleaned);
                    if (!isNaN(n) && isFinite(n)) precoVal = n;
                }catch(ex){ precoVal = null; }
            }

            const novo = { id: String(id || Date.now()), codigo: '', produto: nome || ('Produto ' + id), quantidade: qty, estoque: Number(estoque) || 0 };
...
function configurarAdicionarProduto(){
    ...
    btn.addEventListener('click', function(e){
        ...
        if(!nome){ try{ showSystemToast('Informe o produto'); }catch(e){ alert('Informe o produto'); } produtoInput.focus(); return; }
        if(qty <= 0){ try{ showSystemToast('Informe quantidade válida'); }catch(e){ alert('Informe quantidade válida'); } qtyInput.focus(); return; }
        // Usar o id selecionado no autocomplete quando disponível (para manter mesmo id do meus-itens)
        var selectedId = produtoInput.dataset.itemId || produtoInput.getAttribute('data-item-id') || String(Date.now());
        // Adicionar à lista de etiquetas selecionadas
        const novo = { id: String(selectedId), produto: nome, quantidade: qty, estoque: 0 };
        adicionarEtiquetaNaLista(novo);
        ...
        try{ delete produtoInput.dataset.itemId; }catch(e){}
        produtoInput.focus();
...
function configurarProdutoAutocomplete(){
    ...
    async function fetchItens(q){
        ...
        }catch(e){ console.warn('fetchItens error', e); return []; }
    }
    ...
    function render(items){
        ...
        items.slice(0,40).forEach((it, idx) => {
            const el = document.createElement('div'); el.className = 'autocomplete-item'; el.tabIndex = 0;
            const nome = it.nome || it.descricao || '';
            el.innerHTML = `<div>${escapeHtml(nome)}</div>` + (it.codigo?`<div class="autocomplete-sub">${escapeHtml(it.codigo)}</div>`:'');
            el.addEventListener('click', function(ev){ ev.stopPropagation(); selectItem(it); });
            el.addEventListener('keydown', function(e){ if(e.key === 'Enter'){ e.preventDefault(); selectItem(it); } });
    ...
    function selectItem(item){
        ...
        const qty = document.getElementById('produtoQuantidade'); if(qty) qty.focus();
        dropdown.style.display = 'none';
    }

    const deb = debounce(async function(q){ const items = await fetchItens(q); render(items); }, 200);

    input.addEventListener('input', function(e){ deb(this.value.trim()); });
    input.addEventListener('focus', function(e){ deb(this.value.trim()); });
    input.addEventListener('click', function(e){ deb(this.value.trim()); });

    document.addEventListener('click', function(ev){ if(!dropdown.contains(ev.target) && ev.target !== input) dropdown.style.display = 'none'; });
    window.addEventListener('resize', function(){ if(dropdown.style.display!=='none') position(); });
    window.addEventListener('scroll', function(){ if(dropdown.style.display!=='none') position(); }, true);
}
...
function configurarBotaoBuscarNotas() {
    ...
    if (btnBuscarNotas) {
        btnBuscarNotas.addEventListener('click', function() {
            ...
            atualizarContadorEtiquetas();
            console.log('ℹ️ Busca de notas de entrada: implementação de backend ausente — nenhum produto adicionado.');
            try{ showSystemToast('Busca de notas de entrada não implementada (nenhum produto adicionado).'); }catch(e){}
        });
...
function configurarBotaoVisualizarEtiquetas() {
    const btnVisualizarEtiquetas = document.getElementById('btnVisualizarEtiquetas');
    ...
    if (btnVisualizarEtiquetas) {
        btnVisualizarEtiquetas.addEventListener('click', function() {
            ...
            const primeiraLinha = document.querySelector('input[type="number"][value="1"]').value;
            const primeiraColuna = document.querySelectorAll('input[type="number"][value="1"]')[1].value;
            const gerarPorEl = document.querySelector('.form-control-small');
            const gerarPor = gerarPorEl ? (gerarPorEl.tagName === 'SELECT' ? gerarPorEl.value : (gerarPorEl.textContent||gerarPorEl.value||'Código do produto')) : 'Código do produto';
            ...
            const etiquetasPayload = (etiquetasSelecionadas || []).map(it => ({
                ...
                preco: (it.preco !== undefined && it.preco !== null) ? it.preco : (it.preco_venda || it.precoVenda || it.preco_venda_formatted || it.preco_formatted || null)
            }));

            const modelo = (document.getElementById('modeloEtiquetaField') || {}).value || '';
            const apiUrl = (window.location.port === '3000') ? '/api/relatorios/etiquetas/pdf' : `${window.location.protocol}//${window.location.hostname}:3000/api/relatorios/etiquetas/pdf`;

            (async function(){
                try{
                    // Log do payload para depuração: confirma exatamente o que será enviado ao backend
                    try{ console.log('📤 etiquetasPayload (sample 0..9):', JSON.parse(JSON.stringify((etiquetasPayload||[]).slice(0,10)))); } catch(e){ console.log('📤 etiquetasPayload (raw):', etiquetasPayload); }
                    const resp = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type':'application/json' }, body: JSON.stringify({ etiquetas: etiquetasPayload, primeiraLinha: primeiraLinha, primeiraColuna: primeiraColuna, gerarPor: gerarPor, modelo: modelo }) });
                    if(!resp.ok){ const txt = await resp.text().catch(()=>null); alert('Erro gerando PDF: ' + (resp.status + ' ' + (txt || resp.statusText))); return; }
                ...
                }catch(err){ console.error('Erro ao gerar/abrir PDF', err); alert('Erro ao gerar PDF: ' + (err && err.message)); }
            })();
        });
...
function configurarImpressoraSelector(){
    ...
    current.addEventListener('click', function(e){ e.stopPropagation(); if(dropdown.style.display === 'block') closeDropdown(); else openDropdown(); });
    ...
    dropdown.querySelectorAll('.impressora-item').forEach(it => {
        it.addEventListener('click', async function(e){
            ...
            // persist to backend
            try{
                await fetch('/api/impressora', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ tipo: val }) });
                console.log('Impressora selecionada:', val);
            }catch(err){ console.warn('Erro salvando impressora', err); }
        });
    });
    ...
    (async function(){
        ...
        }catch(e){ console.warn('Erro carregando impressora', e); }
    })();
}
...
function configurarBotoesAcaoLista() {
    ...
    if (btnLimpar) {
        btnLimpar.addEventListener('click', function() {
            ...
            if (confirm(`Deseja realmente limpar ${etiquetasSelecionadas.length} etiqueta(s)?`)) {
                etiquetasSelecionadas = [];
                atualizarContadorEtiquetas();
...
function adicionarEtiquetaNaLista(item){
    ...
    const existe = etiquetasSelecionadas.find(e => String(e.id) === String(item.id));
    if(existe){
        existe.quantidade = (Number(existe.quantidade) || 0) + (Number(item.quantidade) || 1);
    } else {
...
function removerEtiquetaPorId(id){
    etiquetasSelecionadas = etiquetasSelecionadas.filter(e => String(e.id) !== String(id));
    atualizarContadorEtiquetas();
    renderListaEtiquetas();
}
...
function renderListaEtiquetas(){
    ...
    if(!etiquetasSelecionadas || etiquetasSelecionadas.length===0){
        container.innerHTML = '<div style="padding:12px;color:#666">Nenhuma etiqueta adicionada.</div>';
        return;
    }
    ...
    etiquetasSelecionadas.forEach((it, idx) => {
        ...
        qtyInput.type = 'number'; qtyInput.value = Number(it.quantidade) || 1; qtyInput.min = 1;
        qtyInput.style.width = '80px'; qtyInput.className = 'form-control-tiny';
        qtyInput.addEventListener('change', function(){ it.quantidade = Number(this.value) || 1; });
        ...
        btnRem.addEventListener('click', function(){ removerEtiquetaPorId(it.id); });
