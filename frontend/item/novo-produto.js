...
function openEditarComposicaoModal(prefill, onSave) {
    ...
    if (inputRepeticao) {
        ...
        if (btnOk) btnOk.addEventListener('click', async function(e){ e.preventDefault();
...
function addQuickNewControl(inputId, storageKey, modalTitle) {
        ...
        item.addEventListener('click', function(e){
            ...
            openQuickNewModal(modalTitle || 'Novo', 'Descrição', function(val){
                ...
                if (storageKey === 'marcas_list' && typeof ApiClient !== 'undefined' && ApiClient.criarMarca) {
                    ApiClient.criarMarca({ nome: valor }).then(() => {
                        input.value = valor;
                        showToast('Criado com sucesso','success',2000);
                        try { if (input.__select_cached && Array.isArray(input.__select_cached)) input.__select_cached.push({ descricao: valor }); } catch(e){}
                        try { window.dispatchEvent(new CustomEvent('marcas:updated', { detail: { nome: valor } })); } catch(e){}
                    }).catch(err => {
                        console.debug('[quick-new marca] falha API, aplicando em memória', err);
                        input.value = valor;
                        showToast('Criado localmente (API indisponível)','warning',3000);
                        try { if (input.__select_cached && Array.isArray(input.__select_cached)) input.__select_cached.push({ descricao: valor }); } catch(e){}
                    });
                } else {
                    try {
                    ...
                    } catch(e) { console.debug('Falha ao salvar localmente', e); }
                    input.value = valor;
                    showToast('Criado com sucesso','success',2000);
                    try { if (input.__select_cached && Array.isArray(input.__select_cached)) input.__select_cached.push({ descricao: valor }); } catch(e){}
                    if (inputId === 'agrupamento') {
                        setTimeout(() => { try { fetchAgrupamentos().then(items => { input.__select_cached = items; }); } catch(e){} }, 100);
                    }
                }
            });
        });
    ...
    btn.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); openMenu(); });
...
function inicializarFormulario() {
    ...
    if (produtoClonar) {
        ...
        try { localStorage.removeItem('produtoClonar'); } catch(e){}
    } else if (produtoId) {
    ...
    try {
        ...
        if (tipoUrl) {
            const norm = String(tipoUrl || '').toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu,'');
            let candidate = null;
            if (norm.indexOf('produto') !== -1) candidate = 'produto';
            else if (norm.indexOf('servico') !== -1 || norm.indexOf('servi') !== -1) candidate = 'servico';
            else if (norm.indexOf('plano') !== -1) candidate = 'plano';
            if (candidate) {
                const btn = document.querySelector(`.btn-toggle[data-value="${candidate}"]`);
                if (btn) {
                    // usar click para disparar handlers já existentes (altera classe e hidden input)
                    setTimeout(()=>{ try{ btn.click(); }catch(e){} }, 10);
                } else {
                    const hidden = document.getElementById('tipo'); if (hidden) hidden.value = candidate;
                }
            }
        }
    } catch(e) { console.debug('aplicar tipo da URL falhou', e); }
...
function configurarDropdownCentroResultado() {
    ...
    function showDropdown(e) {
        ...
        const createList = (items) => {
            ...
            items.forEach(it => {
                ...
                el.addEventListener('click', function(ev){
                    ...
                    try { closeDropdown(); console.log('DEBUG centroResultado: closeDropdown() chamado'); } catch(e) { console.error('DEBUG centroResultado: closeDropdown() erro', e); }
        ...
        fetchCentroResultadosMerged().then(items => { cached = items; createList(items); });
    }
...
function configurarDropdownPerfilComissaoNovo() {
    ...
    function closeDropdown(){ console.log('DEBUG centroResultado: closeDropdown executando'); if (dropdown && dropdown.parentElement) dropdown.parentElement.removeChild(dropdown); dropdown = null; suppressOpen = true; setTimeout(()=>{ suppressOpen = false; }, 120); }
    document.addEventListener('click', function(e){ if (!wrapper.contains(e.target)) closeDropdown(); });
    ...
    try {
        ...
        if (raw) {
            ...
            if (Array.isArray(stored) && stored.length > 0) {
                ...
                if (produto) console.debug('[editar-produto] produto encontrado em localStorage');
            }
        }
    } catch (e) { console.debug('[editar-produto] erro lendo localStorage', e); }

    // Se não encontrado, fallback para produtosExemplo local
    if (!produto) {
        produto = (produtosExemplo || []).find(p => Number(p.id) === Number(produtoId));
    }
...
function preencherFormulario(dados) {
    ...
    if (dados.centroResultado) document.getElementById('centroResultado').value = dados.centroResultado;
    if (dados.categoria) document.getElementById('categoria').value = dados.categoria;
    if (dados.agrupamento) document.getElementById('agrupamento').value = dados.agrupamento;
    if (dados.perfilComissao) document.getElementById('perfilComissao').value = dados.perfilComissao;
    ...
    if (dados.diasOportunidadeVenda) document.getElementById('diasOportunidadeVenda').value = dados.diasOportunidadeVenda;
    if (dados.marca) document.getElementById('marca').value = dados.marca;
    if (dados.codigoBarras) document.getElementById('codigoBarras').value = dados.codigoBarras;
    ...
    try {
        const custoBaseNum = (typeof dados.custoBase === 'number') ? dados.custoBase : (Number(dados.custoBase) || 0);
        const precoNum = (typeof dados.preco === 'number') ? dados.preco : (Number(dados.preco) || 0);
        if (custoBaseNum || precoNum) {
            ...
            const percentualMargem = (custoBaseNum !== 0) ? ((margemReais / custoBaseNum) * 100) : 0;

            document.getElementById('reaisMargem').value = margemReais.toFixed(2);
            document.getElementById('percentualMargem').value = percentualMargem.toFixed(2);
        }
    } catch(e) { console.debug('Erro ao preencher preços/margem:', e); }
    
    if (dados.perfilDesconto) document.getElementById('perfilDesconto').value = dados.perfilDesconto;
    
    // Estoque
    if (dados.empresa) document.getElementById('empresa').value = dados.empresa;
    if (dados.estoqueMinimo) document.getElementById('estoqueMinimo').value = dados.estoqueMinimo;
    if (dados.estoqueIdeal) document.getElementById('estoqueIdeal').value = dados.estoqueIdeal;
    if (dados.localizacao) document.getElementById('localizacao').value = dados.localizacao;
    if (dados.fatorCompra) document.getElementById('fatorCompra').value = dados.fatorCompra;
    
    // Formulários
    if (dados.atendimento) document.getElementById('atendimento').value = dados.atendimento;
    if (dados.observacao) document.getElementById('observacao').value = dados.observacao;
    
    // Validade
    if (dados.perfilValidade) document.getElementById('perfilValidade').value = dados.perfilValidade;
    if (dados.validade) document.getElementById('validade').value = dados.validade;
    ...
    try {
        if (dados.diasOportunidadeVendaServico) document.getElementById('diasOportunidadeVendaServico').value = dados.diasOportunidadeVendaServico;
        if (dados.duracaoHoras !== undefined && document.getElementById('duracaoHoras')) document.getElementById('duracaoHoras').value = dados.duracaoHoras;
        if (dados.duracaoMinutos !== undefined && document.getElementById('duracaoMinutos')) document.getElementById('duracaoMinutos').value = dados.duracaoMinutos;
        if (dados.situacaoTributariaECF) document.getElementById('situacaoTributariaECF').value = dados.situacaoTributariaECF;
        if (dados.impostoISS) document.getElementById('impostoISS').value = dados.impostoISS;
    } catch(e) { console.debug('erro preenchendo campos de serviço (novo-produto)', e); }
...
document.addEventListener('DOMContentLoaded', function(){
    try{
        ...
        if(raw){
            ...
            try{ openEditarNcmPanel(mapped); }catch(e){ console.warn('openEditarNcmPanel failed', e); }
                // armazenar índice original (se houver) para retorno ao modal de import
                try{
                    if(dados._orig_parsed_index !== undefined && dados._orig_parsed_index !== null){
                        window.__origParsedIndex = Number(dados._orig_parsed_index);
                        try{
                            const f = document.getElementById('formEditarProduto') || document.body;
                            const hid = document.createElement('input'); hid.type = 'hidden'; hid.id = '__origParsedIndex'; hid.value = String(window.__origParsedIndex);
                            f.appendChild(hid);
                        }catch(e){ /* noop */ }
                    }
                }catch(e){}
                // remover chave para evitar reaplicação indevida
                try{ sessionStorage.removeItem('novoProdutoFromXML'); }catch(e){}
        }
    }catch(e){ console.warn('Erro lendo novoProdutoFromXML', e); }
});
...
function openEditarNcmPanel(dados){
    ...
    const prev = document.getElementById('editarNcmPanel'); if(prev) prev.remove();
    const overlay = document.createElement('div'); overlay.id = 'editarNcmPanel';
    overlay.style.position = 'fixed'; overlay.style.right = '0'; overlay.style.top = '0'; overlay.style.bottom = '0'; overlay.style.width = '420px'; overlay.style.maxWidth='100%'; overlay.style.background = '#fff'; overlay.style.boxShadow = '0 0 0 1px rgba(0,0,0,0.02), -12px 0 40px rgba(2,6,23,0.12)'; overlay.style.zIndex = '120000'; overlay.style.display='flex'; overlay.style.flexDirection='column';
    // header
    const header = document.createElement('div'); header.style.padding='14px 18px'; header.style.borderBottom='1px solid #eef0f2'; header.style.display='flex'; header.style.alignItems='center'; header.style.justifyContent='space-between';
    const h = document.createElement('h3'); h.textContent = 'Editando NCM'; h.style.margin='0'; h.style.fontSize='16px'; h.style.color='#333'; header.appendChild(h);
    const btnClose = document.createElement('button'); btnClose.className='btn btn-secondary'; btnClose.textContent='Fechar'; btnClose.style.background='#6c757d'; btnClose.style.color='#fff'; btnClose.style.border='none'; btnClose.style.padding='6px 10px'; btnClose.style.borderRadius='6px'; header.appendChild(btnClose);
    overlay.appendChild(header);

    const body = document.createElement('div'); body.style.padding='18px'; body.style.overflow='auto'; body.style.flex='1';
    body.innerHTML = `
    ...
    const footer = document.createElement('div'); footer.style.padding='14px 18px'; footer.style.borderTop='1px solid #eef0f2'; footer.style.display='flex'; footer.style.justifyContent='flex-end'; footer.style.gap='12px';
    const btnSave = document.createElement('button'); btnSave.textContent = 'Salvar NCM'; btnSave.className='btn btn-primary'; btnSave.style.background='#28a745'; btnSave.style.color='#fff'; btnSave.style.border='none'; btnSave.style.padding='10px 14px'; btnSave.style.borderRadius='6px';
    ...
    try{
        const perfilContainer = document.getElementById('editarNcm_perfil_container');
        ...
        function renderPerfilList(filter){
            ...
            const top = document.createElement('div'); top.className='autocomplete-topline'; perfilDropdown.appendChild(top);
            const normalized = (filter||'').toLowerCase();
            const items = perfilOptions.filter(p=>p.toLowerCase().indexOf(normalized) !== -1);
            if(items.length === 0){ const empty = document.createElement('div'); empty.className='dropdown-empty'; empty.textContent='Nenhum resultado'; empty.style.padding='10px 12px'; perfilDropdown.appendChild(empty); return; }
            items.forEach(text=>{
                ...
                it.addEventListener('click', function(e){
                    ...
                    perfilContainer.innerHTML = '<span id="editarNcm_perfil_text" style="color:#222">'+text+'</span><i id="editarNcm_perfil_icon" class="fas fa-caret-down" style="margin-left:8px;color:#888"></i>';
                    perfilValue.value = text;
                    // rebind perfilText
                    try{ perfilText = document.getElementById('editarNcm_perfil_text'); }catch(e){}
                    // fechar dropdown e remover listener de clique fora imediatamente
                    try{ perfilDropdown.style.display = 'none'; }catch(e){}
                    try{ document.removeEventListener('click', outsideClick); }catch(e){}
                    // garantir que o estado principal também seja atualizado
                    try{ const main = document.getElementById('perfilTributacao'); if(main){ if(main.tagName.toLowerCase()==='select'){ for(let i=0;i<main.options.length;i++){ if(main.options[i].text===text){ main.value = main.options[i].value; break; } } } else { main.value = text; } } }catch(e){}
                });
        ...
        perfilContainer.addEventListener('click', function(e){
            ...
            const current = perfilText && perfilText.textContent && perfilText.textContent!=='Selecione um perfil' ? perfilText.textContent : '';
            const input = document.createElement('input'); input.type='text'; input.placeholder = current || 'Pesquisar...'; input.value = ''; input.style.width='100%'; input.style.border='none'; input.style.outline='none'; input.style.background='transparent'; input.style.fontSize='14px';
            perfilContainer.innerHTML = '';
            perfilContainer.appendChild(input);
            perfilContainer.appendChild(document.getElementById('editarNcm_perfil_icon') || document.createElement('i'));
            positionPerfilDropdown(); renderPerfilList('');
            perfilDropdown.style.display = 'block';
            input.focus();

            function onInput(){ renderPerfilList(input.value); positionPerfilDropdown(); }
            ...
            function closeAndRestore(){
                ...
                perfilContainer.innerHTML = '<span id="editarNcm_perfil_text" style="color:'+color+'">'+(val||'Selecione um perfil')+'</span><i id="editarNcm_perfil_icon" class="fas fa-caret-down" style="margin-left:8px;color:#888"></i>';
                // rebind references
                try{ perfilText = document.getElementById('editarNcm_perfil_text'); }catch(e){}
                perfilDropdown.style.display = 'none';
            ...
            function outsideClick(ev){ if(!perfilDropdown.contains(ev.target) && !perfilContainer.contains(ev.target)) { closeAndRestore(); } }
            setTimeout(()=>{ document.addEventListener('click', outsideClick); },50);

            // keyboard: ESC to close
            input.addEventListener('keydown', function(ev){ if(ev.key==='Escape'){ ev.preventDefault(); closeAndRestore(); } if(ev.key==='ArrowDown'){ const f = perfilDropdown.querySelector('.grupo-dropdown-item'); if(f) f.focus(); } });
        });

    }catch(e){ console.debug('Erro configurando dropdown perfil tributacao', e); }
                    ...
                    if(dados._orig_parsed_index !== undefined){
                        ...
                        if(raw){
                            ...
                            if(parsed && Array.isArray(parsed.items) && parsed.items[idx]){
                                ...
                                const fNcm = (candidate.ncm || candidate.NCM || candidate.Ncm || (candidate.prod && (candidate.prod.ncm || candidate.prod.NCM)) || candidate.NCMTrib || candidate.cNCM || '').toString().trim();
                                if(fNcm) {
                                    document.getElementById('editarNcm_input_ncm').value = fNcm;
                                    console.debug('openEditarNcmPanel: preenchido ncm via entradaImported index', idx, fNcm);
                                }
                            }
                        }
                    }
                }catch(e){ console.debug('openEditarNcmPanel: fallback entradaImported read failed', e); }
            }
            if(dados.nome || dados.descricao) document.getElementById('editarNcm_input_desc').value = dados.nome || dados.descricao || ''; 
            // tentar também preencher campo principal se já estiver no DOM
            try{ const mainNcm = document.getElementById('ncm'); if(mainNcm && document.getElementById('editarNcm_input_ncm').value) mainNcm.value = document.getElementById('editarNcm_input_ncm').value; }catch(e){}
        }
    }catch(e){}

    btnClose.addEventListener('click', function(){ try{ overlay.remove(); }catch(e){} });

    btnSave.addEventListener('click', function(){
        try{
            const nVal = (document.getElementById('editarNcm_input_ncm').value || '').trim();
            const dVal = (document.getElementById('editarNcm_input_desc').value || '').trim();
            if(!nVal || !dVal){ alert('Preencha NCM e Descrição'); return; }
            // preencher o formulário principal
            try{ if(document.getElementById('ncm')) document.getElementById('ncm').value = nVal; }catch(e){}
            try{ if(document.getElementById('descricao')) document.getElementById('descricao').value = dVal; }catch(e){}
            // fechar painel
            overlay.remove();
            showToast('NCM salvo no formulário', 'success', 3000);
        }catch(e){ console.error('Salvar NCM error', e); alert('Erro ao salvar NCM'); }
    });
}
...
async function salvarProduto(e) {
    ...
    try {
        ...
        } catch(e) { console.debug('erro coletando composicao (novo-produto)', e); }
        ...
        try {
            const origIdx = (document.getElementById('__origParsedIndex') && document.getElementById('__origParsedIndex').value) || window.__origParsedIndex || null;
            const last = {
                id: produtoSalvo.id,
                nome: produtoSalvo.nome || produtoSalvo.descricao || item.nome || item.descricao || null,
            ...
            try { sessionStorage.setItem('lastCreatedProduct', JSON.stringify(last)); } catch(e) { console.debug('sessionStorage set failed', e); }
        } catch(e) { console.debug('Erro preparando lastCreatedProduct', e); }

        // Verificar se veio do fluxo de importação XML
        const fromXML = new URLSearchParams(window.location.search).get('from') === 'xml';
        
        // Redirecionar: se veio do XML, voltar para entrada-mercadoria com modal aberto
        if (fromXML) {
            setTimeout(() => { 
                try { 
                    const origIdx = (document.getElementById('__origParsedIndex') && document.getElementById('__origParsedIndex').value) || window.__origParsedIndex || '';
                    const q = '?openMapping=1' + (origIdx !== '' ? ('&origIdx=' + encodeURIComponent(origIdx)) : '');
            ...
            setTimeout(() => { window.location.href = './meus-itens.html'; }, 800);
        }
        ...
        } catch(e) { console.debug('Falha ao registrar priceHistory local (novo-produto)', e); }
