...
// ========================================
// FUNCIONALIDADES DA PÁGINA NOVA TRANSFERÊNCIA
// ========================================

let itensTransferencia = [];
let totalTransferenciaValor = 0;

function inicializarNovaTransferencia() {
    console.log('🔄 Inicializando página de Nova Transferência');
    ...
    if (dataEmissaoElement) {
        dataEmissaoElement.textContent = dataFormatada;
    }
    
    // Configurar event listeners
    configurarEventListenersTransferencia();
    
    // Renderizar tabela inicial (vazia)
    renderizarItensTransferencia();
    
    console.log('✅ Página de Nova Transferência inicializada');
}

function configurarEventListenersTransferencia() {
    ...
    if (btnAdicionarItem) {
        btnAdicionarItem.addEventListener('click', adicionarItemTransferencia);
    }
    
    if (btnFinalizar) {
        btnFinalizar.addEventListener('click', finalizarTransferencia);
    }
    
    if (btnNova) {
        btnNova.addEventListener('click', novaTransferencia);
    }
...
function adicionarItemTransferencia() {
    ...
    const validadeInput = document.getElementById('validadeItem');
    ...
    itensTransferencia.push(novoItem);
    totalTransferenciaValor += custo * quantidade;
    
    // Atualizar interface
    renderizarItensTransferencia();
    atualizarTotalTransferencia();
...
function removerItemTransferencia(itemId) {
    const itemIndex = itensTransferencia.findIndex(item => item.id === itemId);
    
    if (itemIndex !== -1) {
        const item = itensTransferencia[itemIndex];
        totalTransferenciaValor -= item.custo * item.quantidade;
        itensTransferencia.splice(itemIndex, 1);
...
function renderizarItensTransferencia() {
    const tbody = document.getElementById('itensTransferenciaBody');
    ...
    itensTransferencia.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <td>${item.quantidade.toFixed(3).replace('.', ',')}</td>
            <td>
                <button class="btn-remove-item-transfer" onclick="removerItemTransferencia(${item.id})">
...
function atualizarTotalTransferencia() {
    const totalElement = document.getElementById('totalTransferencia');
    if (totalElement) {
        totalElement.textContent = totalTransferenciaValor.toFixed(2).replace('.', ',');
    }
}

function finalizarTransferencia() {
    const empresaDestino = document.getElementById('empresaDestino');
    
    if (!empresaDestino || !empresaDestino.value.trim()) {
        alert('Por favor, selecione uma Empresa Destino!');
        return;
    }
...
function novaTransferencia() {
    if (confirm('Limpar todos os dados e criar nova transferência?')) {
        // Limpar campos
        document.getElementById('empresaDestino').value = '';
        document.getElementById('observacoes').value = '';
        
        // Limpar itens
        itensTransferencia = [];
        totalTransferenciaValor = 0;
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', inicializarNovaTransferencia);
} else {
    inicializarNovaTransferencia();
}
