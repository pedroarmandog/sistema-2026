...
function renderizarTabela() {
    ...
    dadosPaginados.forEach(entrada => {
        ...
        let displayId = rawId;
        if (/^sim-\d+$/.test(rawId)) displayId = rawId.replace(/^sim-/, '').slice(-6);
...
async function saveDraftEntrada(desiredSituacao){
    ...
    try{
        if(!draftEntradaId){
            ...
            console.log('[outras-entradas] saveDraftEntrada payload (create):', payload);
            const res = await fetch('/api/entrada/manual', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
            if(res.ok){
            ...
            console.log('[outras-entradas] saveDraftEntrada payload (update):', payload, 'draftId=', draftEntradaId);
            const res = await fetch('/api/entrada/manual/' + encodeURIComponent(draftEntradaId), { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
            if(res.ok){
...
async function loadEntradasFromServer(){
    try{
        ...
        console.log('[outras-entradas] GET /api/entrada/manual status=', res.status);
        if(res.ok){
            const data = await res.json();
                console.log('[outras-entradas] resposta raw:', data);
                try{
                    if(Array.isArray(data) && data.length>0){
                        console.log('[outras-entradas] amostra raw[0]:', JSON.stringify(data[0], null, 2));
                ...
                console.log('[outras-entradas] carregadas entradas do servidor (normalizadas):', entradasData.length);
                ...
                try{
                    ...
                    if(hasFinalizadas && currentStatus === 'pendentes'){
                        console.log('[outras-entradas] entradas finalizadas detectadas — alternando filtro para todas');
                ...
                console.warn('[outras-entradas] GET /api/entrada/manual retornou não-array');
            }
        } else {
            const txt = await res.text().catch(()=>null);
...
async function finalizarEntrada() {
    ...
    if (itensAdicionados.length === 0 && draftEntradaId) {
        try{
            const res = await fetch('/api/entrada/manual/' + encodeURIComponent(draftEntradaId));
            if(res.ok){
            ...
            try{
                ...
                if(window.ApiClient && Array.isArray(itensParaAtualizar) && itensParaAtualizar.length>0){
                    ...
                    if(failed.length > 0){
                        ...
                        try{ showToast('Entrada finalizada, porém falha ao atualizar estoque de alguns produtos', 'warning'); }catch(e){}
                    } else {
