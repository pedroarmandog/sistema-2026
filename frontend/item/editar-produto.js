...
function openEditarComposicaoModal(prefill, onSave) {
  ...
  document.addEventListener("click", function (ev) {
    ...
    const clickedInsideItemInput =
      ev.target === inputItem || inputItem.contains(ev.target);
    const clickedInsideItemSuggestions = suggestions.contains(ev.target);
    const clickedInsideRepInput =
      inputRepeticao &&
      (ev.target === inputRepeticao || inputRepeticao.contains(ev.target));
    const clickedInsideRepSuggestions = repeticaoSuggestions.contains(
      ev.target,
    );
  ...
  btnOk.addEventListener("click", function (e) {
    e.preventDefault();
    const item = (modal.querySelector("#composicaoItemInput").value || "")
      .toString()
      .trim();
    const repeticao = (
      modal.querySelector("#composicaoRepeticaoInput").value || ""
    ...
    try {
      ...
      if (container) {
        ...
        if (selectedId)
          try {
            tr.setAttribute("data-selected-id", selectedId);
          } catch (e) {}
...
function configurarDropdownCentroResultado() {
  ...
  function showDropdown(e) {
    ...
    const createList = (items) => {
      ...
      items.forEach((it) => {
        ...
        el.addEventListener("click", function (ev) {
          console.log(
            "DEBUG centroResultado(editar): item clicado ->",
            it.descricao,
          );
          input.value = it.descricao;
          ...
          try {
            closeDropdown();
            console.log(
              "DEBUG centroResultado(editar): closeDropdown() chamado",
            );
          } catch (e) {
            console.error(
              "DEBUG centroResultado(editar): closeDropdown() erro",
              e,
            );
          }
          try {
    ...
    fetchCentroResultados()
      .then((items) => {
        cached = items;
        createList(items);
      })
      .catch((err) => {
        console.error("Erro ao carregar centros (editar-produto):", err);
        createList([]);
      });
  }
...
function configurarDropdownPerfilComissaoNovo() {
  ...
  window.addEventListener("perfis_validade:updated", function () {
...
function preencherFormulario(dados) {
  ...
  try {
    ...
    if (dados.perfilComissao) {
      setVal("perfilComissao", dados.perfilComissao);
      setTimeout(() => mostrarPainelComissaoProduto(dados.perfilComissao), 0);
    }
    ...
    if (dados.diasOportunidadeVenda)
      setVal("diasOportunidadeVenda", dados.diasOportunidadeVenda);
    ...
    if (dados.codigoBarras) {
      ...
      try {
        ...
        const useVal =
          Array.isArray(dados.codigosBarras) && dados.codigosBarras.length > 0
            ? dados.codigosBarras[0]
            : dados.codigoBarras;
        if (firstBarcodeInput) {
          firstBarcodeInput.value =
            useVal === undefined || useVal === null ? "" : useVal.toString();
          // Se houver múltiplos códigos, adicionar linhas extras
          try {
            if (
              Array.isArray(dados.codigosBarras) &&
              dados.codigosBarras.length > 1
            ) {
              const extras = dados.codigosBarras.slice(1);
              extras.forEach(function (v) {
                try {
                  if (typeof window.addBarcodeField === "function")
                    window.addBarcodeField(v);
                } catch (e) {}
              });
            }
          } catch (e) {}
        ...
        try {
          setVal("codigoBarras", dados.codigoBarras);
        } catch (_) {}
      }
    }
    ...
    if (dados.custoBase !== undefined && dados.custoBase !== null) {
      ...
      const percentualMargem =
        custoBase > 0 ? (margemReais / custoBase) * 100 : 0;
    ...
    try {
      ...
      if (dados.duracaoHoras !== undefined) {
        const el = getEl("duracaoHoras");
        if (el) el.value = dados.duracaoHoras;
      }
      if (dados.duracaoMinutos !== undefined) {
        const el = getEl("duracaoMinutos");
        if (el) el.value = dados.duracaoMinutos;
      }
      if (dados.situacaoTributariaECF)
        setVal("situacaoTributariaECF", dados.situacaoTributariaECF);
      if (dados.impostoISS) setVal("impostoISS", dados.impostoISS);
    } catch (e) {
      console.debug("erro preenchendo campos de serviço (editar-produto)", e);
    }
    ...
    try {
        ...
        if (container) {
          ...
          dados.composicao.forEach((c) => {
            try {
              ...
              const qtdLabel =
                Number(c.quantidade) === 1
                  ? "1 vez ao mês"
                  : (Number(c.quantidade) || 0) + " vezes ao mês";
              const tr = document.createElement("tr");
              ...
              if (c.selectedId)
                try {
                  tr.setAttribute("data-selected-id", c.selectedId);
                } catch (e) {}
              tbody.appendChild(tr);
              const btnRem = tr.querySelector(".btn-remove-composicao");
              if (btnRem) {
      ...
      console.debug("erro reconstruindo composicao", e);
    }
...
async function salvarProduto(e) {
  ...
  try {
    ...
    try {
      ...
      console.debug("erro coletando composicao", e);
    }
    ...
    if (wasCreate) {
      ...
      console.log(
        "📤 Enviando atualização produto (id:",
        item.id,
        "payload):",
        item,
      );
    ...
    if (!window.modoClonar && produtoAtual && produtoAtual.id) {
      try {
        const prev =
          produtoAtual && produtoAtual.permiteEstoqueNegativo
            ? produtoAtual.permiteEstoqueNegativo.toString()
            : "nao";
        const now = (item.permiteEstoqueNegativo || "nao").toString();
        if (prev !== now) {
          const valorGlobal = now;
          const resp =
            await ApiClient.updatePermiteEstoqueNegativoGlobal(valorGlobal);
          if (resp && typeof resp.updated !== "undefined") {
            showToast(
              `Regra aplicada para ${resp.updated} produtos`,
              "info",
              1800,
            );
          } else {
            showToast(
              "Regra de estoque negativo aplicada para todos os produtos",
              "info",
              1600,
            );
          }
        ...
        console.warn(
          "Falha ao aplicar regra global de estoque negativo:",
          errGlobal,
        );
        // continuar mesmo se falhar; o produto já foi salvo
        showToast(
          "Produto salvo (falha ao aplicar regra global)",
          "warning",
          2200,
        );
      }
    ...
    try {
      ...
      const exists = arr.find(
        (p) =>
          p.date === entry.date &&
          Number(p.sale) === Number(entry.sale) &&
          Number(p.cost) === Number(entry.cost),
      );
      if (!exists) {
        arr.push(entry);
      ...
      console.debug("Falha ao registrar priceHistory local", e);
    }
  } catch (err) {
  ...
  function insertRowFromData(data) {
    try {
      ...
      if (data && data.selectedId)
        try {
          tr.setAttribute("data-selected-id", data.selectedId);
        } catch (e) {}
      tbody.appendChild(tr);
      // animação suave
      try {
