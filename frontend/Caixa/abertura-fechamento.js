...
class CaixaManager {
    ...
    async loadUsuarioLogado() {
        try {
            ...
            if (usuarioId && usuarioNome) {
                ...
                this.showNotification('Usuário não identificado. Faça login novamente.', 'warning');
            }
        } catch (error) {
    ...
    initializeEventListeners() {
        ...
        if (terminalOptionsBtn && terminalDropdown) {
            ...
            const terminalOptions = terminalDropdown.querySelectorAll('.terminal-option');
    ...
    async handleAbrirCaixa() {
        ...
        try {
            ...
            this.showNotification(error.message || 'Erro ao abrir caixa. Tente novamente.', 'error');
        }
    }
    ...
    async handleFecharCaixa() {
        ...
        if (!senhaInput.value) {
            this.showNotification('Por favor, digite sua senha para fechar o caixa', 'error');
            return;
        }
        ...
        try {
            ...
            const response = await fetch(`http://localhost:3000/api/caixas/${this.caixaAtual.id}/fechar`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            this.showNotification(error.message || 'Erro ao fechar caixa. Tente novamente.', 'error');
        }
    }
    ...
    async validarSenhaUsuario(senha) {
        try {
            ...
            const response = await fetch('http://localhost:3000/api/usuarios/validar-senha', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    updateCaixaStatus(aberto, caixa = null) {
        ...
        if (aberto && caixa) {
            ...
            if (acaoCaixaBtn) {
                acaoCaixaBtn.innerHTML = '<i class="fas fa-lock-open"></i> Abrir o Caixa';
    ...
    async atualizarFundoTroco(valorDinheiro) {
        ...
        console.log(`💰 Fundo de troco atualizado: ${formatarMoeda(this.fundoTroco)}`);
    }
}
