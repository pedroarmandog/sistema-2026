...
class DemonstrativoCaixa {
    constructor() {
        ...
        // Detectar se estamos sendo servidos por Live Server (porta 5500) ou por outro host
        // Se não estivermos na porta 3000, usar o backend em http://localhost:3000
        this.apiBase = (location.port && location.port !== '3000') ? 'http://localhost:3000' : '';
    ...
    zerarTotais() {
        ...
        if (cardDinheiro) {
            ...
            const items = cardDinheiro.querySelectorAll('.total-item span:last-child');
            if (items.length >= 11) {
        ...
        if (cardCartao) {
            ...
            if (items.length >= 4) for (let i = 0; i < 4; i++) items[i].textContent = '0.00';
            const footer = cardCartao.querySelector('.total-footer'); if (footer) footer.innerHTML = '';
        }
        ...
        if (cardCheque) {
            ...
            if (items.length >= 7) for (let i = 0; i < 7; i++) items[i].textContent = '0.00';
            const footer = cardCheque.querySelector('.total-footer'); if (footer) footer.innerHTML = '';
        }
        ...
        if (cardCrediario) {
            ...
            const items = cardCrediario.querySelectorAll('.total-item span:last-child');
            if (items.length >= 2) for (let i = 0; i < 2; i++) items[i].textContent = '0.00';
            const footer = cardCrediario.querySelector('.total-footer'); if (footer) footer.innerHTML = '';
        }
        ...
        if (cardPix) {
            ...
            if (items.length >= 4) for (let i = 0; i < 4; i++) items[i].textContent = '0.00';
            const footer = cardPix.querySelector('.total-footer'); if (footer) footer.innerHTML = '';
        }

        // Remover gráfico se existir
        if (this._chart0) {
            try { this._chart0.destroy(); } catch (e) {}
            this._chart0 = null;
        }
    ...
    setupEventListeners() {
        ...
        if (btnAbrirPesquisa) {
            btnAbrirPesquisa.addEventListener('click', () => this.abrirModalPesquisa());
        }

        // Botão para fechar modal
        const btnFecharPesquisa = document.getElementById('btnFecharPesquisa');
        if (btnFecharPesquisa) {
            btnFecharPesquisa.addEventListener('click', () => this.fecharModalPesquisa());
        }
        ...
        if (btnPesquisarPeriodo) {
            btnPesquisarPeriodo.addEventListener('click', () => this.abrirCalendarios());
        }

        // Navegação dos calendários
        document.getElementById('btnMesAnterior1')?.addEventListener('click', () => this.navegarMes(1, -1));
        document.getElementById('btnMesSeguinte1')?.addEventListener('click', () => this.navegarMes(1, 1));
        document.getElementById('btnMesAnterior2')?.addEventListener('click', () => this.navegarMes(2, -1));
        document.getElementById('btnMesSeguinte2')?.addEventListener('click', () => this.navegarMes(2, 1));

        // Botão confirmar período
        const btnConfirmarPeriodo = document.getElementById('btnConfirmarPeriodo');
        if (btnConfirmarPeriodo) {
            btnConfirmarPeriodo.addEventListener('click', () => this.confirmarPeriodo());
        }
    }
    ...
    async abrirModalPesquisa() {
        ...
        if (!numeroCaixa) {
            this.mostrarNotificacao('Por favor, informe o número do caixa', 'warning');
            return;
        }
        ...
        try {
            ...
            const numeroFormatado = numeroCaixa.includes('Caixa') ? numeroCaixa : `Caixa ${numeroCaixa}`;
            console.log('Buscando caixa:', numeroFormatado);
            
            const response = await fetch(`${this.apiBase}/api/caixas/numero/${encodeURIComponent(numeroFormatado)}`);
            ...
            if (data.caixa) {
                ...
                document.getElementById('dataAbertura').value = this.formatarDataHora(data.caixa.dataAbertura);
                document.getElementById('dataFechamento').value = data.caixa.dataFechamento 
    ...
    abrirCalendarios() {
        const calendariosContainer = document.getElementById('calendariosContainer');
    ...
    renderizarCalendario(numCalendario) {
        ...
        for (let i = 0; i < diaSemanaInicio; i++) {
            const mesAnterior = new Date(mesAtual.getFullYear(), mesAtual.getMonth(), 0);
        ...
        for (let dia = 1; dia <= diasNoMes; dia++) {
            ...
            if (this.dataSelecionadaInicio && dataAtual.toDateString() === this.dataSelecionadaInicio.toDateString()) {
                diaEl.classList.add('dia-selecionado');
            }
            if (this.dataSelecionadaFim && dataAtual.toDateString() === this.dataSelecionadaFim.toDateString()) {
                diaEl.classList.add('dia-selecionado');
            }

            // Marcar range entre datas
            if (this.dataSelecionadaInicio && this.dataSelecionadaFim) {
                if (dataAtual > this.dataSelecionadaInicio && dataAtual < this.dataSelecionadaFim) {
    ...
    selecionarData(data) {
        if (!this.dataSelecionadaInicio || (this.dataSelecionadaInicio && this.dataSelecionadaFim)) {
        ...
        try {
            ...
            if (!numeroCaixa) {
                ...
                if (numeroInput) {
                    ...
                    try {
                        const respCaixa = await fetch(`${this.apiBase}/api/caixas/numero/${encodeURIComponent(numeroInput)}`);
                        if (respCaixa.ok) {
                            const caixaData = await respCaixa.json();
                            const caixaObj = caixaData.caixa ? caixaData.caixa : (caixaData.numero ? caixaData : null);
            ...
            console.log('buscarDadosPeriodo -> caixaId:', numeroCaixa, 'dataInicio:', this.formatarDataISO(this.dataInicio), 'dataFim:', this.formatarDataISO(this.dataFim));
            const dataInicio = this.formatarDataISO(this.dataInicio);
            const dataFim = this.formatarDataISO(this.dataFim);

            // Buscar vendas do período
            const url = `${this.apiBase}/api/vendas?dataInicio=${dataInicio}&dataFim=${dataFim}` + (numeroCaixa ? `&caixaId=${numeroCaixa}` : '');
            ...
            if (data.vendas) {
                ...
                this.mostrarNotificacao('Nenhuma venda encontrada para o período', 'info');
            }
        } catch (error) {
        ...
        vendas.forEach(venda => {
            ...
            if (venda.pagamentos && Array.isArray(venda.pagamentos)) {
                venda.pagamentos.forEach(pagamento => {
                    ...
                    const forma = (pagamento.formaPagamento || pagamento.forma || pagamento.forma_pagamento || pagamento.tipo || '').toString().toLowerCase();
                    switch (forma) {
                        case 'dinheiro':
                        ...
                        case 'cartao':
                            ...
                            const operadora = pagamento.operadoraCartao || pagamento.operadora || 'Outros';
                            totais.cartao.operadoras[operadora] = (totais.cartao.operadoras[operadora] || 0) + valorPagamento;
                            break;
                        case 'cheque':
                        ...
                        case 'pix':
                            ...
                            const conta = pagamento.conta || pagamento.contaDestino || pagamento.chavePix || pagamento.chave || 'CAIXA COFRE';
                            totais.pix.contas[conta] = (totais.pix.contas[conta] || 0) + valorPagamento;
                            break;
                    }
                });
            }
    ...
    atualizarTotais(totais) {
        ...
        const totalDinheiro = toNum(totais.dinheiro.trocoInicial) + 
        ...
        const valores = {
            ...
            cartao: parseFloat((toNum(totais.cartao.vendas) + toNum(totais.cartao.atendimentos) + toNum(totais.cartao.recebimentos) + toNum(totais.cartao.adiantamentos)).toFixed(2)) || 0,
            pix: parseFloat((toNum(totais.pix.vendas) + toNum(totais.pix.atendimentos) + toNum(totais.pix.recebimentos) + toNum(totais.pix.adiantamentos)).toFixed(2)) || 0,
            crediario: parseFloat((toNum(totais.crediario.vendas) + toNum(totais.crediario.atendimentos)).toFixed(2)) || 0,
            cheque: parseFloat((toNum(totais.cheque.vendas) + toNum(totais.cheque.atendimentos) + toNum(totais.cheque.recebimentos)).toFixed(2)) || 0
        };

        // Atualizar card dinheiro
        const cardDinheiro = document.querySelector('.total-dinheiro');
        if (cardDinheiro) {
            cardDinheiro.querySelector('.total-value').textContent = `R$ ${totalDinheiro.toFixed(2)}`;
        ...
        } catch (e) { console.warn('Erro ao atualizar gráfico:', e); }
    }
    ...
    async atualizarGrafico(valores) {
        ...
        try {
            ...
            const data = [valores.dinheiro, valores.cartao, valores.pix, valores.crediario, valores.cheque];
                ...
                this._chart0 = new Chart(ctx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            data,
                            backgroundColor: ['#27ae60', '#2980b9', '#1abc9c', '#e74c3c', '#f39c12']
                        }]
                    },
                    options: {
                        responsive: true,
    ...
    atualizarCardCartao(totais) {
        const total = totais.vendas + totais.atendimentos + totais.recebimentos + totais.adiantamentos;
    ...
    atualizarCardCheque(totais) {
        const total = totais.vendas + totais.atendimentos + totais.recebimentos + totais.adiantamentos - 
                     totais.devolucoes - totais.pagamentos - totais.movimentoSaida;
    ...
    atualizarCardPix(totais, valores) {
        ...
        if (card) {
            ...
            if (footer) {
                ...
                let totalAgregado = 0;
                if (valores) {
                    totalAgregado = (Number(valores.dinheiro) || 0) + (Number(valores.cartao) || 0) + (Number(valores.pix) || 0) + (Number(valores.crediario) || 0) + (Number(valores.cheque) || 0);
                } else {
                    // fallback: usar soma apenas do PIX
                    totalAgregado = contasEntries.reduce((s, [, v]) => s + (Number(v) || 0), 0);
                }
