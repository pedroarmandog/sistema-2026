...
class MovimentoCaixaManager {
    ...
    setupPagination() {
        ...
        if (nextBtn) {
            nextBtn.addEventListener('click', () => {
                const totalPages = Math.ceil(this.movimentos.length / this.itemsPerPage);
    ...
    async loadMovimentos() {
        // Carregar movimentos do backend via API
        try {
            const response = await fetch('http://localhost:3000/api/movimentos-caixa');
            if (response.ok) {
                const movimentosAPI = await response.json();
                if (Array.isArray(movimentosAPI)) {
                    this.movimentos = movimentosAPI;
                    console.log('✅ Movimentos carregados do banco:', this.movimentos.length);
                } else {
                    this.movimentos = [];
                    console.warn('⚠️ Nenhum movimento encontrado no banco');
                }
            } else {
    ...
    renderTable() {
        ...
        tbody.innerHTML = pageMovimentos.map(movimento => {
            ...
            <tr>
                <td>${movimento.id}</td>
                <td><span class="movimento-tipo ${movimento.tipo}">${movimento.tipo.toUpperCase()}</span></td>
                <td>${movimento.observacao || '-'}</td>
                <td class="${movimento.tipo === 'entrada' ? 'valor-positivo' : 'valor-negativo'}">
    ...
    async adicionarMovimento() {
        ...
        try {
            ...
            if (!caixaAberto || !caixaAberto.aberto) {
                this.showNotification('Nenhum caixa aberto. Abra um caixa antes de adicionar movimentos.', 'warning');
                return;
            }
        } catch (error) {
        ...
        try {
            ...
            const tipoText = tipoSelect.value === 'entrada' ? 'Suprimento' : 'Sangria';
            this.showNotification(`${tipoText} de R$ ${valor.toFixed(2).replace('.', ',')} adicionado com sucesso!`, 'success');
        } catch (error) {
            this.showNotification('Erro ao salvar movimento: ' + error.message, 'error');
        }
    }
