    ...
    static async request(endpoint, options = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                let errorBody = {};
                try { errorBody = await response.json(); } catch(e) { /* não-JSON */ }

                // compor mensagem de erro a partir dos campos mais comuns
                const serverMsg = errorBody && (errorBody.message || errorBody.error || errorBody.details || (typeof errorBody === 'string' ? errorBody : null));
                const composed = serverMsg ? `${serverMsg} (HTTP ${response.status})` : `Erro HTTP: ${response.status}`;
...
window.closeAllSelectDropdowns = function(exceptAncestor) {
    try {
        const els = Array.from(document.querySelectorAll('.select-dropdown, .multi-select-dropdown'));
        els.forEach(el => {
            try {
                if (exceptAncestor && el && exceptAncestor.contains(el)) return; // preserve dropdowns inside exceptAncestor
                if (el && el.parentElement) el.parentElement.removeChild(el);
            } catch (e) { /* ignore per-dropdown errors */ }
        });
    } catch (e) { /* ignore */ }
};
...
document.addEventListener('mousedown', function captureCloseBeforeOpen(e) {
    try {
        const wrapper = e.target && e.target.closest ? e.target.closest('.select-wrapper') : null;
        if (wrapper) {
