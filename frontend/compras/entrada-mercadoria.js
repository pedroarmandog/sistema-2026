...
function salvarEstadoSubmenu(submenuId, isOpen) {
    try {
        const estadoSubmenus = JSON.parse(sessionStorage.getItem('estadoSubmenus') || '{}');
...
function obterEstadoSubmenu(submenuId) {
    try { return JSON.parse(sessionStorage.getItem('estadoSubmenus') || '{}')[submenuId] || false; } catch (e) { return false; }
}
...
function limparEstadoSubmenus() { try { sessionStorage.removeItem('estadoSubmenus'); } catch(e){} }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!shouldOpenMapping) {
        console.log('ℹ️ openMapping não detectado - verificando entradaImported normal');
        try{
            ...
            if(raw){
                try{
                    ...
                    try{
                        findCandidatesForItems(parsed.items || []).then(cands => {
                            try{ showProductMappingModal(parsed, cands || []); } catch(e){ console.error('Erro abrindo modal de mapeamento (auto):', e); }
                        }).catch(e => {
                            try{ showProductMappingModal(parsed, []); } catch(e){ console.error('Erro abrindo modal de mapeamento (fallback):', e); }
                        });
                    }catch(e){ try{ showProductMappingModal(parsed, []); }catch(e){} }
                }catch(e){ console.debug('entradaImported parse failed', e); }
            }
        }catch(e){}
    }
});
...
function obterNomeFornecedor(fornecedor) {
    try {
        ...
        if (typeof fornecedor === 'object') {
            return fornecedor.nome || fornecedor.xNome || fornecedor.razaoSocial || fornecedor.fantasia || fornecedor.nomeFantasia || fornecedor.displayName || fornecedor.companyName || fornecedor.nomeFornecedor || JSON.stringify(fornecedor).replace(/[{}\"]+/g,'').split(',')[0] || '-';
        }
    } catch (e) { /* fallback */ }
    return '-';
}
...
async function inicializarEntradaMercadoria() {
    ...
    console.log('📊 entradasData após carregar entradas:', entradasData.length, 'entradas');
    ...
    try {
        ...
        if (rawImported) {
            try {
                ...
                const entradaTemp = {
                    id: tempId,
                    fornecedor: obterNomeFornecedor(imported.fornecedor) || (imported.fornecedorNome) || '-',
                    numero: imported.numero || imported.serie || '-',
                    emissao: imported.dataEmissao ? formatarData(imported.dataEmissao) : (imported.dataEmissao || '-'),
                    valor: imported.totals && imported.totals.vProd ? Number(imported.totals.vProd) : (imported.valorTotal || 0),
                    situacao: 'pendente',
                    relProdutos: (imported.items && imported.items.length > 0) ? '✓' : '-',
                    _importedData: imported
                };
                // evitar duplicação
                const exists = entradasData.find(e => e.id === entradaTemp.id || (e.numero && e.numero === entradaTemp.numero && e.fornecedor === entradaTemp.fornecedor));
                if (!exists) {
                    entradasData.unshift(entradaTemp);
                    filteredData = [...entradasData];
                    console.log('📥 Entrada importada (temporária) adicionada à lista:', entradaTemp);
                } else {
                    console.log('⚠️ Entrada importada já presente na lista, ignorando');
                }
            } catch (e) { console.debug('entradaImported parse failed', e); }
        }
    } catch (e) { console.debug('Erro ao verificar entradaImported na sessão', e); }
    ...
    try {
        ...
        if (rawSaved) {
            try {
                ...
                const entradaTemp = {
                    id: tempId,
                    fornecedor: obterNomeFornecedor(saved.fornecedor) || (saved.fornecedorNome || '-') ,
                    numero: saved.numero || saved.serie || '-',
                    emissao: saved.dataEmissao ? formatarData(saved.dataEmissao) : (saved.dataEmissao || '-'),
                ...
                const exists = entradasData.find(e => String(e.id) === String(entradaTemp.id) || (e.numero && e.numero === entradaTemp.numero && e.fornecedor === entradaTemp.fornecedor));
                if (!exists) {
                    // inserir no topo
                    entradasData.unshift(entradaTemp);
                    filteredData = [...entradasData];
                    console.log('📥 Entrada finalizada (injetada) adicionada à lista:', entradaTemp);
                } else {
                    // se existe, atualizar o registro existente
                    entradasData = entradasData.map(e => (String(e.id) === String(entradaTemp.id) ? entradaTemp : e));
                    filteredData = [...entradasData];
                    console.log('♻️ Entrada finalizada já presente — atualizada na lista');
                }
            } catch (e) { console.debug('injectedSavedEntrada parse failed', e); }
            try { sessionStorage.removeItem('injectedSavedEntrada'); } catch(e){}
        }
    } catch(e) { console.debug('Erro ao aplicar injectedSavedEntrada', e); }
...
window.acessarNotaConsultada = async function(entradaId, isTemp) {
    console.log('📋 Acessando nota consultada:', entradaId, 'temporária:', isTemp);
    ...
    if (isTemp) {
        ...
        if (entrada && entrada._parsed) {
            ...
            alert('Entrada temporária não encontrada. Tente importar o XML novamente.');
        }
    } else {
        // Se já existe no banco, abrir entrada-manual para edição
...
async function criarRascunhoNoServidor(parsed) {
    try {
        const payload = Object.assign({}, parsed, { rascunho: true, situacao: 'pendente' });
        ...
        try {
            if (payload.fornecedor && typeof payload.fornecedor === 'object') {
                payload.fornecedor = payload.fornecedor.nome || payload.fornecedor.xNome || payload.fornecedor.razaoSocial || payload.fornecedor.fantasia || payload.fornecedor.nomeFantasia || payload.fornecedor.displayName || payload.fornecedor.companyName || JSON.stringify(payload.fornecedor);
            }
        } catch (e) { /* noop */ }
        ...
        if (!res.ok) {
            console.warn('criarRascunhoNoServidor: resposta não ok', res.status, res.statusText);
            return null;
        }
    ...
    try {
        ...
        try {
            if (!Array.isArray(entradas)) {
                ...
                else if (entradas && Array.isArray(entradas.data)) entradas = entradas.data;
        ...
        console.log(`📦 ${entradas.length} entrada(s) encontrada(s) no banco`, entradas);

        // Adicionar entradas à lista
        entradas.forEach(entrada => {
                const entradaFormatada = {
                    id: entrada.id,
                    fornecedor: obterNomeFornecedor(entrada.fornecedor) || (entrada.fornecedorNome || '-'),
...
function configurarEventListenersEntrada() {
    ...
    if (btnImportarXML) {
        btnImportarXML.addEventListener('click', () => {
            try {
                    ...
                    fileInput.addEventListener('change', async function() {
                        ...
                        try {
                            ...
                            if (parsed && parsed.items && parsed.items.length > 0) {
                                ...
                                try {
                                    ...
                                    if (checkRes.ok) {
                                        ...
                                        const duplicada = entradas.find(e => {
                                            const numMatch = e.numero && numero && String(e.numero).trim() === String(numero).trim();
                                            const chaveMatch = e.chaveAcesso && chave && String(e.chaveAcesso).trim() === String(chave).trim();
                                            return numMatch || (chave && chaveMatch);
                                        });
                                        
                                        if (duplicada) {
                                            showEntradaToast(`⚠️ Esta nota fiscal (${numero}) já foi importada! Abrindo entrada existente...`, 5000);
                                            setTimeout(() => {
                                                window.location.href = './entrada-manual.html?id=' + encodeURIComponent(duplicada.id);
                                            }, 1500);
                                            return;
                                        }
                                    }
                                ...
                                try{
                                    const candidates = await findCandidatesForItems(parsed.items);
                                    const anyMissing = candidates.some(arr => !arr || arr.length === 0);
                                    if(anyMissing){
                                        ...
                                        for(let i=0;i<parsed.items.length;i++){
                                            ...
                                            if(cand){ parsed.items[i].matched = cand; parsed.items[i].matchedId = cand.id; parsed.items[i].unitario = parsed.items[i].unitario || Number(cand.preco) || 0; }
                                        }
                                        ...
                                        try {
                                            ...
                                            if (created && created.id) {
                                                window.location.href = './entrada-manual.html?id=' + encodeURIComponent(created.id);
                                                return;
                                            } else {
                                                showEntradaToast('Erro ao salvar rascunho no servidor. Tente novamente.');
                                                return;
                                            }
                                        } catch (e) { console.warn('criarRascunhoNoServidor falhou', e); }
                                        // fallback para sessionStorage se servidor não responder
                                        try{ sessionStorage.setItem('entradaImported', JSON.stringify(parsed)); }catch(e){ console.warn('sessionStorage set failed', e); }
                                        window.location.href = './entrada-manual.html';
                                        return;
                                    }
                                }catch(err){ console.warn('findCandidates failed', err); }
                            }
                        } catch (err) {
                            console.warn('Parsing client-side falhou, tentando servidor', err);
                        }
                        ...
                        try {
                            ...
                            const body = await res.json().catch(()=>null);
                            if (!res.ok) {
                                showEntradaToast('Erro ao importar XML: ' + (body && body.error ? body.error : res.statusText));
                                return;
                            }
                            ...
                            if (body && Array.isArray(body.items) && body.items.length > 0) {
                                ...
                                } catch (e) { console.warn('criarRascunhoNoServidor falhou (fallback)', e); }
                                showEntradaToast('XML importado, mas não foi possível salvar no servidor. Verifique o servidor e tente novamente.');
                                return;
                                return;
                            }
                            // servidor não retornou items - avisar e mostrar mensagem retornada
                            const msg = (body && (body.message || body.ok && JSON.stringify(body))) || 'XML importado, mas nenhum item foi extraído pelo servidor.';
                            showEntradaToast(msg);
                        } catch (err) {
                            console.error('import xml error', err);
                            showEntradaToast('Erro ao enviar XML: ' + (err && err.message ? err.message : String(err)));
                        }
                ...
                fileInput.click();
            } catch (err) { console.error('btnImportarXML handler', err); alert('Erro interno'); }
        });
    }
    ...
    function showEntradaToast(message){
        try{
            ...
            if(!c){ c = document.createElement('div'); c.className = 'entrada-toast-container'; c.style.position='fixed'; c.style.top='18px'; c.style.right='18px'; c.style.zIndex='99999'; document.body.appendChild(c); }
            const t = document.createElement('div'); t.className='entrada-toast'; t.textContent = message; t.style.background='#ffe58a'; t.style.color='#222'; t.style.padding='10px 14px'; t.style.borderRadius='8px'; t.style.boxShadow='0 8px 24px rgba(0,0,0,0.12)'; t.style.marginTop='8px'; t.style.opacity='0'; t.style.transition='opacity .18s,transform .18s'; c.appendChild(t);
            requestAnimationFrame(()=>{ t.style.opacity='1'; t.style.transform='none'; });
            setTimeout(()=>{ t.style.opacity='0'; setTimeout(()=>{ try{ t.remove(); }catch(e){} },220); }, 7000);
        }catch(e){ console.warn('showEntradaToast error', e); }
    }
    ...
    function showInfoToast(message, duration){
        try{
            ...
            if(!c){ c = document.createElement('div'); c.className = 'info-toast-container'; c.style.position='fixed'; c.style.top='18px'; c.style.right='18px'; c.style.zIndex='999999999'; c.style.display='flex'; c.style.flexDirection='column'; c.style.gap='10px'; document.body.appendChild(c); }
            const t = document.createElement('div'); t.className='info-toast'; t.textContent = message;
            t.style.background = 'linear-gradient(180deg,#e6f0ff,#d7eaff)'; t.style.color='#05386b'; t.style.padding='10px 14px'; t.style.borderRadius='8px'; t.style.boxShadow='0 8px 24px rgba(2,6,23,0.12)'; t.style.border='1px solid rgba(3,102,214,0.12)'; t.style.marginTop='8px'; t.style.opacity='0'; t.style.transition='opacity .18s,transform .18s'; c.appendChild(t);
            requestAnimationFrame(()=>{ t.style.opacity='1'; t.style.transform='none'; });
            setTimeout(()=>{ t.style.opacity='0'; setTimeout(()=>{ try{ t.remove(); }catch(e){} },220); }, duration);
        }catch(e){ console.warn('showInfoToast error', e); }
    }
    ...
    function showConfirmModal(message, okLabel = 'Ok', cancelLabel = 'Cancelar'){
        return new Promise((resolve) => {
            try{
                ...
                const prev = document.getElementById('entradaConfirmModal'); if(prev) prev.remove();
                const overlay = document.createElement('div'); overlay.id = 'entradaConfirmModal'; overlay.className = 'entrada-confirm-modal';
                overlay.style.position = 'fixed'; overlay.style.inset = '0'; overlay.style.display = 'flex'; overlay.style.alignItems = 'center'; overlay.style.justifyContent = 'center'; overlay.style.background = 'rgba(0,0,0,0.36)'; overlay.style.zIndex = '9999999';

                const box = document.createElement('div');
                box.style.maxWidth = '520px'; box.style.width = '88%'; box.style.background = '#fff'; box.style.borderRadius = '10px'; box.style.padding = '18px'; box.style.boxShadow = '0 12px 36px rgba(2,6,23,0.24)'; box.style.display = 'flex'; box.style.flexDirection = 'column';

                const txt = document.createElement('div'); txt.style.marginBottom = '14px'; txt.style.color = '#222'; txt.style.fontSize = '15px'; txt.textContent = message || '';

                const footer = document.createElement('div'); footer.style.display = 'flex'; footer.style.justifyContent = 'flex-end'; footer.style.gap = '10px';
                const btnCancel = document.createElement('button'); btnCancel.textContent = cancelLabel; btnCancel.className = 'btn-secondary';
                const btnOk = document.createElement('button'); btnOk.textContent = okLabel; btnOk.className = 'btn-primary';
                // estilos mínimos para parecer com o sistema
                [btnCancel, btnOk].forEach(b => { b.style.cursor = 'pointer'; b.style.padding = '8px 14px'; b.style.borderRadius = '8px'; b.style.border = 'none'; b.style.fontWeight = '600'; });
                btnCancel.style.background = '#fff'; btnCancel.style.color = '#495057'; btnCancel.style.border = '1px solid #ced4da';
                btnOk.style.background = '#0d6efd'; btnOk.style.color = '#fff'; btnOk.style.boxShadow = '0 6px 18px rgba(13,110,253,0.12)';

                footer.appendChild(btnCancel); footer.appendChild(btnOk);
                box.appendChild(txt); box.appendChild(footer); overlay.appendChild(box); document.body.appendChild(overlay);

                function cleanup(){ try{ overlay.remove(); }catch(e){} document.removeEventListener('keydown', kd); }
                ...
                function kd(e){ if(e.key === 'Escape'){ cleanup(); resolve(false); } if(e.key === 'Enter'){ cleanup(); resolve(true); } }
                document.addEventListener('keydown', kd);
                // foco no botão ok por padrão
                setTimeout(()=>{ try{ btnOk.focus(); }catch(e){} }, 50);
            }catch(e){ console.error('showConfirmModal error', e); resolve(false); }
        });
    }
    ...
    function parseNFeXml(xmlText){
        ...
        if(!inf){ // talvez esteja dentro de NFe > infNFe
            const NFe = doc.getElementsByTagName('NFe')[0];
            if(NFe) inf = NFe.getElementsByTagName('infNFe')[0];
        }

        const getText = (el, tag) => {
            try{ const t = (el.getElementsByTagName(tag)[0] || {}).textContent || ''; return t.trim(); }catch(e){return '';}
        };
        ...
        if(!result.chaveAcesso){ result.chaveAcesso = getText(doc, 'chNFe') || '' }

        const ide = inf ? (inf.getElementsByTagName('ide')[0] || {}) : (doc.getElementsByTagName('ide')[0] || {});
        result.numero = getText(ide, 'nNF') || '';
        result.serie = getText(ide, 'serie') || '';
        result.dataEmissao = getText(ide, 'dhEmi') || getText(ide, 'dEmi') || '';

        const emit = inf ? (inf.getElementsByTagName('emit')[0] || {}) : (doc.getElementsByTagName('emit')[0] || {});
        result.fornecedor = {
        ...
        // itens: tags <det>
        const dets = inf ? inf.getElementsByTagName('det') : doc.getElementsByTagName('det');
        for(let i=0;i<dets.length;i++){
            try{
                ...
                const qCom = parseFloat((getText(prod, 'qCom') || getText(prod, 'qtd') || '').replace(',', '.') || '0') || 0;
                const vUnCom = parseFloat((getText(prod, 'vUnCom') || getText(prod, 'vUn') || getText(prod, 'vUnComercial') || '').replace(',', '.') || '0') || 0;
                const vProd = parseFloat((getText(prod, 'vProd') || '').replace(',', '.') || '0') || (qCom * vUnCom);
                ...
                if(med){
                    validade = getText(med, 'dVal') || getText(med, 'dVenc') || getText(med, 'dFab') || '';
                    lote = getText(med, 'nLote') || '';
                }

                // 2) procurar tags diretamente em <det> ou em <prod>
                validade = validade || getText(det, 'dVenc') || getText(det, 'dVal') || getText(prod, 'dVenc') || getText(prod, 'dVal') || '';
                ...
                if(!validade){
                    const cand = getText(det, 'infAdProd') || getText(det, 'infAd') || getText(det, 'obsFisco') || getText(prod, 'infAdProd') || '';
                ...
                if(!validade){
                    ...
                    const patterns = [/\d{4}-\d{2}-\d{2}/, /\d{2}\/\d{2}\/\d{4}/, /\d{2}-\d{2}-\d{4}/, /\d{8}/];
                ...
                let ncm = getText(prod, 'NCM') || getText(prod, 'Ncm') || getText(prod, 'NCMTrib') || getText(prod, 'cNCM') || '';
                if(!ncm){
                    // procurar no conteúdo do <prod> / <det> um número de 8 dígitos (formato comum do NCM)
                    const txtDet = (det.textContent || '') + ' ' + (prod.textContent || '');
                ...
                result.items.push({ codigo: cProd, descricao: xProd, quantidade: qCom, unitario: vUnCom, total: vProd, validade: validadeISO, lote: lote, ncm: (ncm||'') });
            }catch(e){ continue; }
        }

        // totais
        const totalsTag = inf ? (inf.getElementsByTagName('ICMSTot')[0] || {}) : (doc.getElementsByTagName('ICMSTot')[0] || {});
        result.totals.vProd = parseFloat((totalsTag && (totalsTag.getElementsByTagName('vProd')[0]||{}).textContent || '0').replace(',', '.')) || 0;
        result.totals.vNF = parseFloat((totalsTag && (totalsTag.getElementsByTagName('vNF')[0]||{}).textContent || '0').replace(',', '.')) || 0;
    ...
    async function findCandidatesForItems(items){
        ...
        for(const it of items){
            try{
                const query = encodeURIComponent((it.codigo || it.descricao || it.xProd || it.cProd || '').toString());
                if(!query){ results.push([]); continue; }
                const res = await fetch('/api/itens?q=' + query);
                if(!res.ok){ results.push([]); continue; }
                const data = await res.json();
                results.push(Array.isArray(data) ? data : []);
            }catch(e){ console.warn('candidate fetch error', e); results.push([]); }
        }
        return results;
    }
    ...
    function showProductMappingModal(parsed, candidates){
        try{
            ...
            try{ sessionStorage.setItem('entradaImported', JSON.stringify(parsed)); }catch(e){ console.debug('Erro ao persistir entradaImported', e); }
            // remover modal anterior
            const prev = document.getElementById('mappingModal'); if(prev) prev.remove();
            const overlay = document.createElement('div'); overlay.id = 'mappingModal';
            overlay.classList.add('mapping-modal');
            overlay.style.position = 'fixed'; overlay.style.left = '0'; overlay.style.top='0'; overlay.style.right='0'; overlay.style.bottom='0'; overlay.style.background='rgba(0,0,0,0.4)'; overlay.style.zIndex='100000'; overlay.style.display='flex'; overlay.style.alignItems='center'; overlay.style.justifyContent='center';
            const box = document.createElement('div'); box.style.width='90%'; box.style.maxWidth='1100px'; box.style.maxHeight='80%'; box.style.overflow='auto'; box.style.background='#fff'; box.style.borderRadius='8px'; box.style.padding='18px'; box.style.boxShadow='0 12px 40px rgba(0,0,0,0.3)';
            const title = document.createElement('h3'); title.textContent = 'Produtos sendo comprados pela primeira vez!'; title.style.marginTop='0'; box.appendChild(title);
            ...
            const info = document.createElement('div'); info.style.color='#b02a37'; info.style.marginBottom='12px'; info.innerHTML = 'Caso você já tenha o produto comprado em seu cadastro, clique em "Já tenho o produto cadastrado" e indique qual é o produto. Caso ainda não tiver o produto cadastrado, clique em "Novo Produto" para efetuar o cadastro.';
            ...
            parsed.items.forEach((it, idx) => {
                ...
                const tdNum = document.createElement('td'); tdNum.style.padding='8px'; tdNum.textContent = String(idx+1);
                const tdRef = document.createElement('td'); tdRef.style.padding='8px'; tdRef.textContent = it.codigo || it.cProd || '';
                const tdDesc = document.createElement('td'); tdDesc.style.padding='8px'; tdDesc.textContent = it.descricao || it.xProd || '';
                const tdProd = document.createElement('td'); tdProd.style.padding='8px';
                ...
                const searchWrapper = document.createElement('div'); searchWrapper.style.display = 'flex'; searchWrapper.style.alignItems = 'center'; searchWrapper.style.position = 'relative'; searchWrapper.style.flex = '1 1 auto';
                ...
                try{
                    if (Array.isArray(candidates) && Array.isArray(candidates[idx]) && candidates[idx].length > 0) {
                        const cand0 = candidates[idx][0];
                        const codigoParte = cand0.codigo ? String(cand0.codigo).trim() : '';
                        const nomeParte = (cand0.nome || cand0.descricao || cand0.nomeProduto || cand0.nomeItem || '').trim();
                        inputSearch.value = codigoParte ? `${codigoParte} — ${nomeParte}` : nomeParte;
                        inputSearch.dataset.selectedId = cand0.id;
                        // esconder o botão Novo Produto porque já existe correspondência
                        try{ if(btnNovo) btnNovo.style.display = 'none'; }catch(e){}
                    }
                }catch(e){ /* noop */ }

                // Se candidatos foram fornecidos ao modal, pré-selecionar o primeiro candidato disponível
                try{
                    if (Array.isArray(candidates) && candidates[idx] && candidates[idx].length > 0) {
                        const cand = candidates[idx][0];
                        if (cand) {
                            inputSearch.value = (cand.codigo ? String(cand.codigo).trim() + ' — ' : '') + (cand.nome || cand.descricao || '');
                ...
                const btnNovo = document.createElement('button'); btnNovo.type='button'; btnNovo.textContent='Novo Produto';
                ...
                async function populateResults(q){
                    ...
                    try{
                        ...
                        if(!res.ok){ res = await fetch('/api/itens?q=' + encodeURIComponent(q)); }
                        if(!res.ok) { resultsList.style.display = 'none'; return; }
                        const arr = await res.json();
                        (arr||[]).forEach(c => {
                            const li = document.createElement('li');
                            li.style.listStyle = 'none'; li.style.padding = '8px 12px'; li.style.cursor = 'pointer'; li.style.borderBottom = '1px solid #f1f5f9';
                            const codigoParte = c.codigo ? String(c.codigo).trim() : '';
                            const nomeParte = (c.nome || c.descricao || '').trim();
                            li.textContent = codigoParte ? `${codigoParte} — ${nomeParte}` : nomeParte;
                            li.dataset.id = c.id;
                            li.addEventListener('mousedown', function(e){ // use mousedown para evitar blur antes do click
                                e.preventDefault();
                                ...
                                try{ if(btnNovo) btnNovo.style.display = 'none'; }catch(err){}
                                resultsList.style.display = 'none';
                            });
                            resultsList.appendChild(li);
                        });
                        resultsList.style.display = arr && arr.length ? 'block' : 'none';
                    }catch(e){ console.warn('populateResults error', e); resultsList.style.display='none'; }
                }
                ...
                inputSearch.addEventListener('focus', function(){ if(resultsList.children.length) resultsList.style.display='block'; });
                inputSearch.addEventListener('blur', function(){ setTimeout(()=>{ resultsList.style.display='none'; }, 180); });
                ...
                btnNovo.addEventListener('click', function(){
                    try{
                        ...
                        try{ sessionStorage.setItem('novoProdutoFromXML', JSON.stringify(dadosParaNovo)); }catch(e){ console.warn('sessionStorage set failed', e); }
                        ...
                        try{
                            ...
                            if(Object.keys(selections).length > 0){
                                sessionStorage.setItem('mappingSelections', JSON.stringify(selections));
                                console.log('💾 Seleções salvas antes de navegar:', selections);
                            }
                        }catch(e){ console.warn('Erro ao salvar seleções', e); }
                        
                        // fechar modal antes de navegar
                        const overlayEl = document.getElementById('mappingModal'); if(overlayEl) overlayEl.remove();
                        // navegar para a página de novo produto (mesmo layout)
                        window.location.href = '../item/novo-produto.html?from=xml';
                    }catch(e){ console.warn('Erro ao abrir novo produto', e); }
                });
                ...
                try{
                    ...
                    if(lastRaw){
                        try{
                            ...
                            if(last && (last._orig_parsed_index !== undefined) && Number(last._orig_parsed_index) === Number(idx)){
                                // aplicar dados ao input e esconder botão Novo Produto
                                try{
                                    inputSearch.value = (last.codigo ? (String(last.codigo).trim() + ' — ') : '') + (last.nome || '');
                                    inputSearch.dataset.selectedId = last.id;
                                    if(btnNovo) btnNovo.style.display = 'none';
                                }catch(e){}
                                // limpar chave para não reaplicar em outras linhas
                                try{ sessionStorage.removeItem('lastCreatedProduct'); }catch(e){}
                            }
                        }catch(e){ console.debug('failed parsing lastCreatedProduct', e); }
                    }
                }catch(e){ /* noop */ }
                ...
                try{
                    ...
                    if(selectionsRaw){
                        ...
                        if(selections[idx]){
                            ...
                            if(btnNovo && selections[idx].selectedId) btnNovo.style.display = 'none';
                            console.log('✅ Seleção restaurada para linha', idx, selections[idx]);
                        }
                    }
                }catch(e){ console.debug('failed restoring selections', e); }

                tr.appendChild(tdNum); tr.appendChild(tdRef); tr.appendChild(tdDesc); tr.appendChild(tdProd);
                tbody.appendChild(tr);
            });

            table.appendChild(tbody); box.appendChild(table);

            const footer = document.createElement('div'); footer.className = 'modal-footer'; footer.style.display='flex'; footer.style.justifyContent='flex-end'; footer.style.marginTop='12px';
            const btnSave = document.createElement('button'); btnSave.textContent='Salvar'; btnSave.className='btn btn-primary'; btnSave.style.padding='8px 14px'; btnSave.style.borderRadius='4px';
            const btnCancel = document.createElement('button'); btnCancel.textContent='Cancelar'; btnCancel.className='btn btn-secondary'; btnCancel.style.padding='8px 14px'; btnCancel.style.borderRadius='4px';
            footer.appendChild(btnCancel); footer.appendChild(btnSave); box.appendChild(footer);
            ...
            btnSave.addEventListener('click', async function(){
                ...
                try{
                    ...
                    for(let r=0;r<rowsCheck.length;r++){
                        ...
                        const hasSelected = inputSearch && inputSearch.dataset && inputSearch.dataset.selectedId;
                        const createdFlag = btn && btn.dataset && btn.dataset.create === 'true';
                        if(!hasSelected && !createdFlag){
                            showEntradaToast('Todos os produtos devem ser preenchidos antes de salvar');
                            return;
                        }
                    }
                ...
                for(let r=0;r<rows.length;r++){
                        ...
                        try{
                            const payload = { nome: original.descricao || original.xProd || '', codigo: original.codigo || original.cProd || '', preco: original.unitario || original.vUnCom || 0 };
                            const res = await fetch('/api/itens', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload) });
                            if(res.ok){ const created = await res.json(); finalItems.push({ ...original, matchedId: created.id, codigo: created.codigo || created.id, descricao: created.nome || created.descricao || original.descricao, unitario: Number(created.preco) || original.unitario }); continue; }
                        }catch(e){ console.warn('create product failed', e); }
                        ...
                        finalItems.push(original);
                            ...
                            try{
                                ...
                                if(res.ok){ const prod = await res.json(); finalItems.push({ ...original, matchedId: prod.id, codigo: prod.codigo || prod.id, descricao: prod.nome || prod.descricao || original.descricao, unitario: Number(prod.preco) || original.unitario }); continue; }
                            }catch(e){ console.warn('fetch product by id failed', e); }
                            finalItems.push(original);
                            ...
                            try{
                                ...
                                if(arr && arr.length > 0){ const prod = arr[0]; finalItems.push({ ...original, matchedId: prod.id, codigo: prod.codigo || prod.id, descricao: prod.nome || prod.descricao || original.descricao, unitario: Number(prod.preco) || original.unitario }); continue; }
                ...
                try {
                    const created = await criarRascunhoNoServidor(parsed);
                    if (created && created.id) {
                        try{ sessionStorage.removeItem('mappingSelections'); console.log('🧹 Seleções limpas após salvar'); }catch(e){}
                        overlay.remove();
                        window.location.href = './entrada-manual.html?id=' + encodeURIComponent(created.id);
                        return;
                    }
                ...
                try{ sessionStorage.removeItem('mappingSelections'); console.log('🧹 Seleções limpas após tentativa falha'); }catch(e){}
                overlay.remove();
                showEntradaToast('Não foi possível salvar a entrada no servidor. Verifique o servidor e tente novamente.');
                return;
            });

        }catch(e){ console.error('showProductMappingModal error', e); }
    }
            ...
            setTimeout(() => {
                ...
                try{
                    ...
                    if(raw){
                        ...
                        console.log('⚠️ entradaImported não encontrado - tentando lastCreatedProduct');
                        const lastRaw = sessionStorage.getItem('lastCreatedProduct');
                        if(lastRaw){
                            console.log('📦 lastCreatedProduct encontrado');
                            const last = JSON.parse(lastRaw);
                            const parsed = { items: [ { descricao: last.nome || '', codigo: last.codigo || '', unitario: last.preco || 0 } ] };
                            showProductMappingModal(parsed, []);
                            console.log('✅ Modal de mapeamento aberto com lastCreatedProduct');
                        } else {
                    ...
                    } catch(e) { console.debug('Erro ao limpar query param', e); }
                }catch(e){ 
    ...
    function normalizeDateToISO(s){
        ...
        if(dmy){ const [_, d,m,y] = dmy; return `${y}-${m}-${d}`; }
        // formatos compactos: YYYYMMDD ou DDMMYYYY
        const compact = s.match(/^(\d{8})$/);
        if(compact){ const v = compact[1];
            // decidir se é YYYYMMDD ou DDMMYYYY: se os primeiros 4 > 31 então é YYYYMMDD
            const first4 = parseInt(v.substring(0,4),10);
            if(first4 > 31){ // YYYYMMDD
                const y = v.substring(0,4); const m = v.substring(4,6); const d = v.substring(6,8); return `${y}-${m}-${d}`;
            } else { // DDMMYYYY
                const d = v.substring(0,2); const m = v.substring(2,4); const y = v.substring(4,8); return `${y}-${m}-${d}`;
            }
        }
    ...
    function showConsultaSefazModal(mode = 'byKey', initialChave) {
        ...
        overlay.addEventListener('click', (e) => { try { /* intencionalmente vazio: clique no backdrop não fecha o modal */ } catch(_) {} });
        ...
        if (mode === 'byKey') {
            ...
            btn.addEventListener('click', async () => {
                ...
                if (!chave) { alert('Informe a chave de acesso.'); return; }
                try {
                    resultEl.style.display = 'none'; resultEl.textContent = '';
                    btn.disabled = true; btn.textContent = 'Consultando...';
                    
                    // Primeiro tentar buscar no banco local
                    const res = await fetch(`/api/entrada?chave=${encodeURIComponent(chave)}`);
                    let body = await res.json().catch(()=>null);
                    let notaTemporaria = null;
                    
                    // Se não encontrou no banco (404), buscar XML via API externa
                    if (!res.ok && res.status === 404) {
                        console.log('📡 Nota não encontrada no banco, buscando XML via API externa...');
                        try {
                            // Tentar buscar XML de uma API de consulta (ajuste a URL conforme sua API)
                            const xmlRes = await fetch(`https://api.sefaz.exemplo.com.br/xml?chave=${chave}`).catch(() => null);
                            if (xmlRes && xmlRes.ok) {
                                ...
                                notaTemporaria = {
                                    id: `temp-${Date.now()}`,
                                    fornecedor: parsed.fornecedor?.nome || 'Fornecedor não informado',
                                    numero: parsed.numero || 'S/N',
                                    serie: parsed.serie || '',
                                    dataEmissao: parsed.dataEmissao?.split('T')[0] || new Date().toISOString().split('T')[0],
                                    chaveAcesso: chave,
                                ...
                                body = notaTemporaria;
                                console.log('✅ Nota temporária adicionada à lista:', notaTemporaria);
                            }
                        } catch (xmlErr) {
                    ...
                    sefazConsultas.unshift({ chave, result: body, timestamp: new Date().toLocaleString() });
                    ...
                    if (!body) {
                        ...
                        resultEl.style.display = 'block';
                        const fornecedor = body.fornecedor || 'Fornecedor não informado';
                        const dataEmissao = body.dataEmissao ? new Date(body.dataEmissao + 'T00:00:00').toLocaleDateString('pt-BR') : 'Data não informada';
                        const numero = body.numero || 'S/N';
                        const valorTotal = body.valorTotal ? parseFloat(body.valorTotal).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : 'R$ 0,00';
    ...
    if (btnConsultarSefaz2) {
        btnConsultarSefaz2.addEventListener('click', (e) => {
            try{ e.preventDefault(); e.stopPropagation(); const label = (btnConsultarSefaz2.textContent || 'Consulta Sefaz').trim(); showInfoToast(label + ' — serão implementadas em breve!'); }catch(err){ console.warn(err); }
        });
    }
    ...
    if (btnConfiguracoes) {
        btnConfiguracoes.addEventListener('click', (e) => {
            try{ e.preventDefault(); e.stopPropagation(); const label = (btnConfiguracoes.getAttribute('title') || btnConfiguracoes.textContent || 'Configurações').trim(); showInfoToast(label + ' — serão implementadas em breve!'); }catch(err){ console.warn(err); }
        });
    }
    ...
    if (selectAll) {
        selectAll.addEventListener('change', (e) => {
            const checkboxes = document.querySelectorAll('.entradas-table tbody input[type="checkbox"]');
    ...
    if (btnIgnorar) {
        btnIgnorar.addEventListener('click', async () => {
            ...
            if (selecionados.length === 0) {
                try { showEntradaToast('Selecione ao menos uma entrada para ignorar.', 4200); } catch(e) { alert('Selecione ao menos uma entrada para ignorar.'); }
                return;
            }

            const confirmed = await showConfirmModal(`Confirma marcar ${selecionados.length} entrada(s) como Ignorada(s)?`, 'Ok', 'Cancelar');
            if (!confirmed) return;

            // Atualizar cada entrada localmente e no servidor
            const promises = selecionados.map(async (id) => {
                try {
                    ...
                    const res = await fetch('/api/entrada/manual/' + id, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ situacao: 'ignorada' })
                    });
                    ...
                    entradasData = entradasData.map(e => {
                        if (String(e.id) === String(id)) {
                            const novaSitu = (updated && updated.situacao) ? updated.situacao : 'ignorada';
                            return Object.assign({}, e, { situacao: novaSitu, _data: Object.assign({}, e._data || {}, { situacao: novaSitu }) });
                        }
                        return e;
                    });
                    return { id, ok: true };
                } catch (err) {
                    console.error('Erro ao ignorar entrada', id, err);
            ...
            if (failed.length) {
                try { showEntradaToast(`${failed.length} entrada(s) não puderam ser ignoradas. Veja console para detalhes.`, 5200); } catch(e) { alert(`${failed.length} entrada(s) não puderam ser ignoradas. Veja console para detalhes.`); }
            } else {
                // sucesso
                try { showEntradaToast(`${selecionados.length} entrada(s) marcadas como Ignorada(s)`); } catch(e){}
            }
...
function obterItensSelecionados() {
    const checkboxes = document.querySelectorAll('.entradas-table tbody input[type="checkbox"]:checked');
...
function realizarPesquisaEntrada() {
    ...
    filteredData = entradasData.filter(entrada => {
        ...
        const matchSearch = !searchTerm || 
            (entrada.fornecedor && entrada.fornecedor.toLowerCase().includes(searchTerm)) ||
        ...
        if (currentStatus !== 'todas') {
            matchStatus = entrada.situacao && entrada.situacao.toLowerCase() === currentStatus;
        }
    ...
    console.log(`🔍 Pesquisa realizada - ${filteredData.length} entradas encontradas`);
}
...
function renderizarTabelaEntradas() {
    ...
    function capitalizeStatus(s){ if(!s) return '-'; try{ s = String(s); return s.charAt(0).toUpperCase() + s.slice(1); }catch(e){ return s; } }

    // Renderizar linhas
    dadosPaginados.forEach(entrada => {
        const row = document.createElement('tr');
        // Tornar toda linha clicável (mostrar cursor de mão)
        try { row.style.cursor = 'pointer'; } catch(e) {}
        ...
        const valorDisplay = (!isNaN(valorNum) && isFinite(valorNum)) ? ('R$ ' + valorNum.toFixed(2)) : '-';
    ...
    console.log(`✅ Tabela renderizada com ${dadosPaginados.length} entradas (página ${currentPage})`);
}
...
async function abrirRascunho(entrada) {
    try {
        ...
        const dadosParaEditar = entrada._data || entrada._importedData || entrada._draftData || entrada._parsed || entrada;
        if (!dadosParaEditar) {
        ...
        if (existingId) {
            window.location.href = './entrada-manual.html?id=' + encodeURIComponent(existingId);
            return;
        }
        ...
        try {
            ...
            console.warn('Falha ao criar rascunho no servidor ao abrir rascunho:', err);
        }

        // Se tudo falhar, informar o usuário (não gravar em local/session storage por política)
        alert('Não foi possível abrir a entrada. Verifique a conexão com o servidor e tente novamente.');
