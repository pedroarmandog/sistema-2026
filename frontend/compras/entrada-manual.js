...
function coletarPayloadRascunho() {
    try {
        ...
        const centroResultado = document.getElementById('centroResultado')?.value.trim() || '';
        const categoriaFinanceira = document.getElementById('categoriaFinanceira')?.value.trim() || '';
        
        // Calcular totais dos itens
        const totalProdutos = (window.itensEntrada || []).reduce((sum, item) => sum + (item.totalBruto || 0), 0);
        const valorTotal = parseFloat((document.getElementById('valorTotal')?.value || '0').toString().replace('.', '').replace(',', '.')) || 0;
...
function showSuccessToast(message, duration){
    try{
        ...
        if(!c){ c = document.createElement('div'); c.id = 'entradaManualToastContainer'; c.style.position='fixed'; c.style.top='18px'; c.style.right='18px'; c.style.zIndex='2147483647'; c.style.display='flex'; c.style.flexDirection='column'; c.style.gap='10px'; document.body.appendChild(c); }
        const t = document.createElement('div'); t.className='entrada-manual-toast success'; t.textContent = message;
        t.style.background = 'linear-gradient(180deg,#e6fff2,#d4f7df)'; t.style.color='#0f5132'; t.style.padding='10px 14px'; t.style.borderRadius='8px'; t.style.boxShadow='0 8px 24px rgba(0,0,0,0.12)'; t.style.border='1px solid rgba(34,197,94,0.12)'; t.style.marginTop='8px'; t.style.opacity='0'; t.style.transition='opacity .18s,transform .18s'; c.appendChild(t);
        requestAnimationFrame(()=>{ t.style.opacity='1'; t.style.transform='none'; });
        setTimeout(()=>{ t.style.opacity='0'; setTimeout(()=>{ try{ t.remove(); }catch(e){} },220); }, duration);
    }catch(e){ try{ console.warn('showSuccessToast error', e); }catch(_){} }
}
...
function showErrorToast(message, duration){
    try{
        ...
        let c = document.getElementById('entradaManualToastContainer');
        if(!c){ c = document.createElement('div'); c.id = 'entradaManualToastContainer'; c.style.position='fixed'; c.style.top='18px'; c.style.right='18px'; c.style.zIndex='2147483647'; c.style.display='flex'; c.style.flexDirection='column'; c.style.gap='10px'; document.body.appendChild(c); }
        const t = document.createElement('div'); t.className='entrada-manual-toast error'; t.textContent = message;
        t.style.background = 'linear-gradient(180deg,#fff0f0,#ffe6e6)'; t.style.color='#6b1f1f'; t.style.padding='10px 14px'; t.style.borderRadius='8px'; t.style.boxShadow='0 8px 24px rgba(0,0,0,0.12)'; t.style.border='1px solid rgba(220,38,38,0.12)'; t.style.marginTop='8px'; t.style.opacity='0'; t.style.transition='opacity .18s,transform .18s'; c.appendChild(t);
        requestAnimationFrame(()=>{ t.style.opacity='1'; t.style.transform='none'; });
        setTimeout(()=>{ t.style.opacity='0'; setTimeout(()=>{ try{ t.remove(); }catch(e){} },220); }, duration);
    }catch(e){ try{ console.warn('showErrorToast error', e); }catch(_){} }
}
...
async function salvarRascunho() {
    try {
        ...
        if (!payload) {
            try{ showErrorToast('Nada para salvar como rascunho.'); }catch(e){ alert('Nada para salvar como rascunho.'); }
            return;
        }
        
        const success = await salvarRascunhoInterno(payload, false);
        if (success) {
            try{ showSuccessToast('Rascunho salvo com sucesso! Você encontrará a entrada na lista de Pendentes.'); }catch(e){ alert('Rascunho salvo com sucesso! Você encontrará a entrada na lista de Pendentes.'); }
        }
    } catch (e) {
        console.error('Erro ao salvar rascunho:', e);
        try{ showErrorToast('Erro ao salvar rascunho: ' + (e && e.message ? e.message : String(e))); }catch(_){ alert('Erro ao salvar rascunho: ' + e.message); }
    }
}
    ...
    try {
        ...
        try {
            ...
            for (const it of itemsToSync) {
                ...
                }).catch(err => console.debug('Erro sync validade após salvar:', err));
            }
        } catch (e) {
...
function inicializarEntradaManual() {
    ...
    try { configurarDropdownCentroResultado(); } catch(e){ console.warn('configurarDropdownCentroResultado failed', e); }
    try { configurarDropdownCategoriaFinanceira(); } catch(e){ console.warn('configurarDropdownCategoriaFinanceira failed', e); }
    ...
    try{
        ...
        const hasImportedData = Boolean(editingRaw) || (Boolean(importedRaw) && String(importedAutoFlag) === '1');

        if (entradaId || hasImportedData) {
            // populate imported entrada (se houver)
            try{ setTimeout(()=>{ try{ checkAndPopulateImportedEntrada(); }catch(e){ console.warn('populate imported failed', e); } }, 50); }catch(e){}
            // populate draft/editing entrada (se houver)
            try{ setTimeout(()=>{ try{ checkAndPopulateDraft(); }catch(e){ console.warn('populate draft failed', e); } }, 60); }catch(e){}

            // Se um id foi passado, tentar carregar do servidor (caso o usuário tenha aberto ?id=123)
            if (entradaId) {
                (async function(){
                    try {
                        const r = await fetch('/api/entrada/manual/' + encodeURIComponent(entradaId));
                        if (r.ok) {
                            const data = await r.json().catch(()=>null);
                            if (data) {
                                try{ applyDraftData(data); }catch(e){ console.warn('applyDraftData failed', e); }
                            }
                        }
                    } catch(e) { console.warn('carregar entrada por id falhou', e); }
                })();
            }
        } else {
            console.log('✨ Iniciando nova entrada manual (formulário vazio)');
        }
    }catch(e){ console.warn('Erro ao decidir popular formulário automaticamente', e); }
...
function configurarEventListenersEntradaManual() {
    ...
    const camposTotais = ['desconto', 'seguro', 'despesa', 'frete', 'despesaExtra'];
    ...
    try{ setupFreteDropdown(); }catch(e){ console.warn('setupFreteDropdown error', e); }
    
    // configurar autocompletes para fornecedor e transportador
    try{ setupFornecedorAutocomplete('fornecedor','fornecedorId','fornecedorList'); }catch(e){ console.warn('setupFornecedorAutocomplete error', e); }
    try{ setupFornecedorAutocomplete('transportador','transportadorId','transportadorList'); }catch(e){ console.warn('setupTransportadorAutocomplete error', e); }
}
...
function setupFreteDropdown(){
    ...
    function openList(){ dropdown.classList.add('open'); dropdown.setAttribute('aria-expanded','true'); list.style.display = 'block'; }
    function closeList(){ dropdown.classList.remove('open'); dropdown.setAttribute('aria-expanded','false'); list.style.display = 'none'; }

    toggle.addEventListener('click', function(e){ e.stopPropagation(); const open = dropdown.classList.contains('open'); if(open) closeList(); else openList(); });

    items.forEach(it => {
        it.tabIndex = 0;
        it.addEventListener('click', function(e){ e.stopPropagation(); const v = this.dataset.value; const txt = this.textContent.trim(); if(hidden) hidden.value = v; const label = dropdown.querySelector('.custom-dropdown-selected'); if(label){ label.textContent = txt; label.classList.add('selected'); } closeList(); });
        it.addEventListener('keydown', function(e){ if(e.key === 'Enter' || e.key === ' '){ e.preventDefault(); this.click(); } });
    });

    // fechar ao clicar fora
    document.addEventListener('click', function(e){ if(!dropdown.contains(e.target)) { try{ closeList(); }catch(e){} } });

    // keyboard support on dropdown container
    dropdown.addEventListener('keydown', function(e){
        if(e.key === 'Escape'){ closeList(); }
        if(e.key === 'ArrowDown'){ e.preventDefault(); openList(); const first = list.querySelector('.custom-dropdown-item'); if(first) first.focus(); }
    });
...
function setupFornecedorAutocomplete(inputId, hiddenId, listId){
    ...
    function renderList(arr){
        list.innerHTML = '';
        if(!arr || arr.length===0){ list.style.display='none'; return; }
        const top = document.createElement('div'); top.className='autocomplete-topline'; list.appendChild(top);
        arr.forEach((f, idx) => {
            const it = document.createElement('div'); it.className='autocomplete-item'; it.tabIndex = 0;
            it.dataset.idx = idx; it.dataset.id = f.id; it.dataset.nome = f.nome || f.razaoSocial || '';
            it.innerHTML = `<div class="autocomplete-main">${escapeHtml(f.nome || f.razaoSocial || '')}</div>` + (f.codigo ? `<div class="autocomplete-sub">${escapeHtml(f.codigo)}</div>` : '');
            it.addEventListener('click', function(e){ e.stopPropagation(); selectItem(idx); });
            it.addEventListener('keydown', function(e){ if(e.key==='Enter' || e.key===' '){ e.preventDefault(); selectItem(idx); } });
            list.appendChild(it);
        });
    ...
    const debouncedFetch = debounce(async function(q){
        try{
            ...
            const res = await fetch('/api/fornecedores?q=' + encodeURIComponent(q), { signal: controller.signal });
            const data = await res.json();
            items = Array.isArray(data) ? data : [];
            renderList(items);
        }catch(err){ if(err.name==='AbortError') return; console.warn('autocomplete fetch error', err); }
    }, 300);

    input.addEventListener('input', function(e){ const v = this.value.trim(); if(!v){ list.style.display='none'; return; } debouncedFetch(v); });
    input.addEventListener('focus', function(e){ const v = this.value.trim(); if(v) debouncedFetch(v); });
    ...
    input.addEventListener('keydown', function(e){
        ...
        if(e.key === 'ArrowDown'){
            e.preventDefault(); if(!visible){ if(this.value) debouncedFetch(this.value); return; }
            activeIndex = Math.min(activeIndex + 1, itemsEls.length -1);
            itemsEls[activeIndex]?.focus();
        } else if(e.key === 'ArrowUp'){
            e.preventDefault(); activeIndex = Math.max(activeIndex -1, 0); itemsEls[activeIndex]?.focus();
        } else if(e.key === 'Enter'){
            if(document.activeElement && document.activeElement.classList.contains('autocomplete-item')){ document.activeElement.click(); e.preventDefault(); }
...
function renderizarItens() {
    ...
    if (itensEntrada.length === 0) {
        tbody.innerHTML = '<tr><td colspan="10" style="text-align:center;color:#999;padding:20px;">Nenhum item adicionado</td></tr>';
        return;
    }
    ...
    tbody.querySelectorAll('input').forEach(input => {
        input.addEventListener('input', (e) => {
            ...
            }catch(err){ __editingState = { id, field, value: e.target.value }; }
    ...
    try{
        if(__editingState){
            ...
            if(newInput){
                ...
                if(typeof restoring.selStart === 'number' && typeof restoring.selEnd === 'number'){
                    try{ newInput.selectionStart = restoring.selStart; newInput.selectionEnd = restoring.selEnd; }catch(e){}
                }
            }
            __editingState = null;
        }
    }catch(err){ console.warn('Erro ao restaurar foco de edição', err); }

    // garantir autocomplete em campos descrição (delegação via tbody)
    const descricaoInputs = tbody.querySelectorAll('input[data-field="descricao"]');
    descricaoInputs.forEach(inp => {
        ...
        try{
            ...
            const existing = document.querySelector('.autocomplete-list[data-autocomplete-owner="' + ownerId + '"]');
        ...
        const debounced = debounce(async function(q){
            ...
            try{
                ...
                if(!res.ok){ console.warn('[autocomplete] fetch non-ok', res.status); dropdown.style.display='none'; return; }
                const data = await res.json();
                console.debug('[autocomplete] results count:', Array.isArray(data)?data.length:0);
                // render
                dropdown.innerHTML = '';
                const top = document.createElement('div'); top.className = 'autocomplete-topline'; dropdown.appendChild(top);
                (data||[]).slice(0,20).forEach((p, idx) => {
                    const it = document.createElement('div'); it.className = 'autocomplete-item';
                    it.dataset.id = p.id; it.dataset.idx = idx;
                    it.innerHTML = `<div>${escapeHtml(p.nome || p.descricao || '')}</div>` + (p.codigo ? `<div class="autocomplete-sub">${escapeHtml(p.codigo)}</div>` : '');
                    it.addEventListener('click', function(ev){ ev.stopPropagation(); selectProduto(p, inp, dropdown); });
                ...
                const pageTop = r.bottom + (window.scrollY || window.pageYOffset || 0) + 6;
            ...
            }catch(err){ console.warn('erro itens autocomplete', err); dropdown.style.display='none'; }
        }, 250);

        inp.addEventListener('input', function(e){ debounced(this.value.trim()); });
        inp.addEventListener('focus', function(e){ if(this.value.trim()) debounced(this.value.trim()); });
        inp.addEventListener('keydown', function(e){ if(e.key==='Escape'){ dropdown.style.display='none'; } });

        // fechar ao clicar fora
        document.addEventListener('click', function(ev){ if(!dropdown.contains(ev.target) && ev.target !== inp) { dropdown.style.display='none'; } });
    });
    ...
    try{
        ...
        removeBtns.forEach(btn => {
            ...
            btn.addEventListener('click', function(e){ e.preventDefault(); e.stopPropagation(); const id = this.dataset.id; if(id){ removerItem(id); } });
        });
    }catch(e){ console.warn('Erro ao configurar botões remover', e); }
}
...
function removerItem(id){
    ...
    let counter = 1;
    itensEntrada.forEach(i => { i.numero = counter++; });
    renderizarItens();
    calcularTotais();
}
    ...
    if (['quantidade', 'fator', 'entEstoque', 'unitario'].includes(field)) {
        ...
        if (field === 'validade') {
            ...
            if (prodId && value) {
                ...
                if (iso) {
                    console.log('🔄 Sincronizando validade para produto', prodId, 'valor:', iso);
                    fetch(`/api/itens/${encodeURIComponent(prodId)}`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ validade: iso })
                        ...
                        if (resp.ok) {
                            ...
                            console.warn('⚠️ Falha ao atualizar validade:', resp.status, txt);
                        }
                    }).catch(err => console.warn('❌ Erro ao sincronizar validade:', err));
                } else {
                    console.warn('⚠️ Não foi possível normalizar a data:', value);
                }
            } else {
                console.warn('⚠️ Sincronização não realizada:', prodId ? 'sem valor' : 'sem matchedId');
            }
        }
    }
}
...
function calcularTotais() {
    const totalProdutos = itensEntrada.reduce((sum, item) => sum + item.totalBruto, 0);
    ...
    const despesaExtra = parseFloat(document.getElementById('despesaExtra')?.value) || 0;
    
    const valorTotal = totalProdutos - desconto + seguro + despesa + icmsST + frete + ipi + despesaExtra;
...
function checkAndPopulateImportedEntrada(){
    try{
        ...
        if(!raw) return;
        let data = null;
        try{ data = JSON.parse(raw); }catch(e){ console.warn('entradaImported JSON parse failed', e); return; }
        if(!data) return;
    ...
    }catch(e){ console.warn('checkAndPopulateImportedEntrada error', e); }
}
...
function checkAndPopulateDraft() {
    try {
        ...
        if (entrada.fornecedor && document.getElementById('fornecedor')) {
            ...
            try {
                if (typeof entrada.fornecedor === 'string') {
                    ...
                    if (fIdEl && entrada.fornecedor.id) fIdEl.value = entrada.fornecedor.id;
                } else {
        ...
        if (entrada.dataEmissao && document.getElementById('dataEmissao')) {
            document.getElementById('dataEmissao').value = entrada.dataEmissao.split('T')[0] || entrada.dataEmissao;
        }
        if (entrada.dataEntrada && document.getElementById('dataEntrada')) {
            document.getElementById('dataEntrada').value = entrada.dataEntrada.split('T')[0] || entrada.dataEntrada;
        }
        ...
        if (entrada.centroResultado && document.getElementById('centroResultado')) {
            document.getElementById('centroResultado').value = entrada.centroResultado;
        }
        if (entrada.categoriaFinanceira && document.getElementById('categoriaFinanceira')) {
            document.getElementById('categoriaFinanceira').value = entrada.categoriaFinanceira;
        }
        
        // Restaurar itens
        if (entrada.itens && Array.isArray(entrada.itens) && entrada.itens.length > 0) {
...
function applyDraftData(entrada) {
    try {
        if (!entrada) return;
        console.log('📝 Aplicando dados da entrada (servidor) no formulário:', entrada);
        ...
        if (entrada.totalProdutos && document.getElementById('totalProdutos')) document.getElementById('totalProdutos').value = formatarMoeda(Number(entrada.totalProdutos || 0));
        if (entrada.valorTotal && document.getElementById('valorTotal')) document.getElementById('valorTotal').value = formatarMoeda(Number(entrada.valorTotal || 0));

        showSimpleToast('Entrada carregada do servidor');
    } catch (e) { console.warn('applyDraftData error', e); }
}
...
function populateFromImported(obj){
    try{
        ...
        if(obj.numero && document.getElementById('numero')) document.getElementById('numero').value = obj.numero;
        if(obj.serie && document.getElementById('serie')) document.getElementById('serie').value = obj.serie;
        if(obj.dataEmissao && document.getElementById('dataEmissao')) document.getElementById('dataEmissao').value = (obj.dataEmissao.split('T')[0] || obj.dataEmissao);
        if(obj.chaveAcesso && document.getElementById('chaveAcesso')) document.getElementById('chaveAcesso').value = obj.chaveAcesso;

        // itens
        if(Array.isArray(obj.items) && obj.items.length>0){
            itensEntrada = []; itemCounter = 1;
            obj.items.forEach((it, idx) => {
                const quantidade = Number(it.quantidade) || Number(it.qCom) || 0;
                const unitario = Number(it.unitario) || Number(it.vUnCom) || Number(it.vUnit) || Number(it.preco) || 0;
                const total = Number(it.total) || Number(it.vProd) || (quantidade * unitario) || 0;
                // normalizar validade para formato YYYY-MM-DD usado por <input type="date">
                const validadeRaw = it.validade || it.dVenc || it.dVal || it.dataValidade || '';
        ...
        // totais se fornecidos
        if(obj.totals){
            if(document.getElementById('totalProdutos')) document.getElementById('totalProdutos').value = formatarMoeda(Number(obj.totals.vProd || 0));
            if(document.getElementById('valorTotal')) document.getElementById('valorTotal').value = formatarMoeda(Number(obj.totals.vNF || obj.totals.vProd || 0));
        }
    }catch(e){ console.warn('populateFromImported error', e); }
}
...
function showSimpleToast(message){
    try{
        ...
        requestAnimationFrame(()=>{ t.style.opacity='1'; t.style.transform='none'; });
        setTimeout(()=>{ t.style.opacity='0'; setTimeout(()=>{ try{ t.remove(); }catch(e){} },220); }, 5000);
    }catch(e){ console.warn('showSimpleToast error', e); }
}
...
async function fetchCentroResultadosEntrada(){
    ...
    }catch(e){ console.warn('fetchCentroResultadosEntrada api error', e); }
    ...
    try{
        ...
        return data.map(c => ({ descricao: c.descricao || c.name || '', unidade: c.unidade || '' })).filter(x => x.descricao);
    }catch(e){ console.warn('fetchCentroResultadosEntrada fallback error', e); return []; }
}
...
function configurarDropdownCentroResultado(){
    ...
    if(!wrapper.classList.contains('select-wrapper')){
        const wrap = document.createElement('div'); wrap.className = 'select-wrapper'; try{ wrapper.replaceChild(wrap, input); wrap.appendChild(input); wrapper = wrap; }catch(e){ wrapper = input.parentElement; }
    }
    ...
    input.addEventListener('focus', showDropdown); input.addEventListener('click', showDropdown);
    input.addEventListener('input', function(){ const q = (input.value||'').toLowerCase().trim(); if(!cached) return; const filtered = cached.filter(it => (it.descricao||'').toLowerCase().includes(q) || (it.unidade||'').toLowerCase().includes(q)); renderFiltered(filtered); });
    ...
    function renderFiltered(items){
        ...
        dropdown.innerHTML = '';
        if(!items || items.length===0){ const el = document.createElement('div'); el.className='select-item no-results'; el.textContent='Nenhum resultado'; dropdown.appendChild(el); return; }
        items.forEach(it=>{
            const el = document.createElement('div'); el.className='select-item'; el.textContent = (it.unidade? it.unidade + ' - ' : '') + it.descricao;
            el.addEventListener('click', ()=>{ input.value = it.descricao; close(); try{ input.blur(); }catch(e){} });
        ...
        const pageTop = rect.bottom + (window.scrollY || window.pageYOffset || 0) + 6;
        dropdown.style.left = pageLeft + 'px';
        dropdown.style.top = pageTop + 'px';
        dropdown.style.minWidth = input.offsetWidth + 'px';
    }

    function showDropdown(e){ try{ if(e && e.stopPropagation) e.stopPropagation(); }catch(e){} if(suppressOpen || dropdown) return; if(cached){ renderFiltered(cached); return; } fetchCentroResultadosEntrada().then(items=>{ cached = items || []; renderFiltered(cached); }).catch(()=>{}); }
    ...
    document.addEventListener('click', function(e){ try{ if(dropdown && (wrapper.contains(e.target) || dropdown.contains(e.target))) return; }catch(err){} close(); });
}
...
async function fetchCategoriaFinanceiras(){
    try{
        ...
        if(typeof ApiClient !== 'undefined' && ApiClient.getCategoriaFinanceira) {
            ...
            if(Array.isArray(list)) return list.map(c => ({ descricao: c.descricao || c.name || '' })).filter(x=>x.descricao);
        }
    }catch(e){ console.debug('fetchCategoriaFinanceiras api error', e); }
    ...
    try{
        ...
        for (const url of candidates) {
            try {
                ...
                if (!r.ok) { console.debug('fetchCategoriaFinanceiras: fallback url not ok', url, r.status); continue; }
                html = await r.text();
                loadedFrom = url;
                break;
            } catch(e) { console.debug('fetchCategoriaFinanceiras: fetch error for', url, e); }
        }
        if (!html) { console.warn('fetchCategoriaFinanceiras: none of the candidate urls returned HTML'); return []; }
        console.debug('fetchCategoriaFinanceiras: loaded HTML from', loadedFrom);
        const parser = new DOMParser();
        const doc = parser.parseFromString(html, 'text/html');
        const rows = Array.from(doc.querySelectorAll('.categoria-financeira-table tbody tr'));
        const items = rows.map(tr => ({ descricao: (tr.cells[0] && tr.cells[0].textContent || '').toString().trim() })).filter(x=>x.descricao);
        // dedupe
        const seen = new Set(); const out = []; items.forEach(i=>{ if(!seen.has(i.descricao)){ seen.add(i.descricao); out.push(i); } });
        return out;
    }catch(e){ console.warn('fetchCategoriaFinanceiras fallback error', e); return []; }
}

function configurarDropdownCategoriaFinanceira(){
    const input = document.getElementById('categoriaFinanceira'); if(!input) return;
    let wrapper = input.closest('.input-with-icon') || input.parentElement; if(!wrapper.classList.contains('select-wrapper')){ const wrap = document.createElement('div'); wrap.className='select-wrapper'; try{ wrapper.replaceChild(wrap, input); wrap.appendChild(input); wrapper = wrap; }catch(e){ wrapper = input.parentElement; } }
    try{ input.setAttribute('autocomplete','off'); input.setAttribute('spellcheck','false'); }catch(e){}
    let dropdown=null; let cached=null; let suppressOpen=false;
    input.addEventListener('focus', showDropdown); input.addEventListener('click', showDropdown);
    input.addEventListener('input', function(){ const q = (input.value||'').toLowerCase().trim(); if(!cached) return; const filtered = cached.filter(it=> (it.descricao||'').toLowerCase().includes(q)); renderFiltered(filtered); });
    ...
    function showDropdown(e){ try{ if(e && e.stopPropagation) e.stopPropagation(); }catch(e){} if(suppressOpen || dropdown) return; if(cached){ renderFiltered(cached); return; } fetchCategoriaFinanceiras().then(items=>{ cached = items || []; renderFiltered(cached); }).catch(()=>{}); }
    function close(){ if(dropdown && dropdown.parentElement) dropdown.parentElement.removeChild(dropdown); dropdown=null; suppressOpen=true; setTimeout(()=>{ suppressOpen=false; }, 120); }
    document.addEventListener('click', function(e){ try{ if(dropdown && (wrapper.contains(e.target) || dropdown.contains(e.target))) return; }catch(err){} close(); });
}
...
async function finalizarEntrada() {
    try {
        ...
        if (!fornecedor || !numero || !serie || !dataEmissao || !dataEntrada) {
            try{ showErrorToast('Preencha todos os campos obrigatórios (*).'); }catch(e){ alert('Preencha todos os campos obrigatórios (*).'); }
            return;
        }

        if (itensEntrada.length === 0) {
            try{ showErrorToast('Adicione ao menos um item à entrada.'); }catch(e){ alert('Adicione ao menos um item à entrada.'); }
            return;
        }
        
        const transportador = document.getElementById('transportador')?.value.trim() || '';
        const fretePorConta = document.getElementById('fretePorConta')?.value.trim() || '';
        ...
        const payload = {
            ...
            despesaExtra: parseFloat(document.getElementById('despesaExtra')?.value) || 0,
            totalProdutos: itensEntrada.reduce((sum, item) => sum + item.totalBruto, 0),
            valorTotal: parseFloat((document.getElementById('valorTotal')?.value || '0').replace('.', '').replace(',', '.')) || 0,
            centroResultado: document.getElementById('centroResultado')?.value.trim() || '',
            categoriaFinanceira: document.getElementById('categoriaFinanceira')?.value.trim() || ''
        };
        ...
        if (!res.ok) {
            try{ showErrorToast('Erro ao salvar entrada: ' + (body && body.error ? body.error : res.statusText)); }catch(e){ alert('Erro ao salvar entrada: ' + (body && body.error ? body.error : res.statusText)); }
            return;
        }
        ...
        try{ sessionStorage.removeItem('entradaImported'); }catch(e){}
        ...
        try {
            ...
            if (!created) {
                try {
                    ...
                    console.warn('finalizarEntrada: servidor não retornou JSON, usando payload como fallback (id:', inferredId, ')');
                } catch (e) { created = Object.assign({}, payload, { id: ('tmp-' + Date.now()) }); }
            }
            if (created && created.id) {
                // criar produtos no catálogo apenas para itens que não possuem matchedId
                for (const it of itensEntrada) {
                    try {
                        if (it.matchedId) continue; // já existe no catálogo
                        const itemPayload = {
                            codigo: it.codigo || '',
                        ...
                        const ri = await fetch('/api/meus-itens', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify(itemPayload)
                        });
                        if (ri.ok) {
                            const createdItem = await ri.json().catch(()=>null);
                            // opcional: vincular createdItem.id ao item no servidor (não implementado aqui)
                        }
                    } catch(e) { console.warn('Erro criando produto em meus-itens', e); }
                }

                // Garantir que a entrada esteja com status 'concluido' no servidor
                try {
                    if ((created.situacao || '').toLowerCase() !== 'concluido') {
                        await fetch('/api/entrada/manual/' + encodeURIComponent(created.id), {
                            method: 'PUT',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ situacao: 'concluido' })
                        });
                    }
                } catch(e) { console.warn('Erro atualizando situacao da entrada', e); }
                // sinalizar para a listagem injetar/mostrar esta entrada ao voltar
                try { sessionStorage.setItem('injectedSavedEntrada', JSON.stringify(created)); } catch(e) { console.warn('sessionStorage set failed', e); }
            }
        } catch(e) { console.warn('finalizarEntrada: pós-criação falhou', e); }

        try{
            showSuccessToast('Entrada manual salva com sucesso!');
            // Dar um pequeno atraso para o usuário ver o toast antes de navegar
            setTimeout(()=>{ window.location.href = './entrada-mercadoria.html'; }, 700);
        }catch(e){
        ...
        try{ showErrorToast('Erro ao salvar entrada: ' + (err && err.message ? err.message : String(err))); }catch(e){ alert('Erro ao salvar entrada: ' + (err && err.message ? err.message : String(err))); }
    }
}
...
function excluirNota() {
    if (confirm('Tem certeza que deseja excluir esta nota?')) {
        ...
        if (document.getElementById('serie')) document.getElementById('serie').value = '';
        if (document.getElementById('chaveAcesso')) document.getElementById('chaveAcesso').value = '';
        alert('Nota excluída.');
    }
}
