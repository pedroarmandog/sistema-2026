...
function adicionarPDV() {
    const descricao = prompt('Digite a descrição do PDV:');
    if (descricao && descricao.trim()) {
        const nomeComputador = prompt('Digite o nome do computador:');
        if (nomeComputador && nomeComputador.trim()) {
            const ecfLancamento = prompt('Digite o ECF/Lançamento do Item:');
            if (ecfLancamento && ecfLancamento.trim()) {
                ...
                adicionarLinhaNaTabelaPDV(descricao.trim(), nomeComputador.trim(), ecfLancamento.trim());
...
function realizarPesquisaPDV() {
    ...
    linhas.forEach(linha => {
        ...
        if (descricao.includes(termo) || nomeComputador.includes(termo) || ecfLancamento.includes(termo)) {
            linha.style.display = '';
            encontrados++;
        } else {
...
function atualizarContadorPDV(totalExibidos = null) {
    ...
    if (contador) {
        const linhasVisiveis = totalExibidos !== null ? 
            totalExibidos : 
            document.querySelectorAll('.pdv-table tbody tr:not([style*="display: none"])').length;
