...
function handleDeleteGrupo(grupoId) {
    console.log('Excluir grupo:', grupoId);
    const grupo = gruposData.find(g => g.id === grupoId);
    
    if (grupo) {
        if (grupo.isDefault) {
            showNotification('Os grupos padrão do sistema não podem ser excluídos', 'error');
            return;
        }
        
        if (confirm(`Tem certeza que deseja excluir o grupo "${grupo.descricao}"?`)) {
