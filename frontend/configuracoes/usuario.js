...
// Elementos da página
let searchInput, searchBtn, usuarioTable, paginationInfo, statusFilter, itemsPerPageSelect;
...
async function carregarUsuarios() {
    try {
        ...
        usuariosData = usuarios.map(u => ({
            ...
            dataCriacao: u.createdAt ? new Date(u.createdAt).toLocaleDateString('pt-BR') : '',
            ultimoLogin: u.updatedAt ? new Date(u.updatedAt).toLocaleDateString('pt-BR') : ''
        }));
...
function handleDeleteUsuario(usuarioId) {
    console.log('Excluir usuário:', usuarioId);
    const usuario = usuariosData.find(u => u.id === usuarioId);
    
    if (usuario) {
        if (usuario.nome === 'LOGIN INICIAL') {
            showNotification('O usuário LOGIN INICIAL não pode ser excluído', 'error');
            return;
        }
