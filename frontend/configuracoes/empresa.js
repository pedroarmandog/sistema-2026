...
function handleAddEmpresa() {
    ...
    try {
        ...
        showNotification('Não foi possível abrir o formulário de cadastro', 'error');
    }
}
...
async function handleDeleteEmpresa(empresaId) {
    ...
    try {
        const confirmMsg = 'Deseja realmente excluir esta empresa? Esta ação não poderá ser desfeita.';
        ...
        const totalPages = Math.max(1, Math.ceil(empresasData.length / itemsPerPage));
