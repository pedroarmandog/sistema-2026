...
// Elementos da página
let searchInput, searchBtn, empresaTable, paginationInfo, paginationControls, btnFloatingAdd, btnAddEmpresa;
...
function renderEmpresas(searchTerm = '') {
    ...
    if (searchTerm) {
        filteredEmpresas = empresasData.filter(empresa => 
...
function handleDeleteEmpresa(empresaId) {
...
async function buscarCep(cepRaw) {
    ...
    try {
        ...
        if (cidadeEl) cidadeEl.value = data.localidade ? (data.localidade.toUpperCase() + (data.uf ? ' (' + data.uf + ')' : '')) : '';
...
document.addEventListener('DOMContentLoaded', function() {
    setTimeout(() => {
        ...
        if (btnCancelar) btnCancelar.addEventListener('click', function(){ window.location.href = './empresa.html'; });
    ...
    try{
        ...
        if (editId) {
                ...
                .then(empresa => {
                    if (empresa) {
                        ...
                        if (empresa.endereco && typeof empresa.endereco === 'object') {
                            ...
                            if (endInput && empresa.endereco.rua) endInput.value = empresa.endereco.rua;
                            if (numInput && empresa.endereco.numero) numInput.value = empresa.endereco.numero;
                            if (compInput && empresa.endereco.complemento) compInput.value = empresa.endereco.complemento;
                            if (bairroInput && empresa.endereco.bairro) bairroInput.value = empresa.endereco.bairro;
                            if (cidInput && empresa.endereco.cidade) cidInput.value = empresa.endereco.cidade;
                            if (cepInput && empresa.endereco.cep) cepInput.value = empresa.endereco.cep;
                        }
                        ...
                        if (preview && empresa.logo && empresa.logo !== '') {
                            ...
                            const hasNewLogo = window._currentLogoDataUrl && window._currentLogoDataUrl !== '';
                            if (!hasNewLogo) {
                                // logo é o nome do arquivo, construir URL completo
                                preview.src = `http://localhost:3000/uploads/${empresa.logo}`;
                        ...
                        const e1 = document.getElementById('empresaCadastroNome'); if (e1) e1.innerText = nomeExibicao;
                        const e2 = document.getElementById('empresaFinanceiroNome'); if (e2) e2.innerText = nomeExibicao;
                        const e3 = document.getElementById('empresaPrecoNome'); if (e3) e3.innerText = nomeExibicao;
                        
                        // breadcrumb
                        if (breadcrumbEl) breadcrumbEl.innerText = `${empresa.id} - ${nomeExibicao}`;
                    }
                })
                .catch(e => { console.warn('Falha ao carregar empresa para edição', e); });
        } else {
            ...
            try{
                ...
                const e1 = document.getElementById('empresaCadastroNome'); if (e1) e1.innerText = '---';
                const e2 = document.getElementById('empresaFinanceiroNome'); if (e2) e2.innerText = '---';
                const e3 = document.getElementById('empresaPrecoNome'); if (e3) e3.innerText = '---';
                // limpar breadcrumb (em modo novo não mostrar nome)
                if (breadcrumbEl) breadcrumbEl.innerText = '';
            }catch(e){ /* silencioso */ }
        }
    }catch(e){ console.warn('initFormOnOpen error', e); }
})();
        ...
        img.onload = function(){
            ...
            try {
                ...
                for (let i=3;i<imgd.length;i+=step){
                    if (imgd[i] < 255) { resolve(true); return; }
                }
                ...
                for (let c=0;c<corners.length;c++){
                    const px = corners[c];
                    const idx = px*4 + 3;
                    if (imgd[idx] < 255) { resolve(true); return; }
                }
                resolve(false);
            } catch (e) {
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if (upload && preview) {
        upload.addEventListener('change', async function(e){
            ...
            if (file.type !== 'image/png'){
                if (typeof showNotification === 'function') showNotification('Apenas arquivos PNG são permitidos', 'error');
                upload.value = '';
                return;
            }
            try{
                    ...
                    if (!dimsOk) {
                        if (typeof showNotification === 'function') showNotification('A imagem deve ter exatamente 113x77 pixels', 'error');
                        upload.value = '';
                        return;
                    }
                    const hasTransp = await pngHasTransparency(dataUrl);
                    if (!hasTransp){
                        if (typeof showNotification === 'function') showNotification('O PNG deve ter fundo transparente', 'error');
                        upload.value = '';
                        return;
                    }
                ...
                if (typeof showNotification === 'function') showNotification('Logo carregada com sucesso', 'success');
            }catch(err){
                console.error('Erro ao processar logo:', err);
                if (typeof showNotification === 'function') showNotification('Erro ao processar a imagem', 'error');
                upload.value = '';
            }
        });
    }
    ...
    if (removeBtn && preview){
        removeBtn.addEventListener('click', function(e){
            ...
            if (typeof showNotification === 'function') showNotification('Logo restaurada para o padrão', 'info');
        });
    }
...
function salvarEmpresa(){
    ...
    try{ 
        if (preview && preview.src && preview.src.indexOf('data:') === 0) {
            ...
            console.log('✅ Logo encontrada no preview, tamanho:', logoDataUrl.length, 'bytes');
        } else {
            console.log('⚠️ Sem logo no preview ou não é base64. preview.src:', preview?.src?.substring(0, 50));
        }
    } catch(e){
    ...
    console.log('📦 Dados da empresa:', { ...empresaForm, logo: logoDataUrl ? `[${logoDataUrl.length} bytes]` : 'vazio' });

    if (editId) {
        // Atualizar empresa existente no backend
        ApiClient.atualizarEmpresa(editId, empresaForm)
            .then(() => {
                if (typeof showNotification === 'function') showNotification('Empresa atualizada com sucesso', 'success');
                setTimeout(()=>{ window.location.href = './empresa.html'; }, 600);
            })
            .catch(err => {
                console.error('Erro ao atualizar empresa', err);
                if (typeof showNotification === 'function') showNotification('Erro ao atualizar empresa: ' + err.message, 'error');
            });
    } else {
        // Criar nova empresa no backend
        ApiClient.criarEmpresa(empresaForm)
            .then(() => {
                if (typeof showNotification === 'function') showNotification('Empresa salva com sucesso', 'success');
                setTimeout(()=>{ window.location.href = './empresa.html'; }, 600);
            })
            .catch(err => {
                console.error('Erro ao salvar empresa', err);
                if (typeof showNotification === 'function') showNotification('Erro ao salvar empresa: ' + err.message, 'error');
            });
    }
}
