...
function setupFilterEventListeners() {
    ...
    const filterCheckboxes = document.querySelectorAll('input[type="checkbox"][onchange*="updateFilterTags"]');
    console.log(`📝 Encontrados ${filterCheckboxes.length} checkboxes de filtro`);
    
    filterCheckboxes.forEach((checkbox, index) => {
        console.log(`🔧 Configurando checkbox ${index + 1}: value="${checkbox.value}"`);
        
        // Remover o event listener inline e adicionar um novo
        checkbox.removeAttribute('onchange');
        
        checkbox.addEventListener('change', function() {
            console.log(`✅ Checkbox clicado: ${checkbox.value} (${checkbox.checked})`);
            
            // Determinar a categoria baseada no contexto
            let categoria = 'group'; // Padrão para grupos
            if (checkbox.closest('[id*="birthday"]') || checkbox.value.includes('hoje') || checkbox.value.includes('semana')) {
                categoria = 'birthday';
            }
...
function testFilterFunction() {
    ...
    console.log('📦 Clientes originais:', window.originalClients?.length || 'undefined');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 40px;"><i class="fas fa-spinner fa-spin" style="color: #3498db; margin-right: 10px;"></i>Carregando clientes...</td></tr>';
    } else {
        console.error('❌ Elemento clientsTableBody NÃO encontrado!');
        console.log('📝 Elementos com ID na página:', Array.from(document.querySelectorAll('[id]')).map(el => el.id));
    }
    ...
    if (searchInput) {
        ...
        console.log('📝 Inputs encontrados:', Array.from(allInputs).map(inp => inp.id || inp.name || inp.type));
    }
...
async function loadClients(forceRefresh = false) {
    ...
    try {
        ...
        console.log('✅ Dados válidos, chamando displayClients com', clientesArray.length, 'clientes');
...
function displayClients(clientes) {
    ...
    const hasFiltrosAtivos = window.filtrosAtivos && (
    ...
    if (hasFiltrosAtivos) {
        console.log('🏷️ Filtros ativos detectados - usar dados originais como base');
...
function searchClientsRealTime(searchTerm) {
    ...
    console.log(`✅ Busca concluída: ${window.totalClients} clientes encontrados`);
}
...
async function performDeleteClient(id, clientName) {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            showNotification(`Cliente "${clientName}" foi excluído com sucesso!`, 'success');
            ...
            if (clientRow) {
                ...
                setTimeout(() => {
                    ...
                    if (remainingRows.length === 0) {
                        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 40px; color: #666;"><i class="fas fa-users" style="font-size: 48px; margin-bottom: 15px; display: block; opacity: 0.3;"></i>Nenhum cliente encontrado</td></tr>';
                    }
                }, 300);
            } else {
...
function showNotification(message, type) {
    ...
    const backgroundColor = type === 'success' ? '#27ae60' : type === 'info' ? '#3498db' : '#5a9bd4';
    ...
    notificationDiv.style.fontFamily = "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif";
    ...
    notificationDiv.innerHTML = '<i class="fas ' + iconClass + '"></i><span>' + message + '</span><button onclick="this.parentElement.parentElement.remove()" style="background: none; border: none; color: white; font-size: 18px; cursor: pointer; margin-left: auto;">×</button>';
...
async function exportToExcel() {
    ...
    try {
        ...
        if (hasFiltrosAtivos) {
            ...
            console.log('📋 Exportando dados filtrados:', clientes.length, 'clientes');
            
            // Mostrar quais filtros estão ativos
            const filtrosAtivosTexto = [];
            if (window.filtrosAtivos.grupos.length > 0) {
                filtrosAtivosTexto.push(`Grupos: ${window.filtrosAtivos.grupos.join(', ')}`);
            }
            if (window.filtrosAtivos.status.length > 0) {
                filtrosAtivosTexto.push(`Status: ${window.filtrosAtivos.status.join(', ')}`);
            }
            if (window.filtrosAtivos.aniversariantes.length > 0) {
                filtrosAtivosTexto.push(`Aniversariantes: ${window.filtrosAtivos.aniversariantes.join(', ')}`);
            }
            if (window.filtrosAtivos.busca) {
                filtrosAtivosTexto.push(`Busca: "${window.filtrosAtivos.busca}"`);
            }
            console.log('🏷️ Filtros ativos na exportação:', filtrosAtivosTexto.join(' | '));
        ...
        const dadosExcel = clientes.map((cliente, index) => ({
            ...
            'Data de Nascimento': cliente.data_nascimento ? new Date(cliente.data_nascimento).toLocaleDateString('pt-BR') : 'Não informado',
            'Data de Cadastro': new Date(cliente.createdAt).toLocaleDateString('pt-BR'),
            'Hora de Cadastro': new Date(cliente.createdAt).toLocaleTimeString('pt-BR'),
        ...
        showNotification(`✅ Exportado: ${clientes.length} clientes ${tipoExportacao} em ${nomeArquivo}`, 'success');
...
function updateFilterTags(categoria, checkbox) {
    ...
    console.log(`🏷️ Atualizando filtro: ${categoria} = ${valor} (${isChecked ? 'marcado' : 'desmarcado'})`);
    console.log('📊 Estado atual dos filtros:', activeFiltros);
    ...
    if (isChecked) {
        ...
        // Remover filtro
        activeFiltros[categoria] = activeFiltros[categoria].filter(item => item !== valor);
...
function renderFilterTags() {
    ...
    if (activeFiltros.search) {
        const searchTag = createFilterTag('search', activeFiltros.search, `Busca: "${activeFiltros.search}"`);
...
function removeFilterTag(categoria, valor) {
    ...
    if (categoria === 'search') {
        ...
        const searchInputElement = document.getElementById('searchInput') || document.getElementById('searchClients');
...
function applyFilters() {
    ...
    let searchInputElement = document.getElementById('searchInput') || document.getElementById('searchClients');
    ...
    if (searchInput.trim() !== '') {
        ...
        clientesFiltrados = clientesFiltrados.filter(cliente => {
            const searchableText = `${cliente.nome} ${cliente.email || ''} ${cliente.telefone || ''}`.toLowerCase();
            return searchableText.includes(searchInput);
        });
        console.log(`🔍 Filtro de busca: ${clientesAntes} → ${clientesFiltrados.length} clientes`);
    }
    ...
    if (activeFiltros.group.length > 0) {
        ...
        clientesFiltrados = clientesFiltrados.filter(cliente => {
            console.log(`👤 Cliente: ${cliente.nome}, Grupo: ${cliente.grupo_cliente}`);
            
            return activeFiltros.group.some(group => {
                // Verificar correspondência direta primeiro
                if (cliente.grupo_cliente === group) {
                    console.log(`   ✅ Match direto: "${cliente.grupo_cliente}" === "${group}"`);
                    return true;
                }
                
                // Verificar usando a conversão de key
                const clienteGrupoKey = cliente.grupo_cliente ? 
                    cliente.grupo_cliente.toLowerCase().replace(/\s+/g, '-').replace(/[()]/g, '') : '';
                const groupKey = group.toLowerCase().replace(/[()]/g, '');
                
                const match = clienteGrupoKey === groupKey;
                console.log(`   🔍 Comparando keys: "${clienteGrupoKey}" === "${groupKey}" → ${match}`);
                
                return match;
            });
        });
        console.log(`🏷️ Filtro de grupo: ${clientesAntes} → ${clientesFiltrados.length} clientes`);
    }
    ...
    const totalFiltros = (searchInput ? 1 : 0) + activeFiltros.group.length + activeFiltros.birthday.length + activeFiltros.status.length + activeFiltros.days.length + activeFiltros.date.length;
...
function carregarGruposDoLocalStorage() {
    console.warn('⚠️ carregarGruposDoLocalStorage() DEPRECATED - usar ApiClient.getGruposClientes()');
...
function renderGruposFilter(grupos) {
    ...
    grupos.forEach(grupo => {
        const key = grupo.nome.toLowerCase().replace(/\s+/g, '-').replace(/[()]/g, ''); // Converter para key válida
        filterLabels.group[key] = grupo.nome;
    });

    const html = grupos.map(grupo => {
        const key = grupo.nome.toLowerCase().replace(/\s+/g, '-').replace(/[()]/g, '');
        return `
...
function changeItemsPerPage() {
    ...
    console.log(`📄 Valor selecionado: "${selectedValue}" (tipo: ${typeof selectedValue})`);
    ...
    console.log(`📄 - allClients.length: ${window.allClients?.length || 'undefined'}`);
    
    // Se selecionou "Todos" (999999), mostrar todos os clientes
    if (selectedValue === '999999') {
        window.itemsPerPage = Math.max(window.totalClients || window.allClients.length || 100, 100);
        console.log(`📋 ✅ Configurado para mostrar TODOS: ${window.itemsPerPage} itens`);
    } else {
        window.itemsPerPage = parseInt(selectedValue);
        console.log(`📄 ✅ Configurado para ${window.itemsPerPage} itens por página`);
    }
...
function updateDisplayAndPagination() {
    console.log(`🔄 Atualizando display: página ${window.currentPage}, ${window.itemsPerPage} itens por página, total ${window.totalClients}`);
    
    // Calcular índices para a página atual
    const startIndex = (window.currentPage - 1) * window.itemsPerPage;
    const endIndex = Math.min(startIndex + window.itemsPerPage, window.totalClients);
...
function updatePaginationInfo() {
    ...
    const startIndex = window.totalClients > 0 ? (window.currentPage - 1) * window.itemsPerPage + 1 : 0;
    const endIndex = Math.min(window.currentPage * window.itemsPerPage, window.totalClients);
    ...
    if (pageInfo) {
        pageInfo.textContent = totalPages > 0 ? `Página ${window.currentPage} de ${totalPages}` : 'Página 0 de 0';
    }
    ...
    console.log(`📊 Paginação: Página ${window.currentPage}/${totalPages}, mostrando ${startIndex}-${endIndex} de ${window.totalClients} clientes`);
}
...
function updateClientsTable(clients) {
    ...
    clients.forEach((cliente, index) => {
        ...
        const newestBadge = isNewest ? '<span style="background: #27ae60; color: white; padding: 2px 6px; border-radius: 10px; font-size: 9px; font-weight: bold; margin-left: 8px;">MAIS RECENTE</span>' : '';
        
        // Status do cliente
        const isAtivo = cliente.ativo === true || cliente.ativo === 1 || cliente.ativo === 'true';
        const statusHTML = `
        ...
        const dataFormatada = cliente.createdAt ? 
            ...
            (cliente.data_cadastro ? new Date(cliente.data_cadastro).toLocaleDateString('pt-BR') : new Date().toLocaleDateString('pt-BR'));
        
        clientesHTML += '<tr class="client-row" onclick="viewClientDetails(' + cliente.id + ')" style="cursor: pointer;" data-grupo="' + (cliente.grupo_cliente || '') + '" data-cliente-id="' + cliente.id + '" data-nascimento="' + (cliente.data_nascimento || '') + '">';
        clientesHTML += '<td><div style="display: flex; align-items: center; gap: 12px;">' + avatar + '<div><strong>' + cliente.nome + '</strong>' + newestBadge + statusHTML + '</div></div></td>';
        ...
        if (Array.isArray(cliente.pets) && cliente.pets.length > 0) {
            ...
            clientesHTML += '<td class="pets-cell"><span class="pets-placeholder">—</span></td>';
        }

        clientesHTML += '<td>' + dataFormatada + '</td>';
        clientesHTML += '<td><div class="action-buttons" onclick="event.stopPropagation();"><button class="btn-edit" onclick="editClient(' + cliente.id + ')" title="Editar"><i class="fas fa-edit"></i></button><button class="btn-delete" onclick="deleteClient(' + cliente.id + ')" title="Excluir"><i class="fas fa-trash"></i></button></div></td>';
        clientesHTML += '</tr>';
    });
...
async function fetchPetsForClient(clientId, cell) {
    ...
    try {
        ...
        try { cell.style.minWidth = '160px'; } catch(e) {}
...
window.addEventListener('focus', () => {
    ...
    console.log('🔄 Página ganhou foco, tempo desde último foco:', timeDiff + 'ms');
...
window.addEventListener('pageshow', (event) => {
    ...
    if (event.persisted) {
        console.log('🔄 Página restaurada do cache (botão voltar), recarregando dados...');
        loadGruposFilter();
...
window.addEventListener('load', () => {
    ...
    if (lastUnload && (currentTime - parseInt(lastUnload)) < 10000) { // 10 segundos
        console.log('🔄 Detectado retorno recente de outra página, forçando atualização...');
        setTimeout(() => {
            loadGruposFilter();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (performance.navigation.type === 2) { // TYPE_BACK_FORWARD
        console.log('🔄 Detectado navegação via botão voltar/avançar, forçando atualização...');
        setTimeout(() => {
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        ...
        isPageVisible = true;
        if (hasLeftPage) {
            console.log('🔄 Página voltou a ficar visível após estar oculta, atualizando...');
...
window.addEventListener('focus', () => {
    const now = Date.now();
    if (now - lastUpdateCheck > 3000) { // 3 segundos
...
let storageCheckInterval = setInterval(() => {
    if (!document.hidden && document.hasFocus()) {
        ...
        if (lastChange) {
            ...
            if (now - changeTime < 30000) { // 30 segundos
...
window.addEventListener('storage', (e) => {
    ...
    if (e.key === 'clienteStatusUpdated') {
        console.log('🔄 Status de cliente atualizado em outra aba, recarregando...');
    ...
    if (!document.hidden && document.hasFocus()) {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                if (data.hasUpdates) {
                    console.log('🔄 Detectadas atualizações via polling, recarregando...');
                    loadClients();
                }
            }
        } catch (error) {
