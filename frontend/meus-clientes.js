class MeusClientesManager {
    ...
    updatePagination() {
        ...
        if (paginationInfo) {
            const startItem = this.filteredClientes.length === 0 ? 0 : (this.currentPage - 1) * this.itemsPerPage + 1;
            const endItem = Math.min(this.currentPage * this.itemsPerPage, this.filteredClientes.length);
