...
async function loadClients() {
    ...
    try {
        ...
        try {
            ...
            console.warn('⚠️ loadClients: fallback para rota relativa por timeout/erro', err && err.message);
            try {
        ...
        if (data.success && Array.isArray(data.clientes)) {
            console.log('🎯 loadClients CHAMANDO displayClients com', data.clientes.length, 'clientes');
            displayClients(data.clientes);
            console.log('✅ loadClients Clientes carregados silenciosamente:', data.clientes.length);
        } else {
        ...
        showError('Erro ao carregar clientes. Verifique se o servidor está rodando.');
...
function displayClients(clientes) {
    ...
    try {
        const htmlContent = clientes.map((cliente, index) => {
            ...
            const newestBadge = isNewest ? '<span style="background: #27ae60; color: white; padding: 2px 8px; border-radius: 12px; font-size: 10px; font-weight: bold;">MAIS RECENTE</span>' : '';
        ...
        console.log('📄 Primeiro cliente HTML:', htmlContent.substring(0, 200) + '...');
        ...
        console.log('✅ FIM displayClientes - Clientes exibidos na tabela com sucesso!');
        ...
        fetch('http://localhost:3000/api/clientes')
            ...
            .then(data => {
                ...
                if (data.success && Array.isArray(data.clientes)) {
                    ...
                    const simpleHTML = data.clientes.map(cliente => `
                        <tr>
                            <td>${cliente.nome}</td>
                            <td>${cliente.email || '---'}</td>
                            <td>${cliente.telefone}</td>
...
function refreshClientsFix() {
    ...
    fetchWithTimeout('http://localhost:3000/api/clientes', {}, 4000)
        .catch(err => {
            console.warn('⚠️ refreshClientsFix: fallback para rota relativa por timeout/erro', err && err.message);
