const { Produto, Cliente, Pet, Agendamento, Venda, sequelize } = require('../models');
...
exports.aniversariantes = async (req, res) => {
    try {
        ...
        // Buscar pets com data de nascimento definida (sem usar include devido a possíveis instâncias Sequelize diferentes)
        ...
        const petsFiltrados = pets.filter(pet => {
            const dataPet = pet.data_nascimento || pet.dataNascimento || pet.dataNascimento;
        ...
        const clienteIds = Array.from(new Set(petsFiltrados.map(p => p.cliente_id || p.clienteId).filter(Boolean)));
...
exports.oportunidadesVenda = async (req, res) => {
    try {
        ...
        vendas.forEach(venda => {
            if (!venda.clienteId || !venda.itens || !Array.isArray(venda.itens)) return;
            ...
            venda.itens.forEach(item => {
                ...
                if (!oportunidadesMap.has(key)) {
                    ...
                    const diasDesdeCompra = Math.floor((new Date() - new Date(venda.data)) / (1000 * 60 * 60 * 24));
                    
                    // Considerar oportunidade se passou mais de 30 dias
                    if (diasDesdeCompra >= 30) {
                        oportunidadesMap.set(key, {
                            clienteNome: venda.Cliente?.nome || venda.cliente || 'Cliente não identificado',
...
exports.levaTraz = async (req, res) => {
    try {
        ...
        // Buscar agendamentos do dia que mencionam "leva", "traz", "buscar" ou "transporte" nas observações
        ...
        const levaTrazList = agendamentos
            .filter(ag => {
                ...
                return obs.includes('leva') || obs.includes('traz') || 
                       obs.includes('buscar') || obs.includes('busca') || 
                       obs.includes('transporte') || obs.includes('entregar');
            })
            .map(ag => {
                const obs = (ag.observacoes || '').toLowerCase();
                let tipo = 'buscar';
...
exports.produtosVencimento = async (req, res) => {
    try {
        ...
        const produtosComValidade = produtos
            ...
            .filter(p => p.diasRestantes !== null && p.diasRestantes <= 90 && p.diasRestantes >= 0)
        ...
        vendas.forEach(v => {
            try {
                ...
                if (v.totais) {
                    ...
                    total = parseFloat(totaisObj.final || totaisObj.totalFinal || totaisObj.total || 0) || 0;
                }
                if (!isNaN(total)) {
                    soma += total;
                    contador++;
