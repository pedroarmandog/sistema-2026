...
exports.createCliente = async (req, res) => {
    try {
        const { 
            ...
            limite_credito, grupo_cliente, perfil_desconto, 
            como_nos_conheceu, observacoes, proximidade, ativo,
            telefones_adicionais, emails_adicionais
        } = req.body
        ...
        if (telefones_adicionais) {
            try {
                telefonesAdicionaisArray = typeof telefones_adicionais === 'string' 
        ...
        if (emails_adicionais) {
            try {
                emailsAdicionaisArray = typeof emails_adicionais === 'string' 
        ...
        const cliente = await Cliente.create({ 
            nome, cpf, rg, data_nascimento, idade, sexo, telefone, email,
            telefones_adicionais: telefonesAdicionaisArray,
            emails_adicionais: emailsAdicionaisArray,
            cep, endereco, numero, complemento, bairro, cidade, estado,
            limite_credito, grupo_cliente, perfil_desconto, 
            como_nos_conheceu, observacoes, proximidade, ativo, imagem_perfil
        })
...
exports.getAllClientes = async (req, res) => {
    try {
        ...
        const clientes = await Cliente.findAll({
            where: whereClause,
            attributes: ['id', 'nome', 'email', 'telefone', 'cpf', 'cidade', 'ativo', 'createdAt', 'grupo_cliente', 'imagem_perfil', 'data_nascimento'],
...
exports.getClienteById = async (req, res) => {
    try {
            ...
            include: [
                {
                    ...
                    attributes: ['id', 'nome', 'raca', 'genero', 'data_nascimento', 'porte', 'pelagem', 'chip', 'pedigree_rg', 'alergias', 'observacao', 'tags']
                },
                {
                    model: Venda,
                    as: 'vendas',
                    attributes: ['id', 'data', 'totais', 'totalPago', 'status', 'createdAt'],
                    required: false
                }
            ],
        ...
        clienteObj.historico = (clienteObj.vendas || []).map(v => ({
            ...
            data_hora: v.data,
            tipo: 'Venda',
            total: v.totais && v.totais.final ? v.totais.final : (v.totais && v.totais.total) || null,
...
exports.updateCliente = async (req, res) => {
    try {
        ...
        if (updates.telefones_adicionais) {
            try {
                updates.telefones_adicionais = typeof updates.telefones_adicionais === 'string' 
                    ? JSON.parse(updates.telefones_adicionais) 
                    : updates.telefones_adicionais;
            } catch (e) {
                console.log('Erro ao processar telefones_adicionais na atualização:', e);
            }
        }
        
        if (updates.emails_adicionais) {
            try {
                updates.emails_adicionais = typeof updates.emails_adicionais === 'string' 
