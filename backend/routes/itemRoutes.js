...
const blockedFilePath = path.join(__dirname, '..', 'data', 'blocked_medicamentos.json');
function readBlocked(){
    try{
        if(!fs.existsSync(blockedFilePath)) return [];
        const txt = fs.readFileSync(blockedFilePath, 'utf8');
        return JSON.parse(txt || '[]');
    }catch(e){ console.warn('Erro lendo blocked_medicamentos.json', e && e.message); return []; }
}
function writeBlocked(list){
    try{
        ...
        return true;
    }catch(e){ console.error('Erro gravando blocked_medicamentos.json', e && e.message); return false; }
}
...
router.get('/', async (req, res) => {
    try {
        const { q, excludeAgrupamento, agrupamento, fornecedorId, fornecedor } = req.query;
        ...
        if (excludeAgrupamento) {
            ...
            andConditions.push({ agrupamento: { [Op.notLike]: `%${excludeAgrupamento}%` } });
        }
        ...
        if (fornecedorId) {
            ...
            try {
                andConditions.push(Produto.sequelize.literal(`JSON_SEARCH(fornecedores, 'one', '${fid}', NULL, '$[*].id') IS NOT NULL`));
            } catch(e) {
                console.warn('Falha ao adicionar condição fornecedorId (literal):', e && e.message);
            }
        }
        ...
        if (fornecedor) {
            ...
            try {
                andConditions.push(Produto.sequelize.literal(`JSON_SEARCH(fornecedores, 'one', '${fv}', NULL, '$[*].nome') IS NOT NULL`));
            } catch(e) {
                console.warn('Falha ao adicionar condição fornecedor (literal):', e && e.message);
            }
        }
        ...
        try { console.log('GET /api/itens - query params:', req.query); } catch(e){}
        const itens = await Produto.findAll({ where, order: [['nome','ASC']], limit: 200 });
        try {
            console.log(`GET /api/itens - itens retornados: ${itens.length}`);
            if (itens.length > 0) {
                try { console.log('Amostra fornecedores do primeiro item:', JSON.stringify((itens[0].fornecedores || []).slice(0,3))); } catch(e){}
            }
        } catch(e) {}
        res.json(itens);
    } catch (error) {
...
router.get('/debug-fornecedor', async (req, res) => {
    try {
        ...
        if (!fornecedorId && !fornecedor) return res.status(400).json({ error: 'fornecedorId ou fornecedor query obrigatório' });
        ...
        try { console.log('/api/itens/debug-fornecedor - query', { fornecedorId, fornecedor, found: rows.length }); } catch(e){}
        ...
        for (const item of inputArr) {
            try {
                ...
                if (typeof item === 'string') {
                    ...
                    let f = await Fornecedor.findOne({ where: Produto.sequelize.where(Produto.sequelize.fn('LOWER', Produto.sequelize.col('nome')), lower) });
                    if (!f) f = await Fornecedor.findOne({ where: Produto.sequelize.where(Produto.sequelize.fn('LOWER', Produto.sequelize.col('nome')), { [Produto.sequelize.Op.like]: `%${lower}%` }) });
                    if (f) out.push({ id: String(f.id), nome: f.nome, fornecedor: f.nome });
                    ...
                    if (obj.id) {
                        ...
                        if (byId) { obj.id = String(byId.id); obj.nome = byId.nome; }
                        else if (obj.nome) {
                            const lower = String(obj.nome).toLowerCase();
                            let f = await Fornecedor.findOne({ where: Produto.sequelize.where(Produto.sequelize.fn('LOWER', Produto.sequelize.col('nome')), lower) });
                            if (!f) f = await Fornecedor.findOne({ where: Produto.sequelize.where(Produto.sequelize.fn('LOWER', Produto.sequelize.col('nome')), { [Produto.sequelize.Op.like]: `%${lower}%` }) });
                            if (f) { obj.id = String(f.id); obj.nome = f.nome; }
                        }
                ...
                console.warn('normalizeFornecedoresArray: erro processando item', item, e && e.message);
                out.push(item);
            }
        }
        return out;
    } catch (e) { console.warn('normalizeFornecedoresArray erro', e && e.message); return inputArr; }
}
...
router.get('/blocked', async (req, res) => {
    try{
        ...
        res.json(list);
    }catch(e){ console.error('Erro GET /blocked', e); res.status(500).json([]); }
});
...
router.post('/blocked', async (req, res) => {
    try{
        const body = req.body || {};
        const codigo = body.codigo !== undefined && body.codigo !== null ? String(body.codigo).trim() : null;
        const nome = body.nome ? String(body.nome).trim() : null;
        if(!codigo && !nome) return res.status(400).json({ error: 'codigo ou nome obrigatório' });
        const list = readBlocked();
        // evitar duplicatas por codigo ou nome
        const exists = list.find(i => (codigo && String(i.codigo) === String(codigo)) || (nome && String(i.nome).toLowerCase() === String(nome).toLowerCase()));
        if(exists) return res.json({ success:true, message: 'já bloqueado', item: exists });
        const item = { codigo: codigo || null, nome: nome || null, addedAt: new Date().toISOString() };
        list.push(item);
        writeBlocked(list);
        res.json({ success:true, item });
    }catch(e){ console.error('Erro POST /blocked', e); res.status(500).json({ error: 'Erro ao bloquear' }); }
});
...
router.delete('/blocked', async (req, res) => {
    try{
        ...
        list = list.filter(i => !(codigo && String(i.codigo) === String(codigo)) && !(nome && String(i.nome).toLowerCase() === String(nome).toLowerCase()));
        writeBlocked(list);
        res.json({ success:true, removed: before - list.length });
    }catch(e){ console.error('Erro DELETE /blocked', e); res.status(500).json({ error: 'Erro ao remover bloqueio' }); }
});
...
router.get('/last-saved', async (req, res) => {
    ...
    }catch(e){ console.error('Erro GET /last-saved', e); res.status(500).json({ lastSaved: null }); }
});
...
router.post('/last-saved', async (req, res) => {
    try{
        const body = req.body || {};
        const v = body.lastSaved !== undefined && body.lastSaved !== null ? String(body.lastSaved) : null;
        if(!v) return res.status(400).json({ error: 'lastSaved obrigatório' });
        writeLastSaved(v);
        res.json({ success:true, lastSaved: v });
    }catch(e){ console.error('Erro POST /last-saved', e); res.status(500).json({ error: 'Falha ao gravar lastSaved' }); }
});
...
router.delete('/last-saved', async (req, res) => {
    ...
    }catch(e){ console.error('Erro DELETE /last-saved', e); res.status(500).json({ error: 'Falha ao remover lastSaved' }); }
});
...
router.post('/', async (req, res) => {
    try {
        ...
        // validação mínima
        const nome = (data.nome || data.nome === 0) ? String(data.nome).trim() : (data.nome || data.descricao ? String(data.descricao).trim() : '');
        if (!nome) {
            return res.status(400).json({ error: 'Campo nome/descricao é obrigatório' });
        }
        ...
        try{
            ...
            if(exists){
                try{ console.log('🔔 [DEBUG] Item já existe no DB, retornando existente ao invés de criar:', exists.id || exists.codigo); }catch(e){}
                return res.status(200).json(exists.get ? exists.get({ plain: true }) : exists);
            }
        }catch(e){ console.warn('Erro verificando duplicata antes de criar:', e && e.message); }
        ...
        if (payload.id) {
            ...
            try {
                ...
                console.warn('⚠️ Falha ao calcular id sequencial, usando timestamp curto como fallback', qerr && qerr.message);
        ...
        try{
            if (payload.fornecedores) payload.fornecedores = await normalizeFornecedoresArray(payload.fornecedores);
        }catch(e){ console.warn('Erro normalizando fornecedores no POST:', e && e.message); }

        // coerções simples de tipos numéricos (evita erros no DB)
        if (payload.custoBase !== undefined && payload.custoBase !== null) payload.custoBase = Number(payload.custoBase) || 0;
        if (payload.preco !== undefined && payload.preco !== null) payload.preco = Number(payload.preco) || 0;
        if (payload.margem !== undefined && payload.margem !== null) payload.margem = Number(payload.margem) || 0;
        if (payload.estoqueAtual !== undefined && payload.estoqueAtual !== null) payload.estoqueAtual = parseInt(payload.estoqueAtual, 10) || 0;
        if (payload.estoqueMinimo !== undefined && payload.estoqueMinimo !== null) payload.estoqueMinimo = parseInt(payload.estoqueMinimo, 10) || 0;

        // log payload antes de criar (debug temporário)
            try{ console.log('🔔 [DEBUG] Criando produto com payload:', JSON.stringify(payload)); }catch(e){}
            const novo = await Produto.create(payload);
            try{ console.log('🔔 [DEBUG] Produto criado (DB) id=', String(novo.id)); }catch(e){}
            ...
            }catch(e){ console.warn('⚠️ Falha ao reconsultar produto criado; retornando objeto criado:', e && e.message); }
...
router.put('/permite-estoque-negativo', async (req, res) => {
    try {
        const body = req.body || {};
        const valor = (body.permiteEstoqueNegativo === undefined) ? null : String(body.permiteEstoqueNegativo);
        if (valor === null) return res.status(400).json({ error: 'Campo permiteEstoqueNegativo obrigatório' });

        // Atualiza todos os registros na tabela `itens`
        const [updatedCount] = await Produto.update({ permiteEstoqueNegativo: valor }, { where: {} });

        return res.json({ success: true, permiteEstoqueNegativo: valor, updated: Number(updatedCount || 0) });
    } catch (error) {
        console.error('Erro ao atualizar permiteEstoqueNegativo globalmente:', error);
        return res.status(500).json({ error: 'Erro interno ao atualizar flag global' });
    }
})
...
router.put('/:id', async (req, res) => {
    try {
        ...
        try{ console.log('🔔 [DEBUG] PUT /api/itens/' + id + ' - payload recebido:', JSON.stringify(data)); }catch(e){}
        ...
        try{
            if (data.ativo !== undefined && data.ativo !== null) {
                if (data.ativo === true || String(data.ativo).toLowerCase() === 'true') data.ativo = 'sim';
                else if (data.ativo === false || String(data.ativo).toLowerCase() === 'false') data.ativo = 'nao';
                else {
                    var s = String(data.ativo).toLowerCase();
                    if (s === 'sim' || s === 'nao') data.ativo = s;
                }
            }
        }catch(e){ console.debug('Erro normalizando ativo no PUT:', e); }
        ...
        } catch (e) { console.debug('Erro verificando alteração de preco:', e); }

            // Normalizar fornecedores recebidos no update (preencher id/nome quando possível)
            try{
                if (data.fornecedores) data.fornecedores = await normalizeFornecedoresArray(data.fornecedores);
            }catch(e){ console.warn('Erro normalizando fornecedores no PUT:', e && e.message); }

            await produto.update(data);
            try{
                const plain = (produto.get ? produto.get({ plain: true }) : Object.assign({}, produto));
                Object.keys(data||{}).forEach(function(k){ if(typeof data[k] !== 'undefined') plain[k] = data[k]; });
                return res.json(plain);
            }catch(e){ return res.json(produto); }
        try{ console.log('🔔 [DEBUG] Produto atualizado (DB):', JSON.stringify(produto.get ? produto.get({ plain: true }) : produto)); }catch(e){}
        console.debug(`PUT /api/itens/${id} - após update produto.ativo =`, produto.ativo);
...
router.put('/:id/estoque', async (req, res) => {
    try {
        ...
        if (quantidade === undefined || !operacao) return res.status(400).json({ error: 'Quantidade e operacao obrigatórios' });
        const produto = await Produto.findByPk(id);
        if (!produto) return res.status(404).json({ error: 'Produto não encontrado' });
        const atual = Number(produto.estoqueAtual) || 0;
        let novoEstoque = atual;
        if (operacao === 'reduzir') novoEstoque = Math.max(0, atual - Number(quantidade));
        ...
        try {
            ...
            await HistoricoEstoque.create({
                ...
                operacao: operacao === 'adicionar' ? 'Ajuste Manual (Entrada)' : 'Ajuste Manual (Saída)',
                estoqueAnterior: atual,
                quantidade: operacao === 'adicionar' ? Number(quantidade) : -Number(quantidade),
        ...
        res.json({ success: true, produto, estoqueAnterior: atual, novoEstoque, operacao, quantidade });
    } catch (error) {
...
router.post('/import', async (req, res) => {
    try {
        ...
        if (!Array.isArray(lista) || lista.length === 0) return res.status(400).json({ error: 'Lista vazia' });

        const results = [];
        for (const p of lista) {
            try {
                const id = p.id ? String(p.id) : String(Date.now()) + Math.floor(Math.random()*1000);
...
function parseDateString(validadeStr) {
    ...
    const m = String(validadeStr).trim().match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})$/);
    if (m) {
...
function calcularDiasParaValidade(validadeStr) {
    ...
    const v = new Date(validade.getFullYear(), validade.getMonth(), validade.getDate()).getTime();
...
function calcularStatusProduto(produto, defaults = { firstAlert: 30, secondAlert: 15 }) {
    const dias = calcularDiasParaValidade(produto.validade)
    if (dias === null || isNaN(dias)) return { produto, dias: null, status: 'normal', mensagem: null }
    ...
    if (produto.perfilValidade && (produto.perfilValidade.alerta1 || produto.perfilValidade.alerta2)) {
    ...
    if (dias <= low) {
        return { produto, dias, status: 'critical', mensagem: 'Produto próximo ao vencimento' }
    }
    if (dias <= high) {
        return { produto, dias, status: 'warning', mensagem: 'Produto próximo do vencimento' }
    }
