...
async function parseNFeXml(xml) {
  ...
  const fornecedor = { cnpj: emit?.CNPJ || emit?.cnpj || undefined, nome: emit?.xNome || emit?.xnome || undefined };
  const itens = [];
  ...
  return { chave, numero, dataEmissao, valorTotal, fornecedor, itens };
}
