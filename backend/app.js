...
if (SKIP_DB_SYNC) console.log('⚠️ SKIP_DB_SYNC=true — sincronização do DB foi desabilitada para esta execução');
app.use(cors({
...
if (!SKIP_DB_SYNC) {
    try {
      ...
      console.log('Mounted entrada-mercadoria from dist');
        ...
        console.warn('entrada-mercadoria router not found in dist. To enable, run prisma generate/migrate and ensure dist exists.');
        ...
        app.post('/api/entrada/manual', (req, res) => {
            try {
                ...
                mapped.categoriaFinanceira = mapped.categoriaFinanceira || mapped.tipoEntrada || mapped.tipo || null;
                ...
                console.log('[api/entrada/manual][stub] POST payload:', payload, '=> saved:', created);
        ...
        app.put('/api/entrada/manual/:id', (req, res) => {
            try {
                ...
                if (idx === -1) {
                    const created = Object.assign({}, mapped, { id, updatedAt: new Date() });
                    __DEV_ENTRADAS.unshift(created);
                    console.log('[api/entrada/manual][stub] PUT create payload:', body, '=> saved:', created);
                    return res.json(created);
                }
                __DEV_ENTRADAS[idx] = Object.assign({}, __DEV_ENTRADAS[idx], mapped, { id, updatedAt: new Date() });
                console.log('[api/entrada/manual][stub] PUT update id=', id, 'payload:', body, '=> saved:', __DEV_ENTRADAS[idx]);
    ...
    console.log('⚠️ SKIP_DB_SYNC=true — forçando stubs de /api/entrada (desenvolvimento) com tentativa de persistência em DB');
    const __DEV_ENTRADAS = [];
    ...
    app.get('/api/entrada/manual', async (req, res) => {
        ...
        try {
            if (Entrada && typeof Entrada.findAll === 'function') {
                const rows = await Entrada.findAll({ order: [['createdAt', 'DESC']], limit: 200 });
                return res.json(rows.map(r => r.toJSON()));
            }
        } catch (err) {
            console.warn('Falha ao buscar entradas do DB, usando fallback em memória:', err && err.message);
        }
        return res.json(__DEV_ENTRADAS);
    });
    ...
    app.get('/api/entrada', async (req, res) => {
        ...
        try {
            ...
            console.warn('Erro ao buscar entrada por chave no DB:', err && err.message);
        }
    ...
    app.post('/api/entrada/manual', async (req, res) => {
        ...
        try {
            if (Entrada && typeof Entrada.create === 'function') {
                ...
                saved.itens = saved.itens && saved.itens.length>0 ? saved.itens : (mapped.itens || []);
                saved.observacao = saved.observacao || mapped.observacao || '';
                saved.categoriaFinanceira = saved.categoriaFinanceira || mapped.categoriaFinanceira || '';
                saved.tipoEntrada = saved.tipoEntrada || saved.categoriaFinanceira || mapped.tipoEntrada || '';
                saved.valorTotal = (saved.valorTotal != null) ? saved.valorTotal : (mapped.valorTotal || 0);
                saved.situacao = saved.situacao || mapped.situacao || 'Pendente';
                console.log('[api/entrada/manual][db] POST payload:', payload, '=> saved:', saved);
                ...
                try{
                    ...
                    if (saved && String(saved.situacao).toLowerCase().includes('final')){
                        ...
                        const itens = Array.isArray(saved.itens) ? saved.itens : (payload.itens || []);
                        for(const it of itens){
                            try{
                                ...
                                if(!prod && it.codigo){ prod = await Produto.findOne({ where: { codigo: String(it.codigo) } }); }
                                if(!prod) continue;
                                const atual = Number(prod.estoqueAtual) || 0;
                                const novo = atual + quantidade;
                                const updatedProd = await prod.update({ estoqueAtual: novo });
                                if(HistoricoEstoque && typeof HistoricoEstoque.create === 'function'){
                                    await HistoricoEstoque.create({ produtoId: String(prod.id), produtoNome: prod.nome, dataMovimento: saved.dataEmissao || new Date(), operacao: 'Entrada', estoqueAnterior: atual, quantidade: quantidade, novoEstoque: novo, observacao: `Entrada automática via nota ${saved.id}` });
                                }
                                try{ saved.updatedProducts.push(updatedProd.get ? updatedProd.get({ plain: true }) : updatedProd); }catch(e){ saved.updatedProducts.push(updatedProd); }
                            }catch(e){ console.warn('Erro ajustando estoque (POST entrada):', e && e.message); }
                        }
                    }
                }catch(e){ console.warn('Erro no pós-processamento de entrada (POST):', e && e.message); }
                return res.json(saved);
            }
        } catch (err) {
            console.warn('Erro ao criar entrada no DB, usando fallback em memória:', err && err.message);
        }
        // Fallback em memória
        try {
    ...
    app.put('/api/entrada/manual/:id', async (req, res) => {
        ...
        try {
            if (Entrada && typeof Entrada.findByPk === 'function') {
                ...
                if (existing) {
                    ...
                    updated.itens = updated.itens && updated.itens.length>0 ? updated.itens : (mapped.itens || []);
                    updated.observacao = updated.observacao || mapped.observacao || '';
                    updated.categoriaFinanceira = updated.categoriaFinanceira || mapped.categoriaFinanceira || '';
                    updated.tipoEntrada = updated.tipoEntrada || updated.categoriaFinanceira || mapped.tipoEntrada || '';
                    updated.valorTotal = (updated.valorTotal != null) ? updated.valorTotal : (mapped.valorTotal || 0);
                    updated.situacao = updated.situacao || mapped.situacao || 'Pendente';
                    console.log('[api/entrada/manual][db] PUT update id=', id, 'payload:', body, '=> saved:', updated);
                        // pós-processar ajuste de estoque se situação for Finalizado e retornar produtos atualizados
                        try{
                            updated.updatedProducts = [];
                            if (updated && String(updated.situacao).toLowerCase().includes('final')){
                                const Produto = require('./models').Produto;
                                const HistoricoEstoque = require('./models').HistoricoEstoque;
                                const itens = Array.isArray(updated.itens) ? updated.itens : (mapped.itens || []);
                                for(const it of itens){
                                    try{
                                        const quantidade = Number(it.quantidade || it.qty || it.qtd || 0) || 0;
                                        if(quantidade === 0) continue;
                                        ...
                                        const updatedProd = await prod.update({ estoqueAtual: novo });
                                        if(HistoricoEstoque && typeof HistoricoEstoque.create === 'function'){
                                            await HistoricoEstoque.create({ produtoId: String(prod.id), produtoNome: prod.nome, dataMovimento: updated.dataEmissao || new Date(), operacao: 'Entrada', estoqueAnterior: atual, quantidade: quantidade, novoEstoque: novo, observacao: `Entrada automática via nota ${updated.id}` });
                                        }
                                        try{ updated.updatedProducts.push(updatedProd.get ? updatedProd.get({ plain: true }) : updatedProd); }catch(e){ updated.updatedProducts.push(updatedProd); }
                                    }catch(e){ console.warn('Erro ajustando estoque (PUT update entrada):', e && e.message); }
                                }
                            }
                        }catch(e){ console.warn('Erro no pós-processamento de entrada (PUT update):', e && e.message); }
                        return res.json(updated);
                }
                // criar se não existir
                const created = await Entrada.create(Object.assign({}, mapped, { id }));
                const createdJson = created.toJSON();
                createdJson.itens = createdJson.itens && createdJson.itens.length>0 ? createdJson.itens : (mapped.itens || []);
                createdJson.observacao = createdJson.observacao || mapped.observacao || '';
                createdJson.categoriaFinanceira = createdJson.categoriaFinanceira || mapped.categoriaFinanceira || '';
                createdJson.tipoEntrada = createdJson.tipoEntrada || createdJson.categoriaFinanceira || mapped.tipoEntrada || '';
                createdJson.valorTotal = (createdJson.valorTotal != null) ? createdJson.valorTotal : (mapped.valorTotal || 0);
                createdJson.situacao = createdJson.situacao || mapped.situacao || 'Pendente';
                console.log('[api/entrada/manual][db] PUT create id=', id, 'payload:', body, '=> saved:', createdJson);
                // pós-processar ajuste de estoque se finalizado e retornar produtos atualizados
                try{
                    createdJson.updatedProducts = [];
                    if (createdJson && String(createdJson.situacao).toLowerCase().includes('final')){
                        const Produto = require('./models').Produto;
                        const HistoricoEstoque = require('./models').HistoricoEstoque;
                        const itens = Array.isArray(createdJson.itens) ? createdJson.itens : (mapped.itens || []);
                        for(const it of itens){
                            try{
                                const quantidade = Number(it.quantidade || it.qty || it.qtd || 0) || 0;
                                if(quantidade === 0) continue;
                                ...
                                const updatedProd = await prod.update({ estoqueAtual: novo });
                                if(HistoricoEstoque && typeof HistoricoEstoque.create === 'function'){
                                    await HistoricoEstoque.create({ produtoId: String(prod.id), produtoNome: prod.nome, dataMovimento: createdJson.dataEmissao || new Date(), operacao: 'Entrada', estoqueAnterior: atual, quantidade: quantidade, novoEstoque: novo, observacao: `Entrada automática via nota ${createdJson.id}` });
                                }
                                try{ createdJson.updatedProducts.push(updatedProd.get ? updatedProd.get({ plain: true }) : updatedProd); }catch(e){ createdJson.updatedProducts.push(updatedProd); }
                            }catch(e){ console.warn('Erro ajustando estoque (PUT create entrada):', e && e.message); }
                        }
                    }
                }catch(e){ console.warn('Erro no pós-processamento de entrada (PUT create):', e && e.message); }
                return res.json(createdJson);
            }
        } catch (err) {
            console.warn('Erro ao atualizar/criar entrada no DB, fallback em memória:', err && err.message);
        }
        ...
        try {
            const idx = __DEV_ENTRADAS.findIndex(x => String(x.id) === String(id));
            if (idx === -1) {
                const created = Object.assign({}, body || {}, { id, updatedAt: new Date() });
                __DEV_ENTRADAS.unshift(created);
                return res.json(created);
            }
            __DEV_ENTRADAS[idx] = Object.assign({}, __DEV_ENTRADAS[idx], body, { id, updatedAt: new Date() });
            return res.json(__DEV_ENTRADAS[idx]);
        } catch (e) {
            console.error('Stub PUT /api/entrada/manual/:id error', e);
            return res.status(500).json({ error: 'Erro interno stub' });
...
app.get('/api/entrada/manual', async (req, res) => {
    try {
        ...
        if (Entrada && typeof Entrada.findAll === 'function') {
            const rows = await Entrada.findAll({ order: [['createdAt', 'DESC']], limit: 500 });
            return res.json(rows.map(r => r.toJSON()));
        }
    } catch (err) {
        console.warn('Erro ao buscar entradas (GET /api/entrada/manual):', err && err.message);
    }
...
app.get('/api/entrada/manual/:id', async (req, res) => {
    ...
    try {
        ...
        if (typeof __DEV_ENTRADAS !== 'undefined' && Array.isArray(__DEV_ENTRADAS)) {
...
app.put('/api/entrada/manual/:id', async (req, res) => {
    ...
    return res.status(500).json({ error: 'Não foi possível atualizar a entrada' });
});
...
app.get('/api/saida/manual', async (req, res) => {
    try {
        ...
        if (Saida && typeof Saida.findAll === 'function') {
            const rows = await Saida.findAll({ order: [['createdAt', 'DESC']], limit: 500 });
            return res.json(rows.map(r => r.toJSON()));
        }
    } catch (err) {
        console.warn('Erro ao buscar saidas (GET /api/saida/manual):', err && err.message);
    }
    // Retornar o fallback em memória (incluir cópia para segurança)
    try{ return res.json(Array.isArray(__DEV_SAIDAS) ? __DEV_SAIDAS.slice(0) : []); }catch(e){ return res.json([]); }
});
...
app.get('/api/saida/manual/:id', async (req, res) => {
    ...
    } catch (err) { console.warn('Erro GET /api/saida/manual/:id', err && err.message); }
...
app.post('/api/saida/manual', async (req, res) => {
    ...
    try {
        ...
        const HistoricoEstoque = models && models.HistoricoEstoque ? models.HistoricoEstoque : null;
        const mapped = Object.assign({}, payload);
        mapped.situacao = mapped.situacao || mapped.status || 'Pendente';
        mapped.dataEmissao = mapped.dataEmissao || mapped.data || null;
        mapped.valorTotal = mapped.valorTotal || mapped.valor || mapped.total || 0;
        ...
        if (Saida && typeof Saida.create === 'function') {
            ...
            saved.updatedProducts = [];
            if (saved && String(saved.situacao).toLowerCase().includes('final')){
                const itens = Array.isArray(saved.itens) ? saved.itens : (mapped.itens || []);
                for(const it of itens){
                    try{
                        const quantidade = Number(it.quantidade || it.qty || it.qtd || 0) || 0;
                        if(quantidade === 0) continue;
                        if(Produto){
                            ...
                            const updatedProd = await prod.update({ estoqueAtual: novo });
                            if(HistoricoEstoque && typeof HistoricoEstoque.create === 'function'){
                                await HistoricoEstoque.create({ produtoId: String(prod.id), produtoNome: prod.nome, dataMovimento: saved.dataEmissao || new Date(), operacao: 'Saída', estoqueAnterior: atual, quantidade: quantidade, novoEstoque: novo, observacao: `Saída automática via nota ${saved.id}` });
                            }
                            try{ saved.updatedProducts.push(updatedProd.get ? updatedProd.get({ plain: true }) : updatedProd); }catch(e){ saved.updatedProducts.push(updatedProd); }
                        }
                    }catch(e){ console.warn('Erro ajustando estoque (POST saida db):', e && e.message); }
                }
            }
            return res.json(saved);
        }
        ...
        if (created && String(created.situacao).toLowerCase().includes('final')){
            const itens = Array.isArray(created.itens) ? created.itens : (mapped.itens || []);
            for(const it of itens){
                try{
                    const quantidade = Number(it.quantidade || it.qty || it.qtd || 0) || 0;
                    if(quantidade === 0) continue;
                    ...
                    if(Produto){
                        ...
                        const updatedProd = await prod.update({ estoqueAtual: novo });
                        if(HistoricoEstoque && typeof HistoricoEstoque.create === 'function'){
                            await HistoricoEstoque.create({ produtoId: String(prod.id), produtoNome: prod.nome, dataMovimento: created.dataEmissao || new Date(), operacao: 'Saída', estoqueAnterior: atual, quantidade: quantidade, novoEstoque: novo, observacao: `Saída automática via nota ${created.id}` });
                        }
                        try{ created.updatedProducts.push(updatedProd.get ? updatedProd.get({ plain: true }) : updatedProd); }catch(e){ created.updatedProducts.push(updatedProd); }
                    }
                }catch(e){ console.warn('Erro ajustando estoque (POST saida):', e && e.message); }
            }
        }
        __DEV_SAIDAS.unshift(created);
        return res.json(created);
    } catch (err) {
        console.warn('Erro POST /api/saida/manual fallback em memória:', err && err.message);
    }
    try { const id = payload.id || ('sim-' + Date.now()); const created = Object.assign({}, payload, { id, createdAt: new Date() }); __DEV_SAIDAS.unshift(created); return res.json(created);}catch(e){ console.error('Stub POST /api/saida/manual error', e); return res.status(500).json({ error: 'Erro interno stub' }); }
});
...
app.put('/api/saida/manual/:id', async (req, res) => {
    ...
    try {
            ...
            try{
                ...
                if (saved && String(saved.situacao).toLowerCase().includes('final')){
                    ...
                    for(const it of itens){
                        ...
                        }catch(e){ console.warn('Erro ajustando estoque (PUT saida db):', e && e.message); }
                    }
                }
                return res.json(saved);
            }catch(e){ console.warn('Erro atualizando Saida no DB, fallback para memória:', e && e.message); }
        }
        ...
        if (idx === -1) __DEV_SAIDAS.unshift(saved); else __DEV_SAIDAS[idx] = saved;
        ...
        if (saved && String(saved.situacao).toLowerCase().includes('final')){
            ...
            for(const it of itens){
                ...
                }catch(e){ console.warn('Erro ajustando estoque (PUT saida):', e && e.message); }
            }
        }
        return res.json(saved);
    } catch (err) {
        console.warn('Erro PUT /api/saida/manual/:id (fallback):', err && err.message);
    }
    try { const idx = __DEV_SAIDAS.findIndex(x => String(x.id) === String(id)); if(idx===-1){ const created = Object.assign({}, body||{}, { id, updatedAt: new Date() }); __DEV_SAIDAS.unshift(created); return res.json(created);} __DEV_SAIDAS[idx] = Object.assign({}, __DEV_SAIDAS[idx], body, { id, updatedAt: new Date() }); return res.json(__DEV_SAIDAS[idx]); }catch(e){ return res.status(500).json({ error: 'Não foi possível atualizar a saída' }); }
});
...
app.delete('/api/saida/manual/:id', async (req, res) => {
    ...
    try {
        const idx = __DEV_SAIDAS.findIndex(x => String(x.id) === String(id));
        if (idx !== -1) { __DEV_SAIDAS.splice(idx, 1); return res.json({ success: true }); }
    } catch (e) { console.warn('Erro DELETE /api/saida/manual/:id', e && e.message); }
    return res.status(404).json({ error: 'Saída não encontrada' });
});
...
(function ensureTipoEntradasData(){
    try{
        ...
        if(!fs.existsSync(dataFile)){
            const defaults = [
                ...
                { id: Date.now()+3, descricao: 'ENTRADA DE TRANSFERENCIA', ativo: true },
    ...
    }catch(e){ console.warn('Falha ao garantir arquivo tipo_entradas.json', e && e.message); }
})();
...
app.get('/api/editar-produto', async (req, res) => {
    try {
        ...
        const produtos = await Produto.findAll({ where, order: [['ultimoPrecoAlterado','DESC']], limit: 1000 });

        // map to lightweight response
        const out = produtos.map(p => ({ id: p.id, codigo: p.codigo, nome: p.nome, estoque: p.estoqueAtual, ultimoPrecoAlterado: p.ultimoPrecoAlterado }));
        res.json(out);
    } catch (err) {
        console.error('Erro /api/editar-produto', err);
        res.status(500).json({ error: 'Erro interno buscando produtos com preços alterados' });
    }
});
...
if (!SKIP_DB_SYNC) {
    ...
    console.log('⚠️ SKIP_DB_SYNC habilitado — pulando sequelize.sync() principal');
}
...
try {
    ...
    if (!SKIP_DB_SYNC) {
        if (ProdutoModel && typeof ProdutoModel.sync === 'function') {
            ProdutoModel.sync({ alter: true }).then(() => {
                console.log('Tabela `itens` sincronizada/atualizada ✅');
            }).catch(err => {
                console.warn('Não foi possível sincronizar tabela `itens`:', err && err.message);
            });
        }
    } else {
    ...
    console.warn('Aviso: falha ao carregar/sincronizar modelo Produto:', e && e.message);
}
...
try {
    ...
    if (!SKIP_DB_SYNC) {
        if (Fornecedor && typeof Fornecedor.sync === 'function') {
            Fornecedor.sync({ alter: true }).then(() => {
                console.log('Tabela `fornecedores` sincronizada/atualizada ✅');
            }).catch(err => {
                console.warn('Não foi possível sincronizar tabela `fornecedores`:', err && err.message);
            });
        }
    } else {
        console.log('⚠️ SKIP_DB_SYNC habilitado — pulando sync da tabela `fornecedores`');
    }
} catch (e) {
    console.warn('Aviso: falha ao carregar/sincronizar modelo Fornecedor:', e && e.message);
}
...
try {
    ...
    if (!SKIP_DB_SYNC) {
        if (ImpressoraConfig && typeof ImpressoraConfig.sync === 'function') {
            ImpressoraConfig.sync({ alter: true }).then(() => {
                console.log('Tabela `impressora_config` sincronizada/atualizada ✅');
            }).catch(err => {
                console.warn('Não foi possível sincronizar tabela `impressora_config`:', err && err.message);
            });
        }
    } else {
        console.log('⚠️ SKIP_DB_SYNC habilitado — pulando sync da tabela `impressora_config`');
    }
} catch (e) {
    console.warn('Aviso: falha ao carregar/sincronizar modelo ImpressoraConfig:', e && e.message);
}
...
try {
    ...
    if (!SKIP_DB_SYNC) {
        if (ModeloEtiquetaConfig && typeof ModeloEtiquetaConfig.sync === 'function') {
            ModeloEtiquetaConfig.sync({ alter: true }).then(() => {
                console.log('Tabela `modelo_etiqueta_config` sincronizada/atualizada ✅');
            }).catch(err => {
                console.warn('Não foi possível sincronizar tabela `modelo_etiqueta_config`:', err && err.message);
            });
        }
    } else {
        console.log('⚠️ SKIP_DB_SYNC habilitado — pulando sync da tabela `modelo_etiqueta_config`');
    }
} catch (e) {
    console.warn('Aviso: falha ao carregar/sincronizar modelo ModeloEtiquetaConfig:', e && e.message);
}
...
app.get('/api/impressora', async (req, res) => {
    try {
        const ImpressoraConfig = require('./models/ImpressoraConfig');
        const registro = await ImpressoraConfig.findOne({ order: [['updatedAt', 'DESC']] });
        if (!registro) return res.json({ tipo: null });
        res.json({ tipo: registro.tipo, usuario: registro.usuario, updatedAt: registro.updatedAt });
    } catch (err) {
...
app.post('/api/impressora', async (req, res) => {
    try {
        ...
        const novo = await ImpressoraConfig.create({ tipo: String(tipo), usuario: usuario ? String(usuario) : null });
        res.json({ success: true, registro: { id: novo.id, tipo: novo.tipo, usuario: novo.usuario, updatedAt: novo.updatedAt } });
    } catch (err) {
        console.error('Erro POST /api/impressora', err);
        res.status(500).json({ error: 'Erro ao salvar configuração de impressora' });
    }
});
...
app.get('/api/modelo-etiqueta', async (req, res) => {
    try {
        const ModeloEtiquetaConfig = require('./models/ModeloEtiquetaConfig');
        const registro = await ModeloEtiquetaConfig.findOne({ order: [['updatedAt', 'DESC']] });
        if (!registro) return res.json({ modelo: null });
        res.json({ modelo: registro.modelo, usuario: registro.usuario, updatedAt: registro.updatedAt });
    } catch (err) {
        console.error('Erro GET /api/modelo-etiqueta', err);
        res.status(500).json({ error: 'Erro ao obter configuração de modelo de etiqueta' });
    }
});
...
app.post('/api/modelo-etiqueta', async (req, res) => {
    try {
        ...
        if (!modelo) return res.status(400).json({ error: 'Campo modelo é obrigatório' });
        const ModeloEtiquetaConfig = require('./models/ModeloEtiquetaConfig');
        const novo = await ModeloEtiquetaConfig.create({ modelo: String(modelo), usuario: usuario ? String(usuario) : null });
        res.json({ success: true, registro: { id: novo.id, modelo: novo.modelo, usuario: novo.usuario, updatedAt: novo.updatedAt } });
    } catch (err) {
        console.error('Erro POST /api/modelo-etiqueta', err);
        res.status(500).json({ error: 'Erro ao salvar configuração de modelo de etiqueta' });
    }
});
...
try {
    ...
    if (!SKIP_DB_SYNC) {
        if (PerfilProduto && typeof PerfilProduto.sync === 'function') {
            PerfilProduto.sync({ alter: true }).then(() => {
                console.log('Tabela `perfis_produto` sincronizada/atualizada ✅');
            }).catch(err => {
                console.warn('Não foi possível sincronizar tabela `perfis_produto`:', err && err.message);
            });
        }
    } else {
        console.log('⚠️ SKIP_DB_SYNC habilitado — pulando sync da tabela `perfis_produto`');
    }
} catch (e) {
    console.warn('Aviso: falha ao carregar/sincronizar modelo PerfilProduto:', e && e.message);
}

try {
    const descontoModule = require('./models/DescontoRelacao');
    const DescontoRelacao = descontoModule && descontoModule.DescontoRelacao ? descontoModule.DescontoRelacao : null;
    if (!SKIP_DB_SYNC) {
        if (DescontoRelacao && typeof DescontoRelacao.sync === 'function') {
            DescontoRelacao.sync({ alter: true }).then(() => {
                console.log('Tabela `descontos_relacoes` sincronizada/atualizada ✅');
            }).catch(err => {
                console.warn('Não foi possível sincronizar tabela `descontos_relacoes`:', err && err.message);
            });
        }
    } else {
        console.log('⚠️ SKIP_DB_SYNC habilitado — pulando sync da tabela `descontos_relacoes`');
    }
} catch (e) {
    console.warn('Aviso: falha ao carregar/sincronizar modelo DescontoRelacao:', e && e.message);
}
...
try {
    ...
    if (!SKIP_DB_SYNC) {
        if (PerfilCliente && typeof PerfilCliente.sync === 'function') {
            PerfilCliente.sync({ alter: true }).then(() => {
                console.log('Tabela `perfis_cliente` sincronizada/atualizada ✅');
            }).catch(err => {
                console.warn('Não foi possível sincronizar tabela `perfis_cliente`:', err && err.message);
            });
        }
    } else {
        console.log('⚠️ SKIP_DB_SYNC habilitado — pulando sync da tabela `perfis_cliente`');
    }
} catch (e) {
    console.warn('Aviso: falha ao carregar/sincronizar modelo PerfilCliente:', e && e.message);
}
    ...
    if (!SKIP_DB_SYNC) {
        if (Usuario && typeof Usuario.sync === 'function') {
            Usuario.sync({ alter: true }).then(() => {
                ...
                console.error('Erro ao sincronizar tabela `usuarios`:', err && err.message);
            });
        }
    } else {
        console.log('⚠️ SKIP_DB_SYNC habilitado — pulando sync da tabela `usuarios`');
    }
} catch (e) {
