...
const agrupamentosData = [
  {
    ...
    subgrupos: JSON.stringify(['Banho e Tosa', 'BANHO E TOSA', 'BANHOS AVULSOS', 'TOSAS', 'ADICIONAIS', 'PLANOS'])
  },
  {
    name: 'CONSULTÓRIO',
    subgrupos: JSON.stringify(['CONSULTAS', 'EXAMES', 'PROCEDIMENTOS CLINICOS', 'PROCEDIMENTO CIRURGICO'])
  },
  {
    name: 'LOJA',
    subgrupos: JSON.stringify(['RAÇÕES', 'MEDICAMENTOS', 'VACINAS', 'VERMIFUGOS', 'ANTIPARASITARIOS', 'BRINQUEDOS', 'HIGIENE', 'ROUPAS'])
  }
];
...
async function popularAgrupamentos() {
  try {
    ...
    console.log(`📊 Total de agrupamentos inseridos: ${agrupamentosData.length}`);

    // Verifica os dados inseridos
    const count = await Agrupamento.count();
    console.log(`\n🔍 Verificação: ${count} agrupamentos na tabela`);

    const todos = await Agrupamento.findAll();
