...
(async function(){
    try {
        ...
        const entradas = await Entrada.findAll({ attributes: ['id','fornecedor'], where: { fornecedor: { [Op.ne]: null } }, order: [['createdAt','DESC']] });
        const fornecedorRows = await Fornecedor.findAll({ attributes: ['id','nome','cnpj'], order: [['nome','ASC']] });
        ...
        const missing = [];
        nomesEntradas.forEach(n => { if (!nomesFornecedores.has(n)) missing.push(n); });
