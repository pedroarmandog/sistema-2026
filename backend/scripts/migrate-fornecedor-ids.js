...
async function findFornecedorByName(name) {
    ...
    let f = await Fornecedor.findOne({ where: sequelize.where(sequelize.fn('LOWER', sequelize.col('nome')), clean) });
    if (f) return f;
    // 2) tentamos LIKE %name% (mais permissivo)
    f = await Fornecedor.findOne({ where: sequelize.where(sequelize.fn('LOWER', sequelize.col('nome')), { [Op.like]: `%${clean}%` }) });
    return f;
}
...
async function processOne(produto, dryRun) {
        ...
        try {
            ...
            if (typeof item === 'string') {
                ...
                if (obj.id) {
                    ...
                    const f = await findFornecedorByName(name);
                    if (f) { obj.id = String(f.id); obj.nome = f.nome; changed = true; }
                    else obj.nome = name;
                }
                out.push(obj);
            } else {
    ...
    try {
        ...
        if (oldStr !== newStr) {
            changed = true;
            if (!dryRun) {
                await Produto.update({ fornecedores: out }, { where: { id: produto.id } });
...
async function main() {
    ...
    const dryRun = argv.indexOf('--dry-run') !== -1 && argv.indexOf('--apply') === -1;
    const apply = argv.indexOf('--apply') !== -1;
    const limitIndex = argv.indexOf('--limit');
    const limit = (limitIndex !== -1 && argv[limitIndex+1]) ? parseInt(argv[limitIndex+1],10) : null;

    console.log('Iniciando migração de fornecedorIds', { dryRun, apply, limit });
    try {
        const where = {};
        const opts = { where, order: [['nome','ASC']] };
        if (limit) opts.limit = limit;
        const produtos = await Produto.findAll(opts);
        console.log(`Total produtos carregados: ${produtos.length}`);
        let totalChanged = 0;
        let totalUpdated = 0;
        let processed = 0;
        for (const p of produtos) {
            ...
            } catch (e) { console.error('Erro processando produto', p.id, e && e.message); }
        }
        console.log('Migração finalizada', { processed, totalChanged, totalUpdated, dryRun });
        if (dryRun) console.log('Dry-run: nenhuma alteração foi aplicada. Rode com --apply para gravar.');
    } catch (err) {
